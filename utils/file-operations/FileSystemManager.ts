...
class FileSystemManager {
  ...
  async loadBucketStructure(
    bucketName: string,
    forceRefresh: boolean = false,
  ): Promise<BucketTreeStructure | null> {
    try {
      // Check local storage first if not forcing refresh
      if (!forceRefresh) {
        const localStructure =
          await this.localStorage.getBucketStructure(bucketName);
        if (localStructure) {
    ...
    try {
      ...
      if (!forceRefresh) {
        const localStructures =
          await this.localStorage.getAllBucketStructures();
        if (localStructures.length > 0) {
          ...
          this.debugger.logOperation(
            ...
            { source: "local" },
            {
              localStructures,
              success: true,
              structureCount: localStructures.length,
            },
          );
      ...
      for (const result of data.results) {
        ...
        await this.localStorage.updateBucketStructure(
          result.bucket,
          result.structure,
        );
  ...
  private async updateStructureAfterFileOperation(
    ...
    switch (operation) {
      ...
      case "delete":
        structure.contents = structure.contents.filter(
          (item) => item.path !== path,
        );
      ...
      case "rename":
        if (newPath) {
          const itemIndex = structure.contents.findIndex(
            (item) => item.path === path,
          );
          if (itemIndex !== -1) {
  ...
  async uploadFile(
    ...
    try {
      ...
      const serverMetadata: StorageMetadata = {
        ...
        lastModified:
          listData?.[0]?.metadata?.lastModified ??
      ...
      await this.updateStructureAfterFileOperation(
        bucketName,
        path,
        "add",
        fileType,
      ...
      this.debugger.logOperation(
        "uploadFile",
        { bucketName, path },
        { success: true, signedUrl, metadata: serverMetadata },
      );
  ...
  async renameFolder(
    ...
    try {
      ...
      const itemsToMove = structure.contents.filter(
        (item) => item.path.startsWith(oldPath + "/") || item.path === oldPath,
      );
      ...
      for (const item of itemsToMove) {
        ...
        if (item.type !== "FOLDER") {
          await this.localStorage.renameFile(
            bucketName,
            item.path,
            newItemPath,
          );
      ...
      await this.localStorage.updateFolderStructure(
        bucketName,
        oldPath,
        newPath,
      );
  ...
  async refreshFolderContents(
    ...
    try {
      ...
      const localStructure = await this.localStorage.getFolderContents(
        bucketName,
        folderPath,
      );
  ...
  async getPublicUrl(bucketName: string, filePath: string): Promise<string> {
    try {
      ...
      await this.localStorage.cachePublicUrl(
        bucketName,
        filePath,
        data.publicUrl,
      );
      ...
      try {
        const cachedUrl = await this.localStorage.getCachedPublicUrl(
          bucketName,
          filePath,
        );
        if (cachedUrl) {
  ...
  getPublicUrlSync(bucketName: string, filePath: string): string {
    try {
      ...
      this.localStorage
        .cachePublicUrl(bucketName, filePath, data.publicUrl)
  ...
  async createUserDirectories(userId: string): Promise<boolean> {
    try {
      ...
      for (const bucketName of buckets) {
        ...
        if (
          structure &&
          structure.contents.some((item) => item.path === userDir)
        ) {
          this.debugger.logOperation(
        ...
        if (!success) {
          throw new Error(
            `Failed to create directory ${userDir} in ${bucketName}`,
          );
        }
