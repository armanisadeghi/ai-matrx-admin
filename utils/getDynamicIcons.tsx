...
const getRandomGenericIcon = (): LucideIcon => {
    const genericIcons: LucideIcon[] = [
        Activity, Album, Anchor, Armchair, Atom,
        Award, Apple, Circle, Compass, Crown,
        Diamond, Fingerprint, Aperture, Flower2, Globe,
        Heart, Hexagon, PaintBucket, Shield, Target, Table
    ];

    return genericIcons[Math.floor(Math.random() * genericIcons.length)];
};
...
export const getDynamicIconSelection = async (name: string): Promise<LucideIcon> => {
    ...
    return match?.icon || getRandomGenericIcon();
};
