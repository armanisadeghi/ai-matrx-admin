...
export const MatrxEditor = forwardRef<MatrxEditorRef, MatrxEditorProps>(({ onStateChange, editorId, onSelectionChange }, ref) => {
    const { registerEditor, unregisterEditor } = useBrokerSync();
    ...
    }, [editorId, registerEditor, unregisterEditor]);
    ...
    const convertSelectionToBrokerChip = useCallback(
        async ({ editorRef, broker, onProcessContent }: BrokerChipCreationOptions) => {
            ...
            try {
                // Ensure broker is tracked in sync system and update its value
                await handleTextToBroker(content, editorRef.current.dataset.editorId || '', broker.id);
                updateBrokerValue(broker.id, content);
    ...
    const formatSelection = useCallback(
        (style: Partial<TextStyle>) => {
            ...
            const { node: styledNode } = createTextNode(selectedContent, undefined, style);
