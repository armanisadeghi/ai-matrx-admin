...
export function createRecipeTaskBrokers(
  matchingBrokers: DataBrokerRecordWithKey[],
): BrokerValue[] {
  if (!matchingBrokers) {
    return [];
  }
...
export function createRuntimeBrokers({
  matchingBrokers,
  brokerValueRecordsWithBrokerKeys,
}: RuntimeBrokersProps): BrokerValue[] {
...
export function createRecipeOverrides(
  settingsList: Record<string, any>[],
): RecipeOverrides[] {
...
export function createRecipeTaskDataList(
  ...
  const recipeTaskBrokers = createRecipeTaskBrokers(
    brokers as DataBrokerRecordWithKey[],
  );
...
export function createRecipeRuntimePayload(
  ...
  const { id, brokers, settings } = compiledRecipe;
  const runtimeBrokers = createRuntimeBrokers({
    matchingBrokers: brokers,
    brokerValueRecordsWithBrokerKeys,
  });
  const recipeOverrides = createRecipeOverrides(settings);
...
export function createSingleRecipeRuntimeTaskData(
  ...
  const runtimeTaskDataList = createRecipeRuntimePayload(
    task,
    compiledRecipe,
    brokerValueRecordsWithBrokerKeys,
  );
  return runtimeTaskDataList.length > 0
    ? runtimeTaskDataList[0].taskData
    : undefined;
}
...
export function createEnhancedRecipePayload(
  ...
  const runtimeBrokers = brokers.map((broker) => {
    const valueBroker = allBrokerValueRecords.find(
      (record) => record.dataBroker === broker.id,
    );
    const brokerValue = valueBroker?.data?.value;
...
export function createBrokerValues(
  matchingBrokers: BrokersForBackend[],
): NewBrokerValue[] {
  if (!matchingBrokers) {
    return [];
  }
...
export function createNewRecipeOverrides(
  settingsList: Record<string, any>[],
): RecipeOverrides[] {
  return settingsList.map((settings) => {
    ...
    if (Array.isArray(settings.tools)) {
      ...
      settings.tools?.selectedToolIds &&
      Array.isArray(settings.tools.selectedToolIds)
    ) {
...
export function createRecipeToChatTaskDataList(
  compiledRecipe: CompiledRecipe,
): RecipeToChatTaskData[] {
  if (!compiledRecipe) {
    return [];
  }
