...
const row =
  "flex items-center justify-between px-4 py-3.5 border-b border-border/40 last:border-b-0";
const rowLabel = "text-sm font-medium";

const PromptsPreferences = () => {
  const dispatch = useAppDispatch();
  const prompts = useSelector(
    (state: RootState) => state.userPreferences.prompts,
  );
  const aiModels = useSelector(
    (state: RootState) => state.userPreferences.aiModels,
  );
  ...
  const handleSwitch =
    (preference: keyof typeof prompts) => (checked: boolean) =>
      dispatch(
        setPreference({ module: "prompts", preference, value: checked }),
      );
  ...
  return (
    <div>
      <div className={row}>
        <Label htmlFor="showSettingsOnMainPage" className={rowLabel}>
          Settings on Main Page
        </Label>
        <Switch
          id="showSettingsOnMainPage"
          checked={prompts.showSettingsOnMainPage}
          onCheckedChange={handleSwitch("showSettingsOnMainPage")}
        />
      </div>

      <div className={row}>
        <Label htmlFor="defaultModel" className={rowLabel}>
          Default Model
        </Label>
        {isLoadingModels ? (
          ...
          <Select
            ...
            onValueChange={handleSelect("defaultModel")}
          >
            <SelectTrigger id="defaultModel" className="w-40 h-8 text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className={row}>
        <Label htmlFor="includeThinkingInAutoPrompts" className={rowLabel}>
          Thinking Mode
        </Label>
        <Select
          value={prompts.includeThinkingInAutoPrompts}
          onValueChange={(v) =>
            handleSelect("includeThinkingInAutoPrompts")(v as ThinkingMode)
          }
        >
          <SelectTrigger
            id="includeThinkingInAutoPrompts"
      ...
      <div className={row}>
        <Label htmlFor="alwaysIncludeInternalWebSearch" className={rowLabel}>
          Always Web Search
        </Label>
        <Switch
          id="alwaysIncludeInternalWebSearch"
          checked={prompts.alwaysIncludeInternalWebSearch}
          onCheckedChange={handleSwitch("alwaysIncludeInternalWebSearch")}
        />
      </div>

      <div className={row}>
        <Label htmlFor="submitOnEnter" className={rowLabel}>
          Submit on Enter
        </Label>
        <Switch
          id="submitOnEnter"
          checked={prompts.submitOnEnter}
          onCheckedChange={handleSwitch("submitOnEnter")}
        />
      </div>

      <div className={row}>
        <Label htmlFor="autoClearResponsesInEditMode" className={rowLabel}>
          Auto Clear in Edit Mode
        </Label>
        <Switch
          id="autoClearResponsesInEditMode"
          checked={prompts.autoClearResponsesInEditMode}
          onCheckedChange={handleSwitch("autoClearResponsesInEditMode")}
        />
      </div>
    </div>
  );
