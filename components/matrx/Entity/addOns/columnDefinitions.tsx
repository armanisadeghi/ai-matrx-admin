...
import MatrixTableTooltip from "@/components/matrx/Entity/addOns/MatrixTableTooltip";
...
export const createTableColumnDefinitions = <TEntity extends EntityKeys>() => {
  ...
  const createStatusColumn = (options: {
    ...
    cell: ({ getValue }) => {
      ...
      return <Badge variant={state.color}>{state.label}</Badge>;
    },
    meta: {
  ...
  const createActionsColumn = (options: {
    ...
    cell: ({ row }) => (
      <div
        className={cn("flex items-center gap-2", options.containerClassName)}
      >
    ...
    cell: ({ getValue }) => {
      ...
      switch (options.format) {
        ...
        case "percent":
          ...
          break;
        case "compact":
          formatted = Intl.NumberFormat("en", { notation: "compact" }).format(
            value,
          );
          break;
        default:
  ...
  const createTagsColumn = (options: {
    ...
    cell: ({ getValue }) => {
      ...
      const displayTags = options.maxDisplay
        ? tags.slice(0, options.maxDisplay)
        : tags;
