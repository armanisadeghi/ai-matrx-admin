...
export const UnifiedQuickReference: React.FC<UnifiedQuickReferenceProps> = ({
  ...
  const handleRecordLabelChange =
    unifiedLayoutProps.handlers.handleRecordLabelChange;
  ...
  const QuickReferenceComponent = React.useMemo(() => {
    ...
    const quickRefComponentOptions = {
      ...
      LIST_WITH_RELATED: (
        <EntityQuickReferenceListWithRelated {...commonProps} />
      ),
    };

    return (
      quickRefComponentOptions[quickReferenceType] ||
      quickRefComponentOptions.list
    );
  }, [
    selectedEntity,
    quickReferenceType,
    density,
    animationPreset,
    unifiedLayoutProps.QuickReferenceComponent,
    className,
    onCreateEntityClick,
    handleRecordLoad,
    handleError,
