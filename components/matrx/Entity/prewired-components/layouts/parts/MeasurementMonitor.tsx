...
const MeasurementItem = memo(({ name, data, changeCounter }: { name: string; data: MeasurementData; changeCounter: number }) => (
    <div className="rounded px-2 py-1">
        <div className="font-semibold text-primary mb-1">{formatKey(name)}</div>
        <div className="pl-2 space-y-1">
            <div>
                <span className="text-muted-foreground">top: </span>
                <span className={getHighlightColor(changeCounter)}>{Math.round(data?.top)}px</span>
            </div>
            <div>
                <span className="text-muted-foreground">height: </span>
                <span className={getHighlightColor(changeCounter)}>{Math.round(data?.availableHeight)}px</span>
            </div>
        </div>
    </div>
));
...
const MeasurementMonitor = memo(({ measurements }: { measurements: Record<string, MeasurementData> }) => {
    ...
    useEffect(() => {
        ...
        measurementEntries.forEach(([key, data]) => {
            ...
            if (prev) {
                const hasChanged = prev.top !== data.top || prev.availableHeight !== data.availableHeight;
        ...
        <div className="fixed bottom-4 right-4 max-w-[300px] rounded-lg border bg-background/95 p-3 shadow-lg backdrop-blur supports-[backdrop-filter]:bg-background/60 z-50">
            <div className="text-xs font-mono space-y-2">
                {measurementEntries.map(([key, data]) => (
                    <MeasurementItem key={key} name={key} data={data} changeCounter={changeCounters[key] || 0} />
                ))}
            </div>
        </div>
    );
