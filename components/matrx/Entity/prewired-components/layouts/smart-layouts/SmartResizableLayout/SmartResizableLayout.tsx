...
import { DEFAULT_RESIZABLE_LAYOUT_OPTIONS } from "@/app/entities/layout/configs";

export const SmartResizableLayout: React.FC<UnifiedLayoutProps> = (
  unifiedLayoutProps,
) => {
  const selectedEntity = unifiedLayoutProps.layoutState?.selectedEntity || null;
  const options =
    unifiedLayoutProps.resizableLayoutOptions ||
      ...
      sections: [
        ...
        },
        {
          ...
          content: selectedEntity && (
            <QuickReferenceSection {...unifiedLayoutProps} />
          ),
