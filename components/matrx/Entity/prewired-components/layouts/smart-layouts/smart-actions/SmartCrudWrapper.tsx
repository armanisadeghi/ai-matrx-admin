...
import type { SmartCrudWrapperProps } from "../../types";
...
export const SmartCrudWrapper = ({
  ...
  const space = () => {
    const baseSpace =
      mergedLayout.buttonSpacing === "compact"
        ? 1
        : mergedLayout.buttonSpacing === "comfortable"
          ? 4
          : 2;
  ...
  const ButtonContainer = () => {
    ...
    return (
      <div className={cn(`p-${spacing.padding}`, "touch-none")}>
        <LoadingButtonGroup
          className={cn(
            mergedLayout.buttonLayout === "column" && "flex-col",
            "gap-2",
          )}
          gap={spacing.gap}
        >
  ...
  return (
    ...
    >
      {["top", "left"].includes(mergedLayout.buttonsPosition) && (
        <ButtonContainer />
      )}
      <div className="min-w-0 flex-1">{children}</div>
      {["bottom", "right"].includes(mergedLayout.buttonsPosition) && (
        <ButtonContainer />
      )}
    </div>
  );
