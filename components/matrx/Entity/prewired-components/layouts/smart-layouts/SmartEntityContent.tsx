...
export const SmartEntityContent: React.FC<UnifiedLayoutProps> = (
  unifiedLayoutProps,
) => {
  ...
  const crudHandlers: UnifiedCrudHandlers = useMemo(
    () => ({
      ...
      handleFetchOne: (
        ...
        entity.fetchOne(matrxRecordId, (result) => {
      ...
      handleFetchOneWithFkIfk: (
        ...
        entity.fetchOneWithFkIfk(matrxRecordId, (result) => {
          if (!result.success && options?.showToast) {
            toasts.handleError(result.error, "fetch", { showToast: true });
          }
        });
      ...
      handleUpdate: (options?: { showToast?: boolean }) => {
        ...
        entity.updateRecord(activeRecordId, (result) => {
          if (result.success && options?.showToast) {
      ...
      handleCreate: (
        ...
        entity.createRecord([createPayload], (result) => {
          if (result.success && options?.showToast) {
      ...
      handleDelete: (options?: { showToast?: boolean }) => {
        ...
        entity.deleteRecord(activeRecordId, (result) => {
          if (result.success && options?.showToast) {
