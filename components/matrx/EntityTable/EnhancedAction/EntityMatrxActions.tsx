...
export function useEntityOps<TEntity extends EntityKeys>(entityKey: TEntity) {
  ...
  const handleAction = async (
    ...
    if (action.confirmationRequired) {
      ...
      const message =
        typeof action.confirmationRequired === "object"
  ...
  switch (action.name) {
    ...
    case "delete":
      ...
      if (recordId) {
        ...
        console.warn(
          "Delete action should use deleteRecord thunk, not deleteRequest",
        );
      }
      break;
    default:
...
export const EntityOpsButton = <TEntity extends EntityKeys>({
  ...
  const isVisible =
    action.isVisible?.(entityData as EntityData<TEntity>) ?? true;
  const isEnabled =
    action.isEnabled?.(entityData as EntityData<TEntity>) ?? true;
  ...
  const label: string =
    typeof action.label === "function"
      ? action.label(entityData as EntityData<TEntity>)
      : action.label;
...
export const EntityOpsButtonGroup = <TEntity extends EntityKeys>({
  ...
  const customActionsMap = Object.fromEntries(
    customActions.map((action) => [action.name, action]),
  );
  ...
  return (
    <div className={cn("flex items-center space-x-1", className)}>
      {actionNames.map((actionName) => {
        const action =
          customActionsMap[actionName] || standardActions[actionName];
