...
const ComponentWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => <div className="space-y-6">{children}</div>;

export const PinkBlueBrokerDisplay = ({
    prepareRecipeHook,
                ...
                <CardHeader className="pb-4">
                    <CollapsibleTrigger className="flex items-center justify-between w-full group hover:opacity-80 transition-opacity">
                        <div>
                            ...
                            <CardDescription className="text-pink-700 dark:text-pink-300 font-medium">{recipeDescription}</CardDescription>
                        </div>
                        {isOpen ? (
