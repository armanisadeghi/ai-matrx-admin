export type SectionItem = {
  ...
  export function BatchparseMarkdownContent(markdown: string): ParsedContent[] {
    const lines = markdown.split("\n").map(line => line.trim());
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isHeader(line)) {
        ...
        if (currentSection) {
          sections.push({ intro: "", sections: [currentSection], outro: "" }); // Push completed section
        }
        currentSection = { title: line.replace("###", "").trim(), items: [] };
        continue;
      }
    ...
    if (currentSection) sections.push({ intro: "", sections: [currentSection], outro: "" });
