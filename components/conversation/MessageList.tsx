...
import { selectMessages, selectIsStreaming, selectUIState } from '@/lib/redux/chatConversations/selectors';
import { chatConversationsActions } from '@/lib/redux/chatConversations/slice';
import { MessageErrorBoundary } from '@/components/conversation/MessageErrorBoundary';
...
function StreamingAssistantMessage({ sessionId, messageId, compact, audioControls }: StreamingAssistantMessageProps) {
...
export function MessageList({
    ...
    useEffect(() => {
        if (isStreaming && latestAssistantRef.current) {
            latestAssistantRef.current.scrollIntoView({ behavior: 'smooth', block: 'end' });
        }
    }, [isStreaming]);
    ...
    const lastStreamingId = isStreaming
        ? [...messages].reverse().find(m => m.role === 'assistant' && m.status === 'streaming')?.id
        : null;
    ...
    return (
        <div className="flex flex-col gap-4 px-3 pt-16 pb-4">
            {visibleMessages.map((message, index) => {
                ...
                const contentClass = isLast && message.role === 'assistant' ? 'min-h-[50dvh]' : '';
                        ...
                        <MessageErrorBoundary messageId={message.id}>
                            {message.role === 'user' ? (
                                <div className={contentClass}>
                                    <UserMessage
                                        message={message}
                                        onContentChange={onMessageContentChange ? handleContentChange : undefined}
                                        compact={compact}
                                    />
                                </div>
