...
import {
    ThumbsUp, ThumbsDown, Copy, Check, Edit, MoreHorizontal,
    Volume2, Pause, Download, Link as LinkIcon, Loader2,
} from 'lucide-react';
...
export function AssistantMessage({
    ...
    const hasStreamEvents = message.streamEvents && message.streamEvents.length > 0
        && !hasBlockModeEvents;
    // DB-loaded tool updates (no active stream)
    const hasDbToolUpdates = !message.streamEvents && message.toolUpdates && (message.toolUpdates as unknown[]).length > 0;

    const isError = message.status === 'error';
    const isAudioResponse = !!audioUrl;

    // Loading: pending or streaming with no content yet
    const showLoading = message.status === 'pending' ||
        (message.status === 'streaming' && !message.content && !message.streamEvents?.length);
                ...
                {!showLoading && !isError && isAudioResponse && (
                    <div className="rounded-lg border bg-card p-3 space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                            <Button variant="ghost" size="sm" onClick={handleDownloadAudio} disabled={isDownloading}
                            ...
                            }} className="h-6 gap-1 px-2 text-xs text-muted-foreground hover:text-foreground" title="Copy audio link">
