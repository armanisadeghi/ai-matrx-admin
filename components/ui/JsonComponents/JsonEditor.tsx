...
export const EditableJsonViewer: React.FC<EditableJsonViewerProps> = ({
  ...
  const handleAddFirstItem = () => {
    if (readOnly) return;
    const newData = typeof sampleEntry === 'string' ? JSON.parse(sampleEntry) : sampleEntry;
  ...
  return (
    <div
      className={cn(
        "relative dark:bg-zinc-800 text-muted-foreground px-1 mb-2 rounded-md overflow-hidden",
        className
      )}
      style={{ maxHeight }}
      {...props}
    >
      <div className="flex justify-end space-x-2 mb-1">
        ...
        <IconButton
          ...
          tooltip={
            isEnhancedMode ? "Switch to Basic Mode" : "Switch to Enhanced Mode"
          }
          onClick={toggleMode}
        />
        <IconButton
...
export const FullEditableJsonViewer: React.FC<
  EditableJsonViewerProps & { title?: string }
> = ({
  data,
  onChange,
  onFormat,
...
export interface EnhancedEditableJsonViewerProps
...
export const EnhancedEditableJsonViewer: React.FC<
  EnhancedEditableJsonViewerProps
> = ({
  data,
  title,
  onSave,
