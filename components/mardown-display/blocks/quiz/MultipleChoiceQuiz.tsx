  ...
  return (
    <>
      ...
      <div className={`w-full ${isFullScreen ? 'fixed inset-0 z-50 flex items-center justify-center p-4' : 'py-6'}`}>
        <div className={`max-w-4xl mx-auto ${isFullScreen ? 'bg-white dark:bg-gray-900 rounded-2xl shadow-2xl h-full max-h-[95vh] w-full flex flex-col overflow-hidden' : ''}`}>
          {isFullScreen && (
            <div className="flex-shrink-0 p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Quiz Mode</h3>
              <button
                onClick={() => setIsFullScreen(false)}
                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium transition-all"
              >
          ...
          {/* Fullscreen Toggle Button - Visible when not in fullscreen */}
          {!isFullScreen && (
            <button
              onClick={() => setIsFullScreen(true)}
              className="mb-4 ml-auto flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500 dark:bg-blue-600 text-white text-sm font-medium shadow-md hover:bg-blue-600 dark:hover:bg-blue-700 transition-all"
            >
        ...
        <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 border-2 border-blue-200 dark:border-gray-700 rounded-2xl p-6 mb-6 shadow-lg">
          <div className="text-gray-800 dark:text-gray-100">
            ...
            {/* Question text - scrollable if needed in fullscreen */}
            <h2 className="text-lg font-bold mb-2 min-h-[72px] line-clamp-3">
        ...
        <div className={isFullScreen ? 'flex-shrink-0 p-6 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900' : ''}>
          <div className="flex justify-between items-center gap-4">
            ...
            {currentQuestionIndex === questions.length - 1 ? (
              <button
                onClick={handleNext}
                className={`rounded-lg font-semibold bg-green-500 dark:bg-green-600 hover:bg-green-600 dark:hover:bg-green-700 text-white shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200 flex-1 ${
                  isFullScreen ? 'px-6 py-3 text-base' : 'px-4 py-2 text-sm'
                }`}
              >
                View Results →
              ...
              <button
                onClick={handleNext}
                className={`rounded-lg font-semibold bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 text-white shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200 flex-1 ${
                  isFullScreen ? 'px-6 py-3 text-base' : 'px-4 py-2 text-sm'
                }`}
              >
                Next →
