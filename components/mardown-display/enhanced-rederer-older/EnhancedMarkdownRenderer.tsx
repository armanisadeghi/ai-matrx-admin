...
const EventComponent = dynamic(
  () => import("@/components/brokers/output/EventComponent"),
  { ssr: false },
);
    ...
    switch (activeMode) {
      ...
      case "sectionCards":
        try {
          ...
          return (
            <EventComponent sections={parsedContent.sections} tables={[]} />
          );
        } catch (error) {
          console.error("Failed to parse content for section cards:", error);
          return <div className="text-red-500">Failed to parse content</div>;
      ...
      case "structured":
        return (
          <JsonDisplay
            content={content}
            parseFunction={separatedMarkdownParser}
          />
        );
      ...
      case "parsedAsJson":
        return (
          <JsonDisplay content={content} parseFunction={parseMarkdownSimple} />
        );
            ...
            processors={[
              {
                ...
                label: "Markdown Content Parser",
                fn: parseMarkdownSimple,
              },
              {
                name: "separated-markdown",
                label: "Separated Markdown Parser",
                fn: separatedMarkdownParser,
              },
            ]}
          />
        );
