...
import {
  traverseBookmarkPath,
  getBookmarkDescription,
} from "./utils/bookmarkTraversal";
...
export const SocketBookmarkTab: React.FC<SocketBookmarkTabProps> = ({
  ...
  const responseData = useAppSelector((state) =>
    taskId ? selectPrimaryResponseDataByTaskId(taskId)(state) : EMPTY_ARRAY,
  );

  // Extract data using the bookmark path
  const constructBookmarkPath = (
    bookmark: string,
  ...
  const renderComponent = () => {
    ...
    switch (config.component) {
      ...
      case "BasicMarkdownContent":
        ...
        const content =
          typeof traversalResult.data === "string"
            ? traversalResult.data
            : JSON.stringify(traversalResult.data, null, 2);
