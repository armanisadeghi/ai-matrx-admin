...
import { SiGooglecalendar, SiGooglesheets, SiGoogledocs, SiGoogleslides, SiGoogletasks } from "react-icons/si";
...
export default function GoogleAccessCard({ service }: GoogleAccessCardProps) {
    ...
    const { name, scope, description, color = "#4285F4" } = googleServices[service];
            ...
            <div className="w-full pt-2">
                <button
                    ...
                    className={`w-full px-4 py-2 rounded-lg text-white font-medium transition-colors duration-300 flex items-center justify-center space-x-2 ${
                        isAuthorized ? "cursor-not-allowed" : isLoading || isInitializing ? "cursor-wait opacity-90" : "hover:brightness-90"
                    }`}
                >
                    {isLoading ? (
