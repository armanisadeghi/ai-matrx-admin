...
interface DOMNodeCreationResult {
  node: HTMLElement;
  id: string;
}
...
export const insertNodesWithRollback = ({
  nodes,
  target,
  position,
  rollbackNodes
}: InsertNodesOptions): boolean => {
    ...
    try {
      ...
      if (position === 'replaceWith') {
        originalNodes.forEach(node => 
          target.parentNode?.insertBefore(node.cloneNode(true) as HTMLElement, target)
        );
      }
    } catch (rollbackError) {
