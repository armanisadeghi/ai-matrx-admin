...
function buildToolCallObjects(
  ...
  if (completedResult !== undefined) {
    objects.push({
      id: callId,
      type: "mcp_output",
      mcp_output: { result: completedResult },
    });
...
function LivePreviewRenderer({
  ...
  type VisualizationType = React.ComponentType<{
    toolUpdates: ToolCallObject[];
  }>;
  const [Visualization, setVisualization] = useState<VisualizationType | null>(
    null,
  );
  const [loadError, setLoadError] = useState(false);

  const allObjects = buildToolCallObjects(
    toolName,
    rawStreamEvents,
    finalPayload,
  );
  ...
  const [visibleCount, setVisibleCount] = useState(() =>
    simulateStream ? 1 : allObjects.length,
  );
