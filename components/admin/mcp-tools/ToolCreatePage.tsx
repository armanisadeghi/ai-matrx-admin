...
export function ToolCreatePage() {
        ...
        <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                    ...
                    <Input value={tool.name} onChange={e => setField("name", e.target.value)} placeholder="e.g., core_web_search" className="font-mono" style={{ fontSize: "16px" }} />
                </div>
                <div className="space-y-1.5">
                    <Label>Category</Label>
                    <Input value={tool.category} onChange={e => setField("category", e.target.value)} placeholder="e.g., core, web, data" style={{ fontSize: "16px" }} />
                </div>
            </div>
            <div className="space-y-1.5">
                <Label>Description <span className="text-destructive">*</span></Label>
                <Textarea value={tool.description} onChange={e => setField("description", e.target.value)} rows={4} style={{ fontSize: "16px" }} className="resize-none" />
            </div>
            <div className="space-y-1.5">
                <Label>Function Path <span className="text-destructive">*</span></Label>
                <Input value={tool.function_path} onChange={e => setField("function_path", e.target.value)} className="font-mono" style={{ fontSize: "16px" }} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5">
                    <Label>Icon</Label>
                    <IconInputWithValidation value={tool.icon} onChange={v => setField("icon", v)} placeholder="e.g., Search" />
                </div>
                <div className="space-y-1.5">
                    <Label>Version</Label>
                    <Input value={tool.version} onChange={e => setField("version", e.target.value)} placeholder="1.0.0" style={{ fontSize: "16px" }} />
                </div>
                <div className="flex items-center gap-3 pt-6">
                    <Switch checked={tool.is_active} onCheckedChange={v => setField("is_active", v)} />
                    <Label>Active</Label>
                </div>
            </div>
            <div className="space-y-1.5">
                <Label>Tags (comma-separated)</Label>
                <Input value={tool.tags.join(", ")} onChange={e => setField("tags", e.target.value.split(",").map(t => t.trim()).filter(Boolean))} style={{ fontSize: "16px" }} />
            </div>
        </div>
    );
    ...
    const parametersField = (
        <div className="space-y-2 flex flex-col flex-1">
            <div className="flex items-center justify-between">
                ...
                {jsonErrors.parameters && <span className="text-xs text-destructive">JSON Error: {jsonErrors.parameters}</span>}
            </div>
            <Textarea
    ...
    const outputSchemaField = (
        <div className="space-y-2 flex flex-col flex-1">
            <div className="flex items-center justify-between">
                ...
                {jsonErrors.output_schema && <span className="text-xs text-destructive">JSON Error: {jsonErrors.output_schema}</span>}
            </div>
            <Textarea
    ...
    const saveBar = (
        <div className="flex-shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-border bg-background">
            <Button variant="outline" onClick={() => navigateTo("/administration/mcp-tools")} disabled={isSaving || isPending}>Cancel</Button>
    ...
    return (
        <div className="h-[calc(100dvh-var(--header-height))] flex flex-col overflow-hidden">
            <div className="flex-shrink-0 flex items-center gap-3 px-6 py-3 border-b border-border">
                <Button variant="ghost" size="sm" onClick={() => navigateTo("/administration/mcp-tools")} disabled={isPending} className="gap-1.5 h-8">
                            ...
                            <div className="flex-shrink-0 px-6 pt-2 border-b border-border">
                                <TabsList>
                                    ...
                                    <TabsTrigger value="parameters">
                                        Parameters{jsonErrors.parameters && <span className="ml-1 text-destructive">⚠</span>}
                                    </TabsTrigger>
                                    <TabsTrigger value="output">
                                        Output Schema{jsonErrors.output_schema && <span className="ml-1 text-destructive">⚠</span>}
                                    </TabsTrigger>
                                </TabsList>
                            </div>
                            <div className="flex-1 overflow-y-auto">
                                <TabsContent value="basic" className="p-6 m-0">{basicFields}</TabsContent>
                                <TabsContent value="parameters" className="p-6 m-0 flex flex-col h-full">{parametersField}</TabsContent>
                                <TabsContent value="output" className="p-6 m-0 flex flex-col h-full">{outputSchemaField}</TabsContent>
                            </div>
                        </Tabs>
                        {saveBar}
                    </>
