    ...
    return (
        <div className="relative rounded-lg border border-border bg-muted/20 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-muted/40">
                <span className="text-xs font-medium font-mono text-muted-foreground">{label}</span>
                <Button variant="ghost" size="sm" onClick={handleCopy} className="h-6 px-2 gap-1 text-[11px]">
    ...
    return (
        <div className="space-y-6 p-1">
            ...
            <div className="space-y-1.5">
                ...
                <p className="text-sm leading-relaxed">{tool.description || <span className="text-muted-foreground italic">No description</span>}</p>
            </div>

            {/* Key metadata grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-3">
                    <InfoRow icon={<Code className="h-3.5 w-3.5" />} label="Function Path">
                        <code className="text-xs font-mono bg-muted px-1.5 py-0.5 rounded">{tool.function_path}</code>
                    </InfoRow>
                    <InfoRow icon={<Tag className="h-3.5 w-3.5" />} label="Category">
                        {tool.category ? <Badge variant="outline" className="text-xs">{formatText(tool.category)}</Badge> : <span className="text-muted-foreground text-xs">None</span>}
                    </InfoRow>
                    <InfoRow icon={<Hash className="h-3.5 w-3.5" />} label="Version">
                        <span className="text-xs font-mono">{tool.version || "—"}</span>
                    </InfoRow>
                    <InfoRow icon={<Info className="h-3.5 w-3.5" />} label="Icon">
                        <div className="flex items-center gap-2">
                            <span className="text-foreground">{icon}</span>
                            {tool.icon && <code className="text-xs font-mono text-muted-foreground">{tool.icon}</code>}
                            {!tool.icon && <span className="text-xs text-muted-foreground">Auto (from category)</span>}
                        </div>
                    </InfoRow>
                </div>
                <div className="space-y-3">
                    <InfoRow icon={<FileCode className="h-3.5 w-3.5" />} label="ID">
                        <code className="text-xs font-mono text-muted-foreground break-all">{tool.id}</code>
                    </InfoRow>
                    <InfoRow icon={<Calendar className="h-3.5 w-3.5" />} label="Created">
                        <span className="text-xs">{tool.created_at ? new Date(tool.created_at).toLocaleString() : "—"}</span>
                    </InfoRow>
                    <InfoRow icon={<Calendar className="h-3.5 w-3.5" />} label="Updated">
                        <span className="text-xs">{tool.updated_at ? new Date(tool.updated_at).toLocaleString() : "—"}</span>
                    </InfoRow>
                </div>
            </div>
    ...
    return (
        <div className="space-y-2">
            {annotations.map((ann, i) => (
                <div key={i} className="rounded-lg border border-border bg-muted/20 p-3 text-xs font-mono">
                    <pre className="whitespace-pre-wrap overflow-auto">{JSON.stringify(ann, null, 2)}</pre>
                </div>
            ))}
        </div>
    );
                ...
                <div className="flex items-center gap-3 px-6 py-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 ml-auto flex-wrap">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigateTo(`/administration/mcp-tools/${tool.id}/incidents`)}
                            disabled={isPending}
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigateTo(`/administration/mcp-tools/${tool.id}/ui`)}
                            disabled={isPending}
                            className="h-8 gap-1.5 text-xs"
                        >
                        ...
                        <Button
                            size="sm"
                            onClick={() => navigateTo(`/administration/mcp-tools/${tool.id}/edit`)}
                            disabled={isPending}
                            className="h-8 gap-1.5 text-xs"
                        >
                ...
                <Tabs defaultValue="overview" className="flex-1 flex flex-col overflow-hidden">
                    <div className="flex-shrink-0 px-6 pt-2 border-b border-border">
                        <TabsList className="h-9">
                            ...
                            <TabsTrigger value="output-schema" className="text-xs gap-1.5">
                                ...
                                Output Schema
                                {!hasOutputSchema && <span className="text-[10px] text-muted-foreground">(none)</span>}
                            </TabsTrigger>
                            <TabsTrigger value="annotations" className="text-xs gap-1.5">
                                <FileCode className="h-3.5 w-3.5" />
                                Annotations
                                {hasAnnotations && <Badge variant="secondary" className="text-[10px] h-4 px-1 ml-0.5">{(tool.annotations as unknown[]).length}</Badge>}
                            </TabsTrigger>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        <TabsContent value="parameters" className="p-6 m-0 h-full">
                            <JsonDisplay data={tool.parameters} label="parameters (JSON Schema)" />
                        </TabsContent>

                        <TabsContent value="output-schema" className="p-6 m-0 h-full">
                            <JsonDisplay data={tool.output_schema} label="output_schema (JSON Schema)" />
                        </TabsContent>
                        ...
                        <TabsContent value="samples" className="m-0 h-full overflow-hidden flex flex-col">
