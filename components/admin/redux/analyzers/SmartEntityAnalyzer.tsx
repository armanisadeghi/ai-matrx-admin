...
import { MatrxJsonToCollapsible } from "@/components/matrx/matrx-collapsible/json-to-collapsible";
...
const SmartSectionView: React.FC<{
  ...
  if (
    JSON_ONLY_SECTIONS.includes(id) ||
...
const EntitySections: React.FC<{
  ...
  const collapsibleSections = sections.filter((s) =>
    COLLAPSIBLE_SECTIONS.includes(s.id),
  );
  const jsonSections = sections.filter((s) =>
    JSON_ONLY_SECTIONS.includes(s.id),
  );
          ...
          <div className="flex gap-2 flex-wrap">
            {entityOptions.map((entity) => (
              <Button
                key={entity.value}
                variant={
                  currentEntityKey === entity.value ? "default" : "outline"
                }
