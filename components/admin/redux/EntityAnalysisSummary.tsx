...
const EntityAnalyzerView = ({ entityKey }: { entityKey: EntityKeys }) => {
  ...
  const fields = React.useMemo(
    () =>
      Object.values(state.entityMetadata.entityFields).map((field: any) => ({
        name: field.name,
        displayName: field.displayName,
      })),
    [state.entityMetadata.entityFields],
  );

  const activeRecord = React.useMemo(
    () =>
      state.selection.activeRecord &&
      state.records[state.selection.activeRecord]
        ? {
            [state.selection.activeRecord]:
              state.records[state.selection.activeRecord],
          }
  ...
  const unsavedRecord = React.useMemo(
    () =>
      state.selection.activeRecord &&
      state.unsavedRecords[state.selection.activeRecord]
        ? {
            [state.selection.activeRecord]:
              state.unsavedRecords[state.selection.activeRecord],
          }
  ...
  const selectedRecords = React.useMemo(
    () =>
      state.selection.selectedRecords.reduce(
        (acc, id) => {
          if (state.records[id]) {
