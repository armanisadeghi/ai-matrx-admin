  ...
  return useMemo(
    () => ({
      entityStats: {
        ...
        initialized: Object.values(entities).filter(
          (e: any) => e.loading?.initialized,
        ).length,
        withData: Object.values(entities).filter(
          (e: any) => Object.keys((e as any).records || {}).length > 0,
        ).length,
      },
      schemaStats: {
          ...
          {Object.entries(value).map(([key, val]) => (
            <StateNode
              ...
              isLarge={
                typeof val === "object" &&
                val !== null &&
                JSON.stringify(val).length > 10000
              }
            />
          ))}
        </div>
...
export default function ReduxDebugInterface({
  ...
  const [activeTab, setActiveTab] = useState(
    initialSelectedEntity || "overview",
  );
  ...
  const slices = useMemo(() => {
    const availableSlices = Object.keys(globalState).filter(
      (key) => typeof globalState[key] === "object" && key !== "form",
    );
    ...
    if (tabOrder.length === 1) {
      setTabOrder([
        ...
        "entities",
        "globalCache",
        ...availableSlices.filter(
          (s) => !["entities", "globalCache"].includes(s),
        ),
      ]);
    }
        ...
        <div className="h-full flex flex-col">
          ...
          >
            ...
            <ScrollArea className="flex-1">
              ...
              {slices.map((slice) => (
                <TabsContent key={slice} value={slice} className="p-2">
                  <StateNode
                    path={[slice]}
