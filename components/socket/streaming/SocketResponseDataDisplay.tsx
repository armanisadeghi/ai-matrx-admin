...
import { selectResponseDataByListenerId } from "@/lib/redux/socket-io/selectors/socket-response-selectors";
...
export const FullscreenResponseDataDisplay = ({ title = "Data Response", listenerId }: { title?: string; listenerId: string }) => {
    const data = useAppSelector(selectResponseDataByListenerId(listenerId));
...
export const CompactResponseDataDisplay = ({ title = "Data Response", listenerId }: { title?: string; listenerId: string }) => {
    const data = useAppSelector(selectResponseDataByListenerId(listenerId));
    const jsonString = safeStringify(data);
    const hasContent = Array.isArray(data) ? data.length > 0 : data && typeof data === "object" && Object.keys(data).length > 0;
...
const ResponseDataDisplay = ({ title = "Data Response", listenerId, isFullscreen = false }: StreamDataDisplayProps) => {
    return isFullscreen ? <FullscreenResponseDataDisplay title={title} listenerId={listenerId} /> : <CompactResponseDataDisplay title={title} listenerId={listenerId} />;
};
