...
export const EditorProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const registerEditor = useCallback(
    (editorId: string, initialContent = "", initialLayout?: LayoutMetadata) => {
  ...
  const getEditorState = useCallback(
    (editorId: string) => {
      if (!registeredEditors.current.has(editorId)) {
        console.warn(
          `Editor state not found for unregistered editor: ${editorId}`,
        );
        return initialState;
      }
      ...
      if (!state) {
        throw new Error(
          `Editor state not found for registered editor: ${editorId}`,
        );
      }
      return state;
    },
  ...
  const isEditorRegistered = useCallback(
    (editorId: string) => registeredEditors.current.has(editorId),
    [],
  );
  ...
  const isEditorInitialized = useCallback(
    (editorId: string) => getEditorState(editorId).initialized,
    [getEditorState],
  );

  const updateEditorState = useCallback(
    (editorId: string, updates: Partial<EditorState>) => {
      if (!registeredEditors.current.has(editorId)) return;
  ...
  const extractContentMetadata = useCallback(
    (content: string): BrokerMetaData[] => {
  ...
  const getNextColor = useCallback(() => {
    const availableColors = TAILWIND_COLORS.filter(
      (color) => !usedColors.has(color),
    );
    const randomIndex = Math.floor(
      Math.random() * (availableColors.length || TAILWIND_COLORS.length),
    );
    const selectedColor =
      availableColors.length > 0
        ? availableColors[randomIndex]
        : TAILWIND_COLORS[randomIndex];
  ...
  const getContent = useCallback(
    (editorId: string) => getEditorState(editorId).content,
    [getEditorState],
  );

  const getBrokerMetadata = useCallback(
    (editorId: string) => getEditorState(editorId).metadata,
    [getEditorState],
  );
  const getChipData = useCallback(
    (editorId: string) => getEditorState(editorId).chipData,
    [getEditorState],
  );
  ...
  const getAllBrokers = useCallback((): BrokerMetaData[] => {
    ...
    const uniqueMetadataMap = new Map(
      flatMetadata.map((metadata) => [metadata.recordKey, metadata]),
    );
  ...
  const getAllChips = useCallback((): ChipData[] => {
    ...
    const uniqueChipsMap = new Map(
      flatChipsData.map((chips) => [chips.id, chips]),
    );
  ...
  const getContentMode = useCallback(
    (editorId: string): ContentMode => getEditorState(editorId).contentMode,
    [getEditorState],
  );
