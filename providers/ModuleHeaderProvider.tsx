...
const ModuleHeaderContext = createContext<ModuleHeaderContextType | undefined>(
  undefined,
);

export function ModuleHeaderProvider({
  children,
}: {
  ...
  const addHeaderItem = useCallback((item: HeaderItem) => {
    setHeaderItems((prev) => {
      ...
      return [...filtered, { ...item, section: item.section || "left" }].sort(
  ...
  return (
    <ModuleHeaderContext.Provider
      value={{ addHeaderItem, removeHeaderItem, headerItems }}
    >
      {children}
    </ModuleHeaderContext.Provider>
  );
...
export function useModuleHeader() {
  ...
  if (!context) {
    throw new Error(
      "useModuleHeader must be used within a ModuleHeaderProvider",
    );
  }
  return context;
}
