...
import ModuleHeader from "@/components/matrx/navigation/ModuleHeader";
...
export const CollapsibleHeader: React.FC<{
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-end h-6 px-2 bg-muted/50",
        className,
      )}
    >
      <button
...
const DefaultPanel: React.FC<{
  ...
  <div className="h-full w-full flex flex-col">
    {collapsible && (
      <CollapsibleHeader onCollapse={onCollapse} isCollapsed={isCollapsed} />
    )}
    <div className="flex-1 bg-background p-4 overflow-auto">{children}</div>
  </div>
);
...
const RenderSection: React.FC<{
  ...
  return (
    <ResizablePanel defaultSize={actualSize} collapsible={section.collapsible}>
...
export function ModuleLayout({
  ...
  const effectiveSections = React.useMemo(() => {
    ...
    const childrenSection = sections.find(
      (section) => section.type === "content" && section.content === children,
    );
    ...
    return [
      ...sections,
      {
        ...
        defaultSize:
          100 -
          sections.reduce(
            (acc, section) => acc + (section.defaultSize || 0),
            0,
          ),
      },
    ];
  ...
  const defaultSizes = effectiveSections.map(
    (section) => section.defaultSize || 100 / effectiveSections.length,
  );
          ...
          {effectiveSections.map((section, index) => (
            <React.Fragment key={index}>
              <RenderSection
                ...
                onCollapse={() => {
                  const isCollapsed =
                    options.leftSections?.[index]?.collapsible;
                  updateOptions({
