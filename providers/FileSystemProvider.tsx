...
export const FileSystemProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [structures, setStructures] = useState<
    Map<string, BucketStructureWithNodes>
  >(new Map());
  ...
  const syncIntervalRef = React.useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  const initialize = React.useCallback(async () => {
    ...
    try {
      ...
      const processedStructures =
        fileNodeManager.processAllBucketStructures(rawStructures);
  ...
  const getBucketStructure = (
    bucketName: string,
  ): BucketStructureWithNodes | undefined => {
    ...
    if (rawStructure) {
      const processedStructure =
        fileNodeManager.processBucketStructure(rawStructure);
      setStructures((prev) =>
        new Map(prev).set(bucketName, processedStructure),
      );
  ...
  const refreshBucketStructure = async (bucketName: string) => {
    return withLoadingAndToast(
      async () => {
        if (!isInitialized) return undefined;
        const structure =
          await fileSystemManager.loadBucketStructure(bucketName);
        if (!structure || !structure.contents) return undefined;
        const processedStructure =
          fileNodeManager.processBucketStructure(structure);
  ...
  const refreshFolderContents = async (
    bucketName: string,
    folderPath: string,
  ) => {
    return withLoadingAndToast(
      async () => {
        ...
        const success = await fileSystemManager.refreshFolderContents(
          bucketName,
          folderPath,
        );
        ...
        const folderContents = {
          path: folderPath,
          contents: currentStructure.contents.filter(
            (item) =>
        ...
        const processedContents = fileNodeManager.processFolderContents(
          folderContents,
          bucketName,
        );
  ...
  const filterFilesByCategory = (
    files: BucketStructure[],
    category: string,
  ): BucketStructure[] => {
    return files.filter((file) => file.details?.category === category);
  };

  const filterFilesBySubCategory = (
    files: BucketStructure[],
    subCategory: string,
  ): BucketStructure[] => {
    return files.filter((file) => file.details?.subCategory === subCategory);
  };

  const getFilesByCategory = (
    bucketName: string,
  ): Record<string, BucketStructure[]> => {
    const structure = getBucketStructure(bucketName);
    if (!structure) return {};

    return structure.contents.reduce(
      (acc, file) => {
        if (file.type !== "FOLDER") {
  ...
  const uploadFile = async (bucketName: string, path: string, file: File) => {
    const result = await withLoadingAndToast(
      async () => {
        const uploadResult = await fileSystemManager.uploadFile(
          bucketName,
          path,
          file,
        );
  ...
  const getCurrentPublicUrl = async (): Promise<string | null> => {
    try {
      return await fileSystemManager.getPublicUrl(
        currentBucket!,
        currentPath.join("/"),
      );
    } catch (error) {
  ...
  const getBuckets = async () => fileSystemManager.getBuckets();
  ...
  const loadBucketStructure = async (
    bucketName: string,
    ...
    return withLoadingAndToast(
      async () => {
        ...
        if (status.conflicts > 0) {
          toast.warning(
            `Found ${status.conflicts} conflicts that need resolution`,
          );
        }
        return true;
      },
  ...
  return (
    <FileSystemContext.Provider value={value}>
      <FileSystemDialogProvider>{children}</FileSystemDialogProvider>
