...
function HealthIndicator({ status, detail }: { status: HealthStatus; detail: string | null }) {
  ...
  return (
    <Tooltip>
      ...
      {detail && <TooltipContent className="text-xs max-w-xs">{detail}</TooltipContent>}
    </Tooltip>
  );
}
...
function AuthTokenWidget({ config }: { config: UseServerConfigReturn }) {
  ...
  const save = () => {
    ...
    setEditing(false);
    toast.success('Token saved', { description: 'Stored in localStorage for all matrx-ai test pages.' });
  };
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              size="sm"
              variant="ghost"
              onClick={() => { setTemp(config.authToken); setEditing(true); }}
  ...
  return (
    <div className="flex items-center gap-1 flex-1 min-w-[160px] max-w-[360px]">
      <Input
        ...
        onKeyDown={e => {
          if (e.key === 'Enter') save();
          if (e.key === 'Escape' && config.authToken) { setTemp(config.authToken); setEditing(false); }
        }}
      />
      <Tooltip>
        <TooltipTrigger asChild>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              size="sm"
              variant="ghost"
              onClick={() => { setTemp(config.authToken); setEditing(false); }}
              className="h-7 w-7 p-0 flex-shrink-0"
            >
              <X className="h-3 w-3" />
            </Button>
 ...
 *  • Health check button → GET {serverUrl}/api/health
 *  • Auth token widget (stored in localStorage, shared across pages)
 *  • Optional title slot (left) and actions slot (right)
 */
export function ServerBar({ config, title, actions, showAuth = true }: ServerBarProps) {
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 py-1.5 border-b">
          ...
          <Select value={selectValue} onValueChange={handleSelectChange}>
            ...
            <SelectContent>
              {SERVER_PRESETS.map(p => (
                <SelectItem key={p.url} value={p.url} className="text-xs font-mono">
                  {p.label}
                </SelectItem>
              ))}
              <SelectItem value={CUSTOM_URL_VALUE} className="text-xs italic text-muted-foreground">
                Custom URL…
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        <Tooltip>
          ...
          <TooltipContent className="text-xs">
            GET {config.serverUrl}/api/health
          </TooltipContent>
        </Tooltip>

        <HealthIndicator status={config.healthStatus} detail={config.healthDetail} />
        ...
        {/* Right-side actions */}
        {actions && (
          <div className="ml-auto flex items-center gap-1.5 flex-shrink-0">{actions}</div>
        )}
      </div>
    </TooltipProvider>
  );
