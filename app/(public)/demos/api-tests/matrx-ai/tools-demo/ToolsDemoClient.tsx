    ...
    return (
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Label htmlFor={`param-${param.name}`} className="text-xs cursor-pointer text-muted-foreground">
            {param.description}
          </Label>
        </div>
      </div>
...
export default function ToolsDemoClient() {
  ...
  const handleExecute = async () => {
    ...
    try {
      const res = await fetch(`${config.serverUrl}/api/tools/test/execute`, {
        method: 'POST',
            ...
            <Card className="col-span-6 h-full flex flex-col overflow-hidden p-3">
              <div className="flex items-center justify-between mb-2 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {execStatus !== 'idle' && (
                    <Badge
                      variant={execStatus === 'complete' ? 'secondary' : execStatus === 'error' ? 'destructive' : 'default'}
                      className="text-[10px] h-5 px-1.5"
                    >
                      {execStatus === 'running' ? 'Running' : execStatus === 'complete' ? 'Complete' : 'Error'}
                    </Badge>
                  )}
                  {elapsedMs !== null && (
                    <span className="text-[10px] text-muted-foreground font-mono">{elapsedMs.toFixed(1)}ms</span>
                  )}
                </div>
                {callId && (
                  <Tooltip>
                    ...
                    <TooltipContent className="font-mono text-xs">{callId}</TooltipContent>
                  </Tooltip>
                )}
              </div>
              ...
              <Tabs defaultValue="result" className="flex-1 flex flex-col overflow-hidden min-h-0">
                <TabsList className="grid grid-cols-2 h-8 flex-shrink-0">
                  <TabsTrigger value="result" className="text-xs">Result JSON</TabsTrigger>
                  <TabsTrigger value="request" className="text-xs">Request</TabsTrigger>
                </TabsList>

                <TabsContent value="result" className="flex-1 flex flex-col overflow-hidden mt-2 p-3 bg-muted rounded border min-h-0">
                  <div className="flex justify-end flex-shrink-0 mb-1">
                    <CopyButton text={result ? JSON.stringify(result, null, 2) : ''} />
                  </div>
                ...
                <TabsContent value="request" className="flex-1 flex flex-col overflow-hidden mt-2 p-3 bg-muted rounded border min-h-0">
                  <div className="flex justify-end flex-shrink-0 mb-1">
                    <CopyButton text={`POST ${config.serverUrl}/api/tools/test/execute\n\n${requestBodyStr}`} />
                  </div>
                  <div className="flex-1 overflow-y-auto min-h-0">
