...
function KVEditor({ label, pairs, onChange }: {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {pairs.length === 0 && (
        <p className="text-[10px] text-muted-foreground italic pl-0.5">No entries — click Add</p>
      )}
      {pairs.map((pair, i) => (
        <div key={i} className="flex gap-1 items-center">
          ...
          <Input value={pair.value} onChange={e => update(i, 'value', e.target.value)} placeholder="value" className="h-6 text-xs font-mono flex-1 min-w-0" />
...
export default function AgentDemoClient() {
  ...
  const clearResults = () => {
    ...
    setRequestId(null); setStats({ events: 0, bytes: 0, ms: 0, startTime: 0 }); setExecStatus('idle');
  };
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); if (canExecute) handleExecute(); }
  ...
  const buildConfigOverrides = (): Record<string, unknown> | undefined => {
    ...
    const temp = parseFloat(temperature); if (!isNaN(temp)) overrides.temperature = temp;
    const mtok = parseInt(maxOutputTokens, 10); if (!isNaN(mtok)) overrides.max_output_tokens = mtok;
    ...
    if (extraParsed.ok && extraParsed.obj && typeof extraParsed.obj === 'object') {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (controller.signal.aborted) {
        setExecStatus('cancelled'); toast.info('Cancelled');
      } else {
          ...
          <ServerBar
            ...
            title={
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <Badge variant="outline" className="text-[10px] h-5 px-1.5">POST /api/ai/agents/{'{id}'}</Badge>
              </div>
            }
            actions={
              <Tooltip>
                ...
                <TooltipContent className="text-xs">POST /api/ai/agents/{'{id}'}/warm — pre-load agent into cache</TooltipContent>
              </Tooltip>
            }
          />
        </div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Sent as: <code className="font-mono">POST /api/ai/agents/{agentId || '{agent_id}'}</code>
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1.5 p-2.5 bg-muted/30 rounded-md border">
                  {([
                    ['stream', 'Stream response', streamEnabled, setStreamEnabled],
                ...
                {/* Variables */}
                <KVEditor label="Variables" pairs={variables} onChange={setVariables} />
                ...
                <Collapsible open={overridesOpen} onOpenChange={setOverridesOpen}>
                  <CollapsibleTrigger className="flex items-center gap-2 w-full py-1 text-left hover:text-foreground/80">
                    ...
                    <span className="text-xs font-semibold flex-1">Config Overrides</span>
                    {overridesOpen ? <ChevronDown className="h-3 w-3 text-muted-foreground" /> : <ChevronRight className="h-3 w-3 text-muted-foreground" />}
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <div className="space-y-3 pt-2">
                      <p className="text-[10px] text-muted-foreground">
                        These override the agent's default model settings for this run.
                      </p>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <Input value={temperature} onChange={e => setTemperature(e.target.value)} placeholder="0.0–2.0" className="h-7 text-xs" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-[10px] text-muted-foreground">Max Output Tokens</Label>
                          <Input value={maxOutputTokens} onChange={e => setMaxOutputTokens(e.target.value)} placeholder="e.g. 4096" className="h-7 text-xs" />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select value={reasoningEffort || '__none__'} onValueChange={v => setReasoningEffort(v === '__none__' ? '' : v)}>
                          ...
                          <SelectContent>
                            <SelectItem value="__none__" className="text-xs">None (use agent default)</SelectItem>
                      ...
                      <div className="space-y-1">
                        <Label className="text-[10px] text-muted-foreground">Model Override (ai_model_id)</Label>
                        <Input value={modelOverride} onChange={e => setModelOverride(e.target.value)} placeholder="e.g. gpt-4o (overrides agent default)" className="h-7 text-xs font-mono" />
                      </div>

                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <Label className="text-[10px] text-muted-foreground">Additional overrides (JSON)</Label>
                          {!extraParsed.ok && (
                            <Badge variant="destructive" className="text-[9px] h-4 px-1">Invalid JSON</Badge>
                          )}
                        </div>
                        <Textarea
              ...
              <div className="flex-shrink-0 p-3 border-t flex flex-col gap-2">
                <div className="flex gap-2">
                  {!isRunning ? (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button onClick={handleExecute} disabled={!canExecute} className="flex-1 h-8 text-sm gap-2">
                    ...
                    <Button onClick={handleCancel} variant="destructive" className="flex-1 h-8 text-sm gap-2">
                  ...
                  {(tokenUsage.input_tokens !== null || tokenUsage.output_tokens !== null) ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <p className="text-sm text-muted-foreground">
                        {execStatus === 'complete' ? 'No token usage in stream events.' : 'Execute an agent to see token usage.'}
                      </p>
                    </div>
                  )}
                </TabsContent>
