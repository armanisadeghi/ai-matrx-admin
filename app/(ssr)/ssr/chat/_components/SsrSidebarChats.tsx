...
import type { CxConversationSummary, SharedCxConversationSummary } from '@/features/public-chat/types/cx-tables';
...
function ConversationItem({
    ...
    if (isDeleting) {
        return <DeleteConfirm label={item.title || 'Untitled Chat'} onConfirm={onConfirmDelete} onCancel={onCancelDelete} />;
    }

    if (isRenaming) {
        return <InlineRename value={renameValue} onChange={onRenameChange} onConfirm={onConfirmRename} onCancel={onCancelRename} />;
    }

    const handleSelect = (e: React.MouseEvent) => {
        if (e.metaKey || e.ctrlKey) {
            const agentId = new URLSearchParams(window.location.search).get('agent');
            const url = agentId ? `/ssr/chat/c/${item.id}?agent=${agentId}` : `/ssr/chat/c/${item.id}`;
                    ...
                    <div className="flex-shrink-0 pr-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-32" sideOffset={4}>
                                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setIsShareOpen(true); }} className="text-[11px] py-1.5">
                                    <Share2 className="h-3 w-3 mr-2" />Share
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onStartRename(); }} className="text-[11px] py-1.5">
...
function SharedChatsSection({
    activeRequestId, onSelectChat, onCloseSidebar, searchQuery,
}: {
    activeRequestId?: string | null;
    onSelectChat: (requestId: string) => void;
    onCloseSidebar?: () => void;
    ...
    const filtered = useMemo(() => {
        ...
        return sharedChats.filter(c =>
            c.title?.toLowerCase().includes(q) || c.owner_email?.toLowerCase().includes(q)
        );
                    ...
                    <div className="space-y-0.5">
                        {filtered.map((item) => (
                            <SharedConversationItem
                                ...
                                onSelect={() => { onSelectChat(item.id); onCloseSidebar?.(); }}
                            />
                        ))}
                    </div>
                </div>
...
export function SsrSidebarChats({
    activeRequestId, onSelectChat, onNewChat, searchQuery = '', onCloseSidebar,
                        ...
                        setHistory((prev) =>
                            prev.map((item) =>
                                item.id === payload.new.id
                                    ? { ...item, title: payload.new.title || item.title, status: payload.new.status || item.status, updated_at: payload.new.updated_at || item.updated_at }
                                    : item
                            )
                        );
    ...
    const handleConfirmRename = useCallback(async () => {
        ...
        if (success) {
            setHistory(prev => prev.map(h => h.id === renamingId ? { ...h, title: renameValue.trim() } : h));
        }
        setRenamingId(null);
        setRenameValue('');
