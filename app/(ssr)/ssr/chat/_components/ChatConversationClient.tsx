...
const SsrAgentPickerSheet = dynamic(
    () =>
        import('./SsrAgentPickerSheet').then(m => ({
            default: m.SsrAgentPickerSheet,
        })),
    { ssr: false },
);
...
export default function ChatConversationClient({
    ...
    useEffect(() => {
        if (startedAsNew.current && firstMessage && session.sessionId && !sentRef.current) {
    ...
    const derivedCapabilities = modelSettings
        ? {
              supportsImageUrls: (modelSettings as Record<string, unknown>).image_urls !== false,
              supportsFileUrls: (modelSettings as Record<string, unknown>).file_urls !== false,
              supportsYoutubeVideos: (modelSettings as Record<string, unknown>).youtube_videos !== false,
              supportsAudio: true,
          }
        : undefined;
