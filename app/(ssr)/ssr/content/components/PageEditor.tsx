...
interface PageEditorProps {
    ...
    isSaving: boolean;
    error: string | null;
    onSave: (pageId: string, updates: Record<string, unknown>) => Promise<ClientPage>;
    onSaveDraft: (pageId: string, draft: Record<string, unknown>) => Promise<ClientPage>;
...
type EditorTab = 'html' | 'css' | 'js' | 'preview' | 'seo' | 'settings' | 'versions';
...
export default function PageEditor({
    ...
    useEffect(() => {
        if (page) {
            ...
            setMetaDescription(page.meta_description_draft ?? page.meta_description ?? '');
    ...
    const handleRollback = async (versionNumber: number) => {
        if (!page) return;
        if (confirm(`Rollback to version ${versionNumber}? Current content will be replaced.`)) {
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex-none border-b border-border/50 bg-muted/20">
                <div className="flex items-center justify-between px-4 py-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                        ...
                        {isNew ? (
                            ...
                            <>
                                {page?.has_draft && (
                                    <Button variant="ghost" size="sm" onClick={handleDiscard} className="text-xs gap-1.5 text-muted-foreground">
                ...
                <div className="flex items-center gap-0.5 px-4 overflow-x-auto scrollbar-none">
                    {TABS.filter((t) => !isNew || !['versions'].includes(t.id)).map((tab) => {
                        const Icon = tab.icon;
            ...
            <div className="flex-1 overflow-auto">
                ...
                {activeTab === 'seo' && (
                    <div className="p-6 max-w-2xl mx-auto space-y-5">
                        ...
                        <div>
                            <label className="text-sm font-medium text-foreground block mb-1.5">Keywords</label>
                            <Input
                                value={metaKeywords}
                        ...
                        <div>
                            <label className="text-sm font-medium text-foreground block mb-1.5">OG Image URL</label>
                            <Input
                                value={ogImage}
                        ...
                        <div>
                            <label className="text-sm font-medium text-foreground block mb-1.5">Canonical URL</label>
                            <Input
                                value={canonicalUrl}
                        ...
                        <div className="rounded-lg border border-border p-4 bg-muted/20 space-y-1">
                            <p className="text-xs text-muted-foreground font-medium mb-2">Search Preview</p>
                            ...
                            <p className="text-sm text-muted-foreground line-clamp-2">
                                {metaDescription || 'No description set. Add a meta description to improve SEO.'}
                            </p>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'settings' && (
                    <div className="p-6 max-w-2xl mx-auto space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-sm font-medium text-foreground block mb-1.5">Page Type</label>
                                <select
                                    value={pageType}
                    ...
                    <div className="p-6 max-w-2xl mx-auto space-y-4">
                        ...
                        {versions.isLoading ? (
                            ...
                            <p className="text-sm text-muted-foreground text-center py-8">
                                No versions yet. Versions are created automatically when you publish.
                            </p>
                        ) : (
