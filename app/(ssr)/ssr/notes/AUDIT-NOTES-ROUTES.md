...
| **AI Actions fixed** | Root cause identified and fixed: `buildCategoryHierarchy` was called with `"note-editor"` context filter. All `ai-action` items in the DB have `enabled_contexts: ["general"]` only — not `"note-editor"` — so the filter eliminated every item. Fix: pass `undefined` as contextFilter (matching how authenticated notes' `UnifiedContextMenu` works). DB, RPC, and Redux hydration all confirmed working via Supabase MCP inspection. |
