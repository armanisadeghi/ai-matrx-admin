...
export const aiModelEndpointConfig: RelationshipMakerConfig = {
    ...
    parentEntity: "entityOne",
    childEntity: "entityTwo",
    ...
    parentDisplayConfig: {
        ...
        secondaryFields: [{ field: "companyDescription", format: (v) => truncateText(v) }],
    },
    childFilterConfig: [
        {
            label: "All",
        ...
        },
        {
            label: "Unassociated",
            value: "Unassociated",
            filterFn: (model, providers) => providers.filter((p: any) => p.models.some((m: any) => m.id === model.id)).length === 0,
        },
        {
            label: "1+ Matches",
            value: "1+ Matches",
    ...
    additionalFieldsConfig: [], // No additional fields for aiModelEndpoint
    theme: {
    ...
    onCreateChild: async (data) => {
        // Placeholder: Implement API call to create aiModel
        console.log("Creating child:", data);
    },
    onCreateParent: async (data) => {
        // Placeholder: Implement API call to create aiEndpoint
...
export const recipeMessageConfig: RelationshipMakerConfig = {
  ...
  childDisplayConfig: {
    primaryField: "role",
    fallbackPrimary: "Message With No Role",
    secondaryFields: [
  ...
  childFilterConfig: [
    ...
    },
    {
      ...
      filterFn: (child, parents) =>
        parents.filter((p: any) => p.models.some((m: any) => m.id === child.id)).length === 0,
    },
    {
      label: "1+ Matches",
      value: "1+ Matches",
  ...
  onCreateChild: async (data) => {
    ...
    // TODO: Implement API call to create messageTemplate
  },
  onCreateParent: async (data) => {
