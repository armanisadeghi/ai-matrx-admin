...
export function useConversationMessages(
  ...
  const records = useSelector((state: RootState) =>
    selectors.selectAllRecords(state),
  );
  const loadingState = useSelector((state: RootState) =>
    selectors.selectLoadingState(state),
  );
  const paginationInfo = useSelector((state: RootState) =>
    selectors.selectPaginationInfo(state),
  );

  const currentMessages = useSelector((state: RootState) =>
    selectors.selectRecordsByFieldValue(
      state,
      "conversationId",
      conversationId,
    ),
  ...
  const fetchWithDynamicFilters = useCallback(
