...
export async function POST(request: Request) {
	...
	}).filter(Boolean);  // Ensure valid JSON only

	// Manual validation for "input" (either text or File)
	if (typeof input !== "string" && !(input instanceof File)) {
		return new Response("Invalid input", { status: 400 });
	}

	// Manual validation for "message" array
...
// Helper function to check if a message object is valid
