...
const COOLIFY_API_URL = process.env.COOLIFY_API_URL ?? 'https://coolify.app.matrxserver.com/api/v1';
...
export async function GET(request: NextRequest) {
    ...
    if (!uuid) {
        return NextResponse.json({ error: `Unknown app: ${appKey}` }, { status: 400 });
    }
