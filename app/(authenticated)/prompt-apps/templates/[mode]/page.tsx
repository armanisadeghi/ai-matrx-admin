...
import { getTemplateForDisplayMode, DISPLAY_MODE_OPTIONS } from '@/features/prompt-apps/sample-code/templates';
import { TemplatePreviewRenderer } from '@/features/prompt-apps/components/TemplatePreviewRenderer';
import type { AppDisplayMode } from '@/features/prompt-apps/types';
import type { Metadata } from 'next';

const VALID_MODES: AppDisplayMode[] = ['form', 'form-to-chat', 'chat', 'centered-input', 'chat-with-history'];
...
export default async function TemplateDemoPage({
    ...
    const nextMode = currentIndex < VALID_MODES.length - 1 ? VALID_MODES[currentIndex + 1] : null;
    const prevOption = prevMode ? DISPLAY_MODE_OPTIONS.find(o => o.value === prevMode) : null;
    const nextOption = nextMode ? DISPLAY_MODE_OPTIONS.find(o => o.value === nextMode) : null;
            ...
            <div className="flex-shrink-0 px-4 py-1.5 border-b border-border/50 bg-muted/30">
                <p className="text-xs text-muted-foreground">
                    ...
                    <span className="text-muted-foreground/60">
                        — Responses are simulated mock data. Try interacting with the template below.
                    </span>
                </p>
            </div>
