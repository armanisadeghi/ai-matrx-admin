...
import { UserInputData } from './UserInputNode';
...
const UserInputEditor: React.FC<UserInputEditorProps> = ({ 
  ...
  const handleValueChange = (value: string) => {
    ...
    if (editingNode.data_type === 'number') {
      processedValue = value ? parseFloat(value) || 0 : null;
    } else if (editingNode.data_type === 'boolean') {
      processedValue = value.toLowerCase() === 'true';
    } else if (editingNode.data_type === 'json') {
      try {
  ...
  const getValueAsString = () => {
    ...
    if (editingNode.data_type === 'json' && typeof editingNode.value === 'object') {
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    value={editingNode.data_type || 'string'} 
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {editingNode.data_type === 'boolean' ? (
                ...
                <Input
                  type={editingNode.data_type === 'number' ? 'number' : 'text'}
                  value={getValueAsString()}
                  onChange={(e) => handleValueChange(e.target.value)}
