...
export default function EntityJsonBuilderWithSelect({
  ...
  const fieldOptions = useAppSelector(
    selectors.selectNativeFieldOptionsWithDefaults,
  );
  ...
  const generateMatrxRecordId = useCallback(() => {
    ...
    const record = fieldValues.reduce(
      (acc, { field, value }) => {
        acc[field] = value;
        return acc;
      },
    ...
    try {
      ...
      const newFieldValues = fieldValues.filter(
        (fv) => fv.field !== "matrxRecordId",
      );
      const updatedFields = [
        ...newFieldValues,
        {
  ...
  useEffect(() => {
    if (value) {
      try {
        ...
        const initialFields = Object.entries(parsedValue).map(
          ([field, value]) => ({
            field,
            value: String(value ?? ""),
          }),
  ...
  const updateJsonOutput = (fields: FieldValue[]) => {
    const jsonObject = fields.reduce(
      (acc, { field, value }) => {
  ...
  const handleAdd = () => {
    ...
    if (fieldToAdd) {
      const newFieldValues = [
        ...fieldValues,
        { field: fieldToAdd, value: valueToAdd },
      ];
