...
export default function SocketTester() {
    ...
    // Access responses from the Redux store
    ...
    const handleSend = () => {
        try {
            ...
            socketManager.listenForResponse(event, parsedPayload[0]?.index || 0, (data) => {
                ...
                setLog((prevLog) => [
                    ...prevLog,
                    `Response received for ${dynamicEventName}: ${JSON.stringify(data, null, 2)}`,
                ]);
            });
                         ...
                         <ul className="space-y-2">
                             {responses &&
                                 Object.entries(responses).map(([key, response], index) => (
                                     <li key={index} className="text-sm text-primary">
                                         <strong>{key}:</strong> {JSON.stringify(response, null, 2)}
                                     </li>
                                 ))}
                         </ul>
                     )}
