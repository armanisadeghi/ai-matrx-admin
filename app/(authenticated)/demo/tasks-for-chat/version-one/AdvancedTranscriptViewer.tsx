...
type ViewMode = "detailed" | "compact" | "text-only";
...
const AdvancedTranscriptViewer = ({
    ...
    const handleSplitPositionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const pos = parseInt(e.target.value, 10);
        setSplitPosition(pos);

        if (splitSegment) {
            setSplitPreview({
                before: splitSegment.text.substring(0, pos),
                after: splitSegment.text.substring(pos),
            });
        }
    };
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button
                                            ...
                                            variant="ghost"
                                            className="h-7 w-7"
                                            onClick={() => openSplitModal(segment)}
                                        >
    ...
    const renderTranscriptContent = () => {
        if (viewMode === "text-only") {
            return (
                <div className="text-sm space-y-2 leading-relaxed">{transcript.map((segment, index) => renderSegment(segment, index))}</div>
            );
        }
    ...
    return (
        <TooltipProvider>
            <Card className="w-full">
                ...
                <CardContent className="pt-2">{renderTranscriptContent()}</CardContent>
            </Card>
                    ...
                    <div className="space-y-4 py-2">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="e.g. Speaker A, Interviewer"
                            />
                        </div>
