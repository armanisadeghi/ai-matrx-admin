...
export function ImageViewer({
    ...
    const preloadImages = useCallback(async (currentPage: number) => {
        const imagesToLoad = [-2, -1, 0, 1, 2].map(offset =>
            photos[wrap(0, photos.length, currentPage + offset)]?.urls.regular
        ).filter(Boolean);
    ...
    return (
        ...
        >
            <motion.div
                className={`relative bg-transparent rounded-lg overflow-hidden shadow-xl ${isFullScreen ? 'w-full h-full' : 'max-w-4xl w-full max-h-[90vh]'} flex flex-col`}
                ...
                {!isFullScreen && (
                    <>
                        <div className="p-4 bg-card/80 backdrop-blur-sm text-card-foreground">
                            ...
                            <p className="text-sm text-muted-foreground mb-2">{photos[imageIndex]?.description || photos[imageIndex]?.alt_description}</p>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button variant="outline" size="icon" onClick={() => onFavorite(photos[imageIndex])}>
                                            <Heart className={`h-4 w-4 ${isFavorite(photos[imageIndex]) ? 'fill-current text-red-500' : ''}`} />
                                        </Button>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        Favorite
                            ...
                            <div className="flex space-x-2 overflow-x-auto">
                                {photos.map((photo, index) => (
                                    <img
                                        ...
                                        className={`w-20 h-20 object-cover rounded cursor-pointer ${index === imageIndex ? 'ring-2 ring-primary' : ''}`}
                                        onClick={() => {
