...
const ALL_COLUMNS: ColumnDef[] = [
    { key: "status", label: "•", sortable: true, defaultVisible: true, widthClass: "w-8" },
    { key: "overlayId", label: "overlayId", sortable: true, defaultVisible: true, widthClass: "min-w-[210px]" },
    { key: "kind", label: "kind", sortable: true, defaultVisible: true, widthClass: "w-20" },
    { key: "label", label: "label", sortable: true, defaultVisible: true, widthClass: "min-w-[160px]" },
    { key: "slug", label: "slug", sortable: true, defaultVisible: false, widthClass: "min-w-[200px]" },
    { key: "mobilePresentation", label: "mobile", sortable: true, defaultVisible: true, widthClass: "w-24" },
    { key: "mobileSidebarAs", label: "mobile sidebar", sortable: true, defaultVisible: false, widthClass: "w-24" },
    { key: "instanceMode", label: "instance", sortable: true, defaultVisible: true, widthClass: "w-20" },
    { key: "urlSync", label: "?panels=", sortable: true, defaultVisible: true, widthClass: "min-w-[120px]" },
    { key: "ephemeral", label: "ephemeral", sortable: true, defaultVisible: true, widthClass: "w-20" },
    { key: "tile", label: "tile", sortable: true, defaultVisible: true, widthClass: "w-16" },
    { key: "defaultData", label: "defaultData keys", sortable: false, defaultVisible: true, widthClass: "min-w-[200px]" },
    { key: "componentPath", label: "componentImport", sortable: false, defaultVisible: false, widthClass: "min-w-[300px]" },
];
...
export default function WindowPanelsRegistryPage() {
    ...
    const [filterEphemeral, setFilterEphemeral] = useState<"all" | "yes" | "no">("all");
            ...
            >
                ...
                {visibleColumns.map((col) => (
                    <TableCell key={col.key} className={cn(col.widthClass, "px-2 py-1")}>
                        {renderCell(col.key, entry, result, hasTile, defaultDataKeys, componentPath)}
                    </TableCell>
                ))}
            </TableRow>
            {expanded ? (
...
function renderCell(
    ...
    switch (key) {
        ...
        case "slug":
            return <span className="font-mono text-muted-foreground">{entry.slug}</span>;
        ...
        <div className="grid grid-cols-1 gap-3 px-4 py-3 md:grid-cols-2">
            ...
            <section className="space-y-3">
                ...
                <div className="text-[10px] text-muted-foreground">
                    Centralization gaps for this entry — open these to add
                    them to a richer metadata schema:
                    <ul className="ml-4 mt-1 list-disc space-y-0.5">
                        ...
                        {entry.kind === "window" &&
                        !entry.mobilePresentation ? (
                            <li>missing <code>mobilePresentation</code></li>
                        ) : null}
                        <li className="opacity-60">
                            <code>defaultRect</code> (width / height / minWidth /
                            minHeight / position) — not in schema
                        </li>
                        <li className="opacity-60">
                        ...
                        <li className="opacity-60">
                            <code>events</code> declaration (callback contract) —
                            not in schema
                        </li>
                        <li className="opacity-60">
                            <code>persistence</code>{" "}
                            (snapshot / restore / autosave / heavySnapshot) —
                            schema fields exist but not wired
                        </li>
                    </ul>
                </div>
            </section>
...
function sortValue(
    ...
    switch (key) {
        case "status": {
            ...
            return (
                { error: "0", pending: "1", ok: "2", untested: "3" }[s] ?? "9"
            );
        }
        case "overlayId":
