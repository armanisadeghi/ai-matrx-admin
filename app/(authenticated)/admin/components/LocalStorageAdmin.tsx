...
function useLocalStorageManager(): UseLocalStorageManager {
    ...
    const getItem = useCallback(async <T,>(module: string, feature: string, key: string): Promise<T | null> => {
        try {
            ...
            console.error(`LocalStorageAdmin: Error retrieving item from localStorage: ${error}`);
            return null;
        }
    }, [constructKey]);
    ...
    const getAllKeys = useCallback(async (): Promise<string[]> => Object.keys(localStorage), []);
    ...
    const setCookie = useCallback(async (name: string, value: string, options: CookieOptions = {}): Promise<StorageVerification> => {
        try {
    ...
    const verifyCookie = useCallback(async (name: string): Promise<StorageVerification> => {
        try {
            ...
            return cookies[name]
                ? { success: true, message: `Cookie found: ${name}`, data: cookies[name] }
    ...
    const verifyStorageItem = useCallback(async (module: string, feature: string, key: string): Promise<StorageVerification> => {
        try {
            const fullKey = constructKey(module, feature, key);
            const value = localStorage.getItem(fullKey);
            return value
                ? { success: true, message: `Storage item found: ${fullKey}`, data: JSON.parse(value) }
    ...
    const exportStorageData = useCallback(async (): Promise<string> => JSON.stringify(localStorage), []);

    const importStorageData = useCallback(async (data: string): Promise<StorageVerification> => {
        try {
            const parsedData = JSON.parse(data);
            for (const key in parsedData) localStorage.setItem(key, parsedData[key]);
    ...
    return {
        setItem, getItem, removeItem, clearFeature, clearModule,
        getAllKeys, getAllModules, getAllFeatures, clearAll,
        getCookies, setCookie, verifyCookie, verifyStorageItem,
        getStorageSize, exportStorageData, importStorageData,
        getCookieDetails, removeCookie,
    };
}
// -------------------------------------------------------------------------
