...
const DefaultEntitySearchableFkSelect = React.forwardRef<
  HTMLDivElement,
  EntityForeignKeySelectProps
>(
  (
    {
      entityKey: parentEntity,
      dynamicFieldInfo,
      value,
      onChange,
      disabled,
    ...
    const relatedEntity = dynamicFieldInfo.foreignKeyReference
      ?.entity as EntityKeys;
    ...
    const { handleRecordSelect, quickReferenceRecords, setFetchMode } =
      useSelectQuickRef(relatedEntity);
    const [isSheetOpen, setIsSheetOpen] = useState(false);
    // Find the recordKey that matches our primary key value
    const matchingRecord = quickReferenceRecords?.find(
      (record) =>
        record.primaryKeyValues[Object.keys(record.primaryKeyValues)[0]] ===
        value,
    );
    const recordKeyForValue = matchingRecord?.recordKey ?? "";
    const [selectedRecordKey, setSelectedRecordKey] =
      useState<MatrxRecordId>(recordKeyForValue);

    // Track if we have a value that's not in the available records
    const [hasUnknownValue, setHasUnknownValue] = useState<boolean>(
    ...
    const handleLocalChange = useCallback(
      (recordKey: string) => {
        ...
        const primaryKeyValue =
          recordKey === ""
            ? null
            : quickReferenceRecords?.find(
                (record) => record.recordKey === recordKey,
              )?.primaryKeyValues[
                Object.keys(quickReferenceRecords[0].primaryKeyValues)[0]
              ];
...
const EntitySearchableFkSelect = React.forwardRef<
  HTMLDivElement,
  EntityForeignKeySelectProps
>((props, ref) => {
  const relatedEntity = props.dynamicFieldInfo.foreignKeyReference
    ?.entity as EntityKeys;
  ...
  useEffect(() => {
    ...
    if (relatedEntity === "fieldComponents") {
      console.log(
        "EntitySearchableFkSelect has a field component",
        relatedEntity,
      );
    }
  }, [relatedEntity]);
