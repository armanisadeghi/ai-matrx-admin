...
const SchemaBasedJsonEditor = forwardRef<
  HTMLInputElement,
  SchemaBasedJsonEditorProps
>(
  (
    { schema, value, onChange, disabled = false, onFocus, onBlur, className },
    ref,
  ) => {
      ...
      Object.entries(schema).forEach(([key, field]) => {
        switch (field.type) {
          ...
          case "object":
            defaults[key] =
              field.default ||
              (field.fields ? generateDefaultValues(field.fields) : {});
            break;
        }
      });
      return defaults;
    ...
    const renderField = (
      ...
      const currentValue = path.reduce((obj, key) => obj[key], formData)[
        fieldName
      ];
            ...
            <div key={fieldName} className={`mb-2 ${isNested ? "ml-4" : ""}`}>
              ...
              >
                <Checkbox
                  ...
                  disabled={disabled}
                  onCheckedChange={(v) =>
                    handleFieldChange(currentPath, v === true)
                  }
                  className="shrink-0"
                />
              ...
              <div className="space-y-2">
                {currentValue.map((item: any, index: number) => (
                  <div key={index} className="flex items-center gap-2">
                    {field.itemType &&
                      renderArrayItem(field.itemType, item, [
                        ...currentPath,
                        index.toString(),
                      ])}
                    {!disabled && (
                      <Button
                        ...
                        onClick={() =>
                          handleArrayItemRemove(currentPath, index)
                        }
                      >
                        <X className="h-4 w-4" />
                      </Button>
        ...
        case "object":
          return (
            <div className="flex-1 border rounded p-2 bg-background">
              {itemSchema.fields &&
                Object.entries(itemSchema.fields).map(([fieldName, field]) =>
          ...
          return (
            <div className="flex flex-wrap gap-1">
              {value.map((item: any, index: number) => (
                <span
                  key={index}
                  className="px-2 py-0.5 bg-secondary text-secondary-foreground rounded-full text-sm"
                >
                  {field.itemType
                    ? renderViewValue(item, field.itemType)
                    : item}
                </span>
              ))}
            </div>
