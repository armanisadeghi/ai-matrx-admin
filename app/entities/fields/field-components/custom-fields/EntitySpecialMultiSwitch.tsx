...
const EntitySpecialMultiSwitch = React.forwardRef<
  HTMLElement,
  EntitySpecialMultiSwitchProps
>(({ value, onChange, disabled, className, dynamicFieldInfo }, ref) => {
  ...
  const states = (() => {
    ...
    if (
      ...
      PRESET_CONFIGURATIONS[componentProps.preset]
    ) {
      return PRESET_CONFIGURATIONS[componentProps.preset].map((state) => ({
        ...state,
        icon: state.iconName
          ? React.createElement(getIconComponent(state.iconName), { size: 14 })
          : null,
      }));
    }
    ...
    if (Array.isArray(componentProps?.states)) {
      return componentProps.states.map((state) => {
        if (state.iconName) {
          ...
          return {
            ...state,
            icon: React.createElement(getIconComponent(state.iconName), {
              size: 14,
            }),
          };
        }
  ...
  const effectiveValue =
    value ?? dynamicFieldInfo?.defaultValue ?? states[0]?.value;
