...
const EntitySpecialIconSelect = React.forwardRef<
  HTMLElement,
  EntitySpecialIconSelectProps
  ...
  const items = (() => {
    ...
    if (
      ...
      return PRESET_CONFIGURATIONS[componentProps.preset].map((item) => ({
        ...item,
        icon: item.iconName
          ? React.createElement(getIconComponent(item.iconName), { size: 16 })
          : null,
      }));
    }
    ...
    if (Array.isArray(componentProps?.items)) {
      return componentProps.items.map((item) => {
        ...
        if (item.iconName) {
          return {
            ...item,
            icon: React.createElement(getIconComponent(item.iconName), {
              size: 16,
            }),
          };
        }
