...
export const useEntityFinal = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  ...
  const currentPageFiltered = useAppSelector(
    selectors.selectCurrentPageFiltered,
  );
  ...
  const currentPageWithRecordId = useAppSelector(
    selectors.selectCurrentPageWithRecordId,
  );
  ...
  const selectedRecordsWithKey = useAppSelector(
    selectors.selectSelectedRecordsWithKey,
  );
  ...
  const effectiveRecordOrDefaults = useMemo(() => {
    return (matrxRecordId: MatrxRecordId) =>
      selectors.selectEffectiveRecordOrDefaults(
        store.getState(),
        matrxRecordId,
      );
  }, [selectors, store]);
  ...
  const recordsByPrimaryKeys = useMemo(() => {
    return (primaryKeyValuesList: Record<string, MatrxRecordId>[]) =>
      selectors.selectRecordsByPrimaryKeys(
        store.getState(),
  ...
  const matrxRecordIdByPrimaryKey = useMemo(() => {
    return (primaryKeyValues: Record<string, MatrxRecordId>) =>
      selectors.selectMatrxRecordIdByPrimaryKey(
        store.getState(),
        primaryKeyValues,
      );
  }, [selectors, store]);
  ...
  const matrxRecordIdsByPrimaryKeys = useMemo(() => {
    return (primaryKeyValuesList: Record<string, MatrxRecordId>[]) =>
      selectors.selectMatrxRecordIdsByPrimaryKeys(
        store.getState(),
        primaryKeyValuesList,
      );
  }, [selectors, store]);
  ...
  const quickReferenceByPrimaryKey = useMemo(() => {
    return (primaryKeyValues: Record<string, MatrxRecordId>) =>
      selectors.selectQuickReferenceByPrimaryKey(
        store.getState(),
        primaryKeyValues,
      );
  }, [selectors, store]);
  ...
  const fetchRecords = useCallback(
    (
      page: number,
      pageSize: number,
      options?: FetchRecordsPayload["options"],
    ) => {
  ...
  const updateRecord = React.useCallback(
    (
      ...
      const callbackId = callbackManager.register(wrappedCallback);
      dispatch(
        actions.updateRecord({ matrxRecordId, data: updates, callbackId }),
      );
    },
    [actions, dispatch],
  );
