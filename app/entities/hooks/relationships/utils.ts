...
export function getOrderedRecords({
    childRecords,
    joiningRecords,
    orderField,
    childField,
}: OrderedRecordsOptions): EntityData<EntityKeys>[] {
    // Create a map of childField values to their respective orders
    const orderMap = _.reduce(joiningRecords, (acc, joinRecord) => {
...
export type CommonRelationshipStatus = 
    | 'active' 
    | 'inactive'
    | 'deleted'
    | 'isDeleted'
...
export function filterRecordsByStatus({
    ...
    not = false,
    includeNull = false,
}: StatusFilterOptions): EntityData<EntityKeys>[] {
...
export function getActiveRecords(options: StatusRecordsOptions): EntityData<EntityKeys>[] {
...
export function getNonDeletedRecords(options: StatusRecordsOptions): EntityData<EntityKeys>[] {
    return filterRecordsByStatus({
        ...options,
...
export function canTransitionStatus({
    ...
    fromStatus,
    toStatus,
}: StatusTransitionOptions): boolean {
...
export function groupRecordsByStatus({
    records,
    statusField,
}: StatusGroupOptions): Record<RelationshipStatus, EntityData<EntityKeys>[]> {
...
export function getStatusCounts({
    records,
    statusField,
}: StatusCountOptions): Record<RelationshipStatus, number> {
...
type ApplyDefaultValueOptions = {
    ...
    joiningRecord: EntityData<EntityKeys>;
...
export function applyDefaultValue({
    ...
    joiningRecord,
    defaultValueField,
}: ApplyDefaultValueOptions): EntityData<EntityKeys> {
    ...
    const isNullLike = currentValue === null || 
...
export function applyDefaultValues({
    ...
    defaultValueField,
    childField,
}: ApplyDefaultValuesOptions): EntityData<EntityKeys>[] {
...
export function mergeJoinFields({
    childRecords,
    joiningRecords,
    relationshipDefinition
}: MergeJoinFieldsOptions): EntityData<EntityKeys>[] {
...
export function processJoinedData({
    ...
    relationshipDefinition,
    filterStatus
}: ProcessJoinedDataOptions): EntityData<EntityKeys>[] {
    // Exit early if we don't have valid inputs
    if (!childRecords?.length || !joiningRecords?.length) {
        return childRecords;
    }
...
export function filterJoinForChild(
    ...
    return allJoiningRecords.filter(record => 
        record[childField] === childIdValue
    );
}
