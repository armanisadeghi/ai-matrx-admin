...
export function getUpdatedUnifiedLayoutProps(
  ...
  const {
    ...
    fieldFiltering = existingProps.dynamicLayoutOptions?.formStyleOptions
      ?.fieldFiltering,
  ...
  const mergedDynamicLayoutOptions = {
    ...
    componentOptions: {
      ...
      ...otherOverrides?.dynamicLayoutOptions?.componentOptions,
      quickReferenceType:
        finalQuickReferenceType as QuickReferenceComponentType,
    },
    formStyleOptions: {
...
export const getSimplifiedLayoutProps = ({
  ...
  if (!entityKey || !handlers || !defaultShownFields) {
    throw new Error(
      "Required parameters missing: entityKey, handlers, and defaultShownFields are required",
    );
  }
