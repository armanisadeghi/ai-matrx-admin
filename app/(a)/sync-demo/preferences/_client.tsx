...
export function PreferencesDemoClient() {
    ...
    useEffect(() => {
        ...
        bc.onmessage = (e) => {
            setEvents((prev) =>
                [{ at: Date.now(), raw: JSON.stringify(e.data) }, ...prev].slice(
                    0,
                    20,
                ),
            );
        ...
        <div className="flex h-[calc(100vh-2.5rem)] flex-col gap-4 overflow-y-auto bg-textured p-6">
            <header>
                ...
                <p className="text-sm text-muted-foreground">
                    ...
                    localStorage fallback + debounced Supabase upsert +
                    cross-tab broadcast. Open in two tabs to watch broadcast
                    propagation.
                </p>
            </header>
            ...
            <section className="rounded-md border border-border bg-card p-4">
                ...
                <dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-sm">
                    <dt className="text-muted-foreground">Redux prompts.defaultTemperature:</dt>
            ...
            <section className="rounded-md border border-border bg-card p-4">
                ...
                <p className="mb-2 text-xs text-muted-foreground">
                    Compound key: <code>{mounted ? `${identityKey}:${SLICE_NAME}:${IDB_SCHEMA_VERSION}` : "—"}</code>
                </p>
                {mounted && idbRecord ? (
                    <dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-xs">
                        ...
                        <dt className="text-muted-foreground">body.prompts.defaultTemperature:</dt>
                        <dd className="font-mono">
                            {String(
                                (idbRecord.body as { prompts?: { defaultTemperature?: number } })
            ...
            <section className="rounded-md border border-border bg-card p-4">
                <h2 className="mb-2 text-sm font-medium text-muted-foreground">
                    localStorage fallback mirror — <code className="text-xs">{LS_FALLBACK_KEY}</code>
                </h2>
            ...
            <section className="rounded-md border border-border bg-card p-4">
                ...
                <p className="mb-3 text-xs text-muted-foreground">
                    Same control as the theme demo. Format: <code>auth:&lt;userId&gt;</code>
