...
export default function GenerateShellClient() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.info(
        notImpl
          ? "Generate endpoint ships next wave — see features/image-studio/AI-AGENTS.md"
          : msg,
      );
    } finally {
      setBusy(false);
          ...
          {results.map((r) => (
            ...
            >
              ...
              <div className="p-2 flex flex-wrap gap-1.5 text-xs">
                <Link
                  href={`/image-studio/edit?cloudFileId=${encodeURIComponent(r.cloud_file_id)}`}
                  className="px-2 py-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground"
                >
                  Edit
                </Link>
                <Link
                  href={`/image-studio/annotate?cloudFileId=${encodeURIComponent(r.cloud_file_id)}`}
                  className="px-2 py-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground"
                >
                  Annotate
                </Link>
                <Link
                  href={`/image-studio/avatar?cloudFileId=${encodeURIComponent(r.cloud_file_id)}`}
                  className="px-2 py-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground"
                >
                  Use as avatar
                </Link>
