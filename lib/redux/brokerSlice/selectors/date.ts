...
const selectIsToday = createSelector([selectDate], (date): boolean => {
  if (!date) return false;
  const today = new Date();
  return date.toDateString() === today.toDateString();
});

const selectIsPast = createSelector([selectDate], (date): boolean => {
  if (!date) return false;
  return date < new Date();
});

const selectIsFuture = createSelector([selectDate], (date): boolean => {
  if (!date) return false;
  return date > new Date();
});
