...
const selectError = createSelector([selectBrokerSlice], (state) => state.error);
...
const selectIsBrokerIdMapped = createSelector(
  ...
  (map, brokerId) =>
    Object.values(map).some((entry) => entry.mappedItemId === brokerId),
);
...
const selectMapEntry = createSelector(
  [
    selectMap,
    (_: RootState, source: string, mappedItemId: string) => ({
      source,
      mappedItemId,
    }),
  ],
