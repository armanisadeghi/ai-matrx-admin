...
const selectSortedRows = createSelector([selectTable], (table): Row[] => {
  if (!table) return [];
  return [...table.rows].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
});

const selectSortedColumns = createSelector([selectTable], (table): Column[] => {
  if (!table) return [];
...
const selectRowById = createSelector(
  ...
  (table, rowId): Row | undefined =>
    table?.rows.find((row) => row.id === rowId),
);

const selectColumnById = createSelector(
  [
    selectTable,
    (_: RootState, __: BrokerIdentifier, columnId: string) => columnId,
  ],
  (table, columnId): Column | undefined =>
    table?.columns.find((col) => col.id === columnId),
);
