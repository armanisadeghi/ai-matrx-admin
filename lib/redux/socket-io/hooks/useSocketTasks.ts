...
export const useSocketTask = (taskId?: string, connectionId?: string) => {
  ...
  const updateField = useCallback(
    (id: string, field: string, value: any) =>
      dispatch(updateTaskField({ taskId: id, field, value })),
    [dispatch],
  );
  ...
  const addToArray = useCallback(
    (id: string, field: string, item: any) =>
      dispatch(addToArrayField({ taskId: id, field, item })),
    [dispatch],
  );

  // Set array field
  const setArray = useCallback(
    (id: string, field: string, items: any[]) =>
      dispatch(setArrayField({ taskId: id, field, items })),
    [dispatch],
  );
  ...
  const taskData = useSelector(
    taskId
      ? (state: RootState) => selectTaskDataById(state, taskId)
      : () => ({}),
  );
  ...
  const status = useSelector(
    taskId
      ? (state: RootState) => selectTaskStatus(state, taskId)
      : () => "not_found",
  );
  ...
  const isComplete = useSelector(
    taskId ? selectIsTaskComplete(taskId) : () => false,
  );
...
export const useSocketResponse = (listenerId: string) => {
  const stream = useSelector(
    (state: RootState) => state.socketResponse[listenerId] || null,
  );
  // Use performance-optimized text selector for text chunks
  const text = useSelector((state: RootState) =>
    selectCombinedText(listenerId)(state),
  );
  const data = useSelector(
    (state: RootState) => state.socketResponse[listenerId]?.data || [],
  );
  const info = useSelector(
    (state: RootState) => state.socketResponse[listenerId]?.info || [],
  );
  const errors = useSelector(
    (state: RootState) => state.socketResponse[listenerId]?.errors || [],
  );
  const hasErrors = useSelector(
    (state: RootState) =>
      (state.socketResponse[listenerId]?.errors.length || 0) > 0,
  );
  const isEnded = useSelector(
    (state: RootState) => state.socketResponse[listenerId]?.ended || false,
  );
  const relatedTask = useSelector((state: RootState) => {
    return (
      Object.values(state.socketTasks.tasks).find((task) =>
        task.listenerIds.includes(listenerId),
      ) || null
    );
  });
