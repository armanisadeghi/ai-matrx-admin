...
export const selectSocket = (state: RootState, connectionId?: string) => {
  const effectiveConnectionId =
    connectionId || state.socketConnections.primaryConnectionId;
...
export const selectConnectionStatus = (
  ...
  const effectiveConnectionId =
    connectionId || state.socketConnections.primaryConnectionId;
  return state.socketConnections.connections[effectiveConnectionId]
    ?.connectionStatus;
};
...
export const selectIsConnected = (state: RootState, connectionId?: string) => {
  const effectiveConnectionId =
    connectionId || state.socketConnections.primaryConnectionId;
  return (
    state.socketConnections.connections[effectiveConnectionId]
...
export const selectConnectionAttempts = (
  state: RootState,
  connectionId: string,
...
export const selectConnectionHealth = createSelector(
  (state: RootState, connectionId: string) =>
    selectConnectionById(state, connectionId),
  (state: RootState, connectionId: string) =>
    selectConnectionAttempts(state, connectionId),
  (state: RootState, connectionId: string) =>
    selectConnectionError(state, connectionId),
  (state: RootState, connectionId: string) =>
    selectIsReconnecting(state, connectionId),
...
export const selectAnyConnectionReconnecting = createSelector(
  (state: RootState) => state.socketConnections.reconnectingConnections,
  (reconnecting) =>
    Object.values(reconnecting).some((isReconnecting) => isReconnecting),
);
...
export const selectActiveConnections = createSelector(
  ...
  (connections) =>
    connections.filter(
      (conn) => conn.connectionStatus === "connected" && conn.isAuthenticated,
    ),
);
...
export const selectFailedConnections = createSelector(
  ...
  (connections, errors) =>
    connections.filter(
      (conn) => conn.connectionStatus === "error" || errors[conn.connectionId],
    ),
);
