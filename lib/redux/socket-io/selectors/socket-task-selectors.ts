...
export const selectAllTasksArray = createSelector([selectAllTasks], (tasks) =>
  Object.values(tasks),
);

// ==================== Current Task Selectors ====================
export const selectCurrentTaskId = (state: RootState): string | null =>
  state.socketTasks.currentTaskId;
...
export const selectTaskValidationState = createSelector(
  [
    (state: RootState, taskId: string) =>
      selectAllTasks(state)[taskId]?.isValid,
    (state: RootState, taskId: string) =>
      selectAllTasks(state)[taskId]?.validationErrors,
  ],
...
export const selectBuildingTasksCount = createSelector(
  [selectAllTasksArray],
  (tasksArray) =>
    tasksArray.filter((task) => task.status === "building").length,
);
...
export const selectSubmittedTasksCount = createSelector(
  [selectAllTasksArray],
  (tasksArray) =>
    tasksArray.filter((task) => task.status === "submitted").length,
);
...
export const selectCompletedTasksCount = createSelector(
  [selectAllTasksArray],
  (tasksArray) =>
    tasksArray.filter((task) => task.status === "completed").length,
);
