...
export const selectResponseByListenerId = (listenerId: string | undefined) =>
  createSelector([(state: RootState) => state.socketResponse], (responses) =>
    listenerId
      ? (responses[listenerId] as ResponseState | undefined)
      : undefined,
  );
...
export const selectResponseTextByListenerId = (
...
export const selectResponseDataByListenerId = (
  listenerId: string | undefined,
) =>
  createSelector([selectResponseByListenerId(listenerId)], (response) => {
...
export const selectFirstResponseDataByListenerId = (
  listenerId: string | undefined,
) =>
  createSelector([selectResponseDataByListenerId(listenerId)], (data) =>
    data.length > 0 ? data[0] : null,
  );

export const selectResponseInfoByListenerId = (
  listenerId: string | undefined,
) =>
  createSelector([selectResponseByListenerId(listenerId)], (response) => {
    // Ensure transformation to avoid reselect identity function warning
    if (!listenerId || !response) return EMPTY_ARRAY;
    return response.info ?? EMPTY_ARRAY;
  });

export const selectResponseErrorsByListenerId = (
  listenerId: string | undefined,
) =>
  createSelector([selectResponseByListenerId(listenerId)], (response) => {
    // Ensure transformation to avoid reselect identity function warning
...
export const selectHasResponseErrorsByListenerId = (
  listenerId: string | undefined,
) =>
  createSelector(
...
export const selectFirstPrimaryResponseDataByTaskId = (taskId: string) =>
  createSelector([selectPrimaryResponseDataByTaskId(taskId)], (data) =>
    data.length > 0 ? data[0] : null,
  );

export const selectFirstPrimaryResponseInfoByTaskId = (taskId: string) =>
  createSelector([selectPrimaryResponseInfoByTaskId(taskId)], (info) =>
    info.length > 0 ? info[0] : null,
  );
...
export const createTaskResponseSelectors = (taskId: string) => {
  ...
  return {
    ...
    selectTextChunks: createSelector(
      [baseSelector],
      (response) => response?.textChunks || EMPTY_ARRAY,
    ),
