...
export class SocketConnectionManager {
  ...
  private connectionDetails: Map<string, { url: string; namespace: string }> =
    new Map();
  ...
  public static getPredefinedConnections(): PredefinedConnection[] {
    return [
      ...
      {
        name: "GPU Server",
        url: SocketConnectionManager.GPU_SERVER_URL,
        namespace: SocketConnectionManager.DEFAULT_NAMESPACE,
      },
    ];
  }
  ...
  private async establishConnection(
    ...
    return new Promise((resolve) => {
      ...
      socket.on("connect", () => {
        if (DEBUG)
          console.log(
            `[SOCKET] Connection ${connectionId} connected successfully`,
          );
        ...
        if (this.authToken) {
          ...
          if (DEBUG)
            console.log(
              `[SOCKET] Connection ${connectionId} authenticated with token`,
            );
        }
      ...
      socket.on("authenticated", () => {
        if (DEBUG)
          console.log(
            `[SOCKET] Connection ${connectionId} explicitly authenticated`,
          );
        this.authenticationStatus.set(connectionId, true);
      });
      ...
      socket.on("unauthorized", () => {
        if (DEBUG)
          console.log(
            `[SOCKET] Connection ${connectionId} unauthorized - token rejected`,
          );
        this.authenticationStatus.set(connectionId, false);
      });
      ...
      socket.on("connect_error", (error) => {
        if (DEBUG)
          console.log(
            `[SOCKET] Connection ${connectionId} attempt failed:`,
            error.message,
          );
        this.authenticationStatus.set(connectionId, false);
        ...
        if (attempts >= this.maxConnectionAttempts) {
          if (DEBUG)
            console.log(
              `[SOCKET] Max connection attempts reached for ${connectionId}`,
            );
          socket.disconnect();
  ...
  public async initializePrimaryConnection(): Promise<string> {
    ...
    await this.getSocket(
      ...
      SocketConnectionManager.DEFAULT_NAMESPACE,
    );
    return connectionId;
  }
  ...
  public async reconnect(connectionId: string): Promise<any> {
    ...
    if (!details) {
      if (DEBUG)
        console.log(
          `[SOCKET] Cannot reconnect, no details found for connection ${connectionId}`,
        );
      return null;
    }
    ...
    if (DEBUG)
      console.log(
        `[SOCKET] Attempting to reconnect ${connectionId} to ${details.url}${details.namespace}`,
      );
  ...
  public getUrl(connectionId: string): string {
    return (
      this.connectionDetails.get(connectionId)?.url ||
      SocketConnectionManager.DEFAULT_URL
    );
  }
  ...
  public getNamespace(connectionId: string): string {
    return (
      this.connectionDetails.get(connectionId)?.namespace ||
      SocketConnectionManager.DEFAULT_NAMESPACE
    );
  }
  ...
  public async forceReconnectAll(): Promise<void> {
    ...
    this.sockets.forEach((socket, connectionId) => {
      if (socket && !socket.connected) {
        console.log(
          `[SOCKET] Force reconnecting ${connectionId} after visibility change`,
        );
      ...
      async () => {
        if (this.authToken && this.isClientSide) {
          try {
            ...
            if (DEBUG)
              console.log(
                "[SOCKET] Session keep-alive failed, attempting to refresh token",
              );
            this.authToken = await this.getAuthToken();
          }
        }
    ...
    setInterval(() => {
      this.sockets.forEach((socket, connectionId) => {
        if (socket && socket.connected) {
          socket.emit("ping", { timestamp: Date.now() });
          if (DEBUG)
            console.log(`[SOCKET] Sent keep-alive ping to ${connectionId}`);
        }
      });
