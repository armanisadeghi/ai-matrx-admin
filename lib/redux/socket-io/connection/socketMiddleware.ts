  ...
  (action: SocketAction) => {
    ...
    switch (action.type) {
      case "socketConnections/changeConnectionUrl":
        ...
        dispatch(
          setConnection({
            ...
            url: urlChange,
            namespace:
              getState().socketConnections.connections[connectionId]
        ...
        socketManager
          .getSocket(
            connectionId,
            urlChange,
            getState().socketConnections.connections[connectionId]?.namespace ||
              "/UserSession",
          )
          .then((socket: Socket | null) => {
            if (socket) {
              ...
              const isAuthenticated =
                socketManager.isAuthenticated(connectionId);
      ...
      case "socketConnections/changeNamespace":
        const { connectionId: nsConnectionId, namespace: namespaceChange } =
          action.payload;
        ...
        dispatch(
          setConnection({
            ...
            socket: null,
            url:
              getState().socketConnections.connections[nsConnectionId]?.url ||
        ...
        socketManager
          ...
          .then((socket: Socket | null) => {
            if (socket) {
              ...
              const isAuthenticated =
                socketManager.isAuthenticated(nsConnectionId);
              dispatch(
                setIsAuthenticated({
                  connectionId: nsConnectionId,
                  isAuthenticated,
      ...
      case "socketConnections/reconnectConnection":
        ...
        dispatch(
          setConnectionStatus({ connectionId: connId, status: "connecting" }),
        );
        ...
        socketManager.reconnect(connId).then((socket: Socket | null) => {
          if (socket) {
            ...
            dispatch(
              setIsAuthenticated({ connectionId: connId, isAuthenticated }),
            );

            setupSocketListeners(socket, store, connId);
          } else {
            dispatch(
              setConnectionStatus({ connectionId: connId, status: "error" }),
            );
            dispatch(
              setIsAuthenticated({
                connectionId: connId,
      ...
      case "socketConnections/addConnection":
        ...
        socketManager
          .getSocket(addConnectionId, newUrl, newNamespace)
          .then((socket: Socket | null) => {
            if (socket) {
              ...
              const isAuthenticated =
                socketManager.isAuthenticated(addConnectionId);
              dispatch(
                setIsAuthenticated({
                  connectionId: addConnectionId,
                  isAuthenticated,
...
function setupGlobalErrorListener(
  ...
  socket.on("global_error", (errorData) => {
    ...
    if (activeListenerIds.length > 0) {
      ...
      console.warn(
        `[SOCKET] Global error received for connection ${connectionId} but no active responses`,
      );
    }
  });
}
