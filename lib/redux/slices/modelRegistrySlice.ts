...
export const fetchAvailableModels = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    ...
    if (
      state.modelRegistry.lastFetched !== null &&
      state.modelRegistry.availableModels.length > 0
    ) {
...
const modelRegistrySlice = createSlice({
  ...
  reducers: {
    hydrateModels(
      state,
      action: { payload: { availableModels: AIModel[]; lastFetched: number } },
    ) {
...
}) => state.modelRegistry.availableModels;
export const selectModelRegistryLoading = (state: {
  modelRegistry: ModelRegistryState;
}) => state.modelRegistry.isLoading;
export const selectModelRegistryError = (state: {
  modelRegistry: ModelRegistryState;
}) => state.modelRegistry.error;

export const selectModelOptions = createSelector(
  [
    (state: { modelRegistry: ModelRegistryState }) =>
      state.modelRegistry.availableModels,
  ],
  (availableModels) =>
...
export const selectModelControls = (
  state: { modelRegistry: ModelRegistryState },
  modelId: string,
) => {
  const model = state.modelRegistry.availableModels.find(
    (m) => m.id === modelId,
  );
...
/**
 * Returns the display label (common_name → name → id) for a given model UUID.
 * Returns null when modelId is null/empty or the model isn't loaded yet.
 */
export const selectModelLabelById = createSelector(
  [
    (state: { modelRegistry: ModelRegistryState }) =>
      state.modelRegistry.availableModels,
    (_state: { modelRegistry: ModelRegistryState }, modelId: string | null) =>
      modelId,
  ],
  (models, modelId): string | null => {
