...
export function convertToFormat<T extends Record<string, any>>(
  ...
  switch (format) {
    ...
    default: {
      ...
      const entityMapping = Object.entries(entityFormats).reduce(
        (acc, [key, value]) => {
      ...
      const fieldMapping = Object.entries(fieldFormats).reduce(
...
export function convertObjectToPretty<T extends Record<string, any>>(
  ...
  const entityMap = Object.entries(entityFormats).reduce(
    (acc, [entity, formats]) => {
...
export function convertQueryToDatabase<T extends Record<string, any>>(
  ...
  return {
    ...
    filters: query.filters
      ? replaceKeysInObject(query.filters, fieldMap)
      : undefined,
