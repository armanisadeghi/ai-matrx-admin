...
export function createGlobalCacheSlice(initialData: UnifiedSchemaCache) {
  ...
  return createSlice({
    ...
    initialState,
    reducers: {
      // Replaces the entire cache state. Called by injectEntityReducers after
      // replaceReducer() because Redux preserves the existing (empty) state
      // for already-initialized keys — the slice's initialState is only used
      // when the key is undefined in the store.
      hydrateCache: (_state, action: PayloadAction<UnifiedSchemaCache>) => {
      ...
      updateField: (
        ...
        if (state.schema[entityName]) {
          ...
          if (entityFields) {
            ...
            console.log(
              `Field "${fieldId}" updated in schema for entity "${entityName}"`,
            );
          } else {
...
export const doesFieldExist = (
  ...
  if (state.schema[entityName]) {
    ...
    console.log(
      `Checking if field "${fieldId}" exists in entity "${entityName}":`,
      exists,
    );
    return exists;
  } else {
