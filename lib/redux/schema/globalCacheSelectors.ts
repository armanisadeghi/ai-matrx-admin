...
const selectGlobalCache = (state: RootState): GlobalCacheState =>
  state.globalCache;
...
export const selectEntities = createSelector(
  [selectGlobalCache],
  (cache) => cache.entitiesWithoutFields,
);
...
export const makeSelectEntityNameByFormat = (
  ...
  createSelector([selectEntityNameFormats], (entityNameFormats) => {
    ...
    return (
      (entityNameFormats[entityName][format] as AllEntityNameVariations) || null
    );
  });
...
export const selectField = createSelector(
  [
    selectSchema,
    (_: RootState, params: { entityName: EntityKeys; fieldName: string }) =>
      params,
  ],
  (entities, params) => {
...
export const selectEntityPrimaryKeyField = createSelector(
  [
    (state: RootState, entityName: EntityKeys) =>
      selectEntityFields(state, entityName),
  ],
...
export const selectEntityMetadata = createSelector(
  ...
  (
    ...
    if (!entity)
      return { primaryKeyMetadata: undefined, displayFieldMetadata: undefined };
...
export const selectEntityDatabaseName = createSelector(
  ...
  (entityToDatabase, entityName): string =>
    entityToDatabase[entityName] || entityName,
);
...
export const selectEntityBackendName = createSelector(
  ...
  (entityToBackend, entityName): string =>
    entityToBackend[entityName] || entityName,
);
...
export const selectEntityCanonicalName = createSelector(
  [
    ...
    (_: RootState, entityName: AllEntityNameVariations) => entityName,
  ],
  (toCanonicalMap, entityName): EntityKeys =>
    toCanonicalMap[entityName] || (entityName as EntityKeys),
);
...
export const selectEntityPrettyName = createSelector(
  ...
  (entityNameFormats, entityName) =>
    entityNameFormats[entityName]?.["pretty"] as PrettyEntityName<EntityKeys>,
);
...
export const selectEntityAnyName = createSelector(
  [
    selectEntityNameFormats,
    (_: RootState, params: { entityName: EntityKeys; format: NameFormat }) =>
      params,
  ],
  (entityNameFormats, { entityName, format }) =>
    entityNameFormats[entityName]?.[format] || entityName,
);
...
export const selectFieldAnyName = createSelector(
  [
    ...
    (
      _: RootState,
      params: { entityName: EntityKeys; fieldName: string; format: NameFormat },
    ) => params,
  ],
...
export const selectReplaceKeysInObject = createSelector(
  ...
  (data, keyMapping) => {
    ...
    if (Array.isArray(data)) {
      ...
      return processedData.every((item, index) => item === data[index])
        ? data
        : processedData;
    }
...
export const selectConvertDataFormat = createSelector(
  [
    ...
    (_: RootState, _data: any, entityNameMapping: KeyMapping) =>
      entityNameMapping,
    (
      _: RootState,
      _data: any,
      _entityNameMapping: any,
...
export const selectPrettyEntityMapping = createSelector(
  [selectEntityNameFormats],
  (entityNameFormats) =>
    Object.entries(entityNameFormats).reduce(
      (acc, [entity, formats]) => {
...
export const selectPrettyConversion = createSelector(
  [
    (_: RootState, payload: { entityName: EntityKeys; data: any }) =>
      payload.data,
...
export const selectAnyEntityMapping = createSelector(
  [selectEntityNameFormats, (_: RootState, format: NameFormat) => format],
...
export const selectUnknownToAnyObjectFormatConversion = createSelector(
  [
    (
      _: RootState,
      payload: {
        entityAlias: AllEntityNameVariations;
        data: any;
  ...
  (data, entityName, state, targetFormat) => {
    const canonicalData = selectCanonicalConversion(state, {
      entityName,
      data,
    });
    return selectAnyObjectFormatConversion(state, {
      entityName,
      data: canonicalData,
      format: targetFormat,
    });
...
export const selectQueryDatabaseConversion = createSelector(
  [
    ...
    (_: RootState, payload: QueryConversionPayload<EntityKeys>) =>
      payload.entityName,
    (_: RootState, payload: QueryConversionPayload<EntityKeys>) =>
      payload.options,
  ],
  (
    databaseTableName: AnyEntityDatabaseTable,
    fieldMap,
    entityName,
    options,
  ) => {
...
export const selectPayloadOptionsDatabaseConversion = createSelector(
  [
    ...
    (state: RootState, payload: PayloadOptionsConversionPayload<EntityKeys>) =>
      selectEntityDatabaseName(state, payload.entityName),
    (_: RootState, payload: PayloadOptionsConversionPayload<EntityKeys>) =>
      payload.entityName,
    (_: RootState, payload: PayloadOptionsConversionPayload<EntityKeys>) =>
      payload.options,
  ],
  (
    ...
    const result: QueryOptionsReturn<typeof entityName> = {
      ...
      ...(options.filters && {
        filters: selectReplaceKeysInObject(
          {} as RootState,
          options.filters,
          fieldMap,
        ) as Partial<Record<string, any>>,
...
export const selectUnifiedDatabaseObjectConversion = createSelector(
  [
    ...
    (state: RootState, options: FlexibleQueryOptions) => {
      const canonicalName = selectEntityCanonicalName(
        state,
        options.entityNameAnyFormat,
      );
  ...
  (
    ...
    const result: UnifiedDatabaseObject = {
      ...
      databaseDisplayField:
        fieldMap[frontendDisplayField] || frontendDisplayField,
    ...
    if (options.recordKeys) {
      ...
      result.parsedFrontendRecords = parseRecordKeys(
        options.recordKeys,
      ) as Record<AllEntityFieldKeys, string>[];

      if (result.parsedFrontendRecords) {
        result.parsedDatabaseRecords = result.parsedFrontendRecords.map(
          (record) => {
    ...
    const { primaryKeysAndValues, recordKeys } = result.databasePks.reduce(
      (acc, pk, index) => {
        ...
        if (
          index === result.databasePks.length - 1 &&
          acc.recordKeyParts.length > 0
        ) {
    ...
    logger.log(
      "debug",
      "Primary keys and values extracted:",
...
const convertToDatabase = (
  ...
  if (Array.isArray(data)) {
    return data.map((item) =>
      convertSingleObject(item as Record<string, unknown>),
    );
  }
...
export const selectUnifiedDatabaseObjectConversion2 = createSelector(
  ...
  (
    ...
    const tableName = selectEntityDatabaseName(
      state,
      entityName,
    ) as AnyEntityDatabaseTable;
    const fieldMap = selectEntityFieldNameToDatabaseMap(state, entityName);
    const primaryKeyMetadata = selectEntityPrimaryKeyMetadata(
      state,
      entityName,
    );
    const frontendDisplayField = selectEntityDisplayField(
      state,
      entityName,
    ) as AllEntityFieldKeys;
    ...
    if (options.data) {
      result.data = selectFrontendConversion(state, {
        entityName,
        data: options.data,
      });
    }
...
export const selectUnifiedQueryDatabaseConversion = createSelector(
  ...
  (
        ...
        return {
          ...
          ...(join.columns && {
            columns: join.columns.map(
              (column) => joinedTableFieldMap[column] || column,
            ),
          }),
        };
      });
    ...
    const result: UnifiedQueryOptions<typeof entityName> = {
      ...
      ...(options.having && {
        having: selectReplaceKeysInObject(
          {} as RootState,
          options.having,
          mainFieldMap,
        ) as Partial<Record<string, any>>,
