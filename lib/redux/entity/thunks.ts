...
>(
  ...
  async ({ entityKey, actions, payload }, { dispatch, getState }) => {
    if (
      ...
      payload.matrxRecordIds.every((id) => id === null)
    ) {
      return [];
    }
    ...
    try {
      const { existingRecords, recordIdsNotInState, primaryKeysToFetch } =
        entitySelectors.selectRecordsForFetching(payload.matrxRecordIds)(
          getState(),
        );
      ...
      if (primaryKeysToFetch.length > 0) {
        ...
        const queryOptions: QueryOptions<typeof entityKey> = {
          tableName,
          filters: primaryKeysToFetch.reduce(
            (acc, primaryKey) => {
        ...
        if (payload.fetchMode === "fkIfk") {
          for (const recordId of recordIdsNotInState) {
            try {
              ...
              const fetchedRecord = entitySelectors.selectRecordByKey(
                getState(),
                recordId,
              );
