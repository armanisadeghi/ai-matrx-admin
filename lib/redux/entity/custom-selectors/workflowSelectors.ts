...
export const createWorkflowSelectors = () => {
  ...
  const registeredFunctionSelectors =
    createEntitySelectors("registeredFunction");
  ...
  // ====== CORE DATA SELECTORS ======
  const workflows = (state: RootState) =>
    workflowSelectors.selectAllRecords(state) || {};
  const workflowsArray = (state: RootState) =>
    workflowSelectors.selectRecordsArray(state) || [];
  const activeWorkflow = (state: RootState) =>
    workflowSelectors.selectActiveRecord(state);

  const registeredFunctions = (state: RootState) =>
    registeredFunctionSelectors.selectAllRecords(state) || {};
  const registeredFunctionsArray = (state: RootState) =>
    registeredFunctionSelectors.selectRecordsArray(state) || [];
  const activeRegisteredFunction = (state: RootState) =>
    registeredFunctionSelectors.selectActiveRecord(state);

  const args = (state: RootState) => argSelectors.selectAllRecords(state) || {};
  const argsArray = (state: RootState) =>
    argSelectors.selectRecordsArray(state) || [];
  const activeArg = (state: RootState) =>
    argSelectors.selectActiveRecord(state);

  const dataBrokers = (state: RootState) =>
    dataBrokerSelectors.selectAllRecords(state) || {};
  const dataBrokersArray = (state: RootState) =>
    dataBrokerSelectors.selectRecordsArray(state) || [];
  const activeDataBroker = (state: RootState) =>
    dataBrokerSelectors.selectActiveRecord(state);
  ...
  const argsByRegisteredFunctionId = createSelector(
    [
      argsArray,
      (_: RootState, registeredFunctionId: string) => registeredFunctionId,
    ],
    (
      argsArray: ArgRecordWithKey[],
      registeredFunctionId: string,
    ): ArgRecordWithKey[] => {
      if (!argsArray || !registeredFunctionId) return [];
      return argsArray.filter(
        (arg) => arg.registeredFunction === registeredFunctionId,
      );
    },
  );
  ...
  const registeredFunctionWithArgs = createSelector(
    ...
    (functions, argsArray, registeredFunctionId) => {
      ...
      const functionArgs = argsArray.filter(
        (arg: ArgRecordWithKey) =>
          arg.registeredFunction === registeredFunctionId,
      );
  ...
  const allRegisteredFunctionsWithArgs = createSelector(
    ...
    (
      ...
      return functionsArray.map((func) => {
        const functionArgs = argsArray.filter(
          (arg) => arg.registeredFunction === func.id,
        );
        return {
          ...func,
          args: functionArgs,
  ...
  const activeRegisteredFunctionWithArgs = createSelector(
    ...
    (activeFunction, argsArray) => {
      ...
      const functionArgs = argsArray.filter(
        (arg: ArgRecordWithKey) => arg.registeredFunction === activeFunction.id,
      );
  ...
  const activeWorkflowFunctionSteps = createSelector(
    ...
    (steps: any[]) => {
      return steps.filter(
        (step) => step.function_type === "registered_function",
      );
    },
  );
  ...
  const activeWorkflowRecipeSteps = createSelector(
    ...
    (steps: any[]) => {
      return steps.filter(
        (step) => step.function_type === "workflow_recipe_executor",
      );
    },
  );
  ...
  const functionStepByFunctionId = createSelector(
    [
      ...
      (_: RootState, functionId: string) => functionId,
    ],
    ...
    (steps, functionsArray) => {
      return steps.map((step: any, index: number) => {
        ...
        if (step.function_type === "registered_function") {
          functionExists = functionsArray.some(
            (f) => f.id === step.function_id,
          );
          ...
          const validMethods = [
            "recipe_runner",
            "extractor",
            "iterative_recipe_preparer",
            "iterative_recipe_runner",
            "results_processor",
          ];
        ...
        return {
          ...
          errors: {
            ...
            functionNotFound:
              step.function_type === "registered_function" && !functionExists,
            invalidRecipeMethod:
              step.function_type === "workflow_recipe_executor" &&
              !hasValidFunction,
          },
        };
      });
  ...
  const completeStepData = createSelector(
    ...
    (steps, functionsArray, argsArray, stepIndex) => {
      ...
      const overrideMap = new Map(
        existingOverrides.map((o: any) => [o.name, o]),
      );
        ...
        functionArgs.forEach((arg) => {
          if (arg.required) {
            ...
            if (!isReady && !hasValue && !hasOldValue) {
              errors.push(
                `Required argument '${arg.name}' is not ready and has no value`,
              );
            }
          }
        });
        ...
        overrides.forEach((override: any) => {
          ...
          if (arg) {
            ...
            if (value !== null && value !== undefined) {
              ...
              if (!isValidType) {
                errors.push(
                  `Invalid type for '${override.name}': expected ${arg.dataType}`,
                );
              }
            }
          }
  ...
  const enrichBrokerReference = createSelector(
    ...
    (
      ...
      if (isPotentialRealBroker) {
        ...
        const realBroker = brokersArray.find(
  ...
  const validateBrokerArgCompatibility = createSelector(
    [
      dataBrokersArray,
      (_: RootState, brokerId: string, argType: string) => ({
        brokerId,
        argType,
      }),
    ],
    (brokersArray: DataBrokerRecordWithKey[], { brokerId, argType }) => {
      if (!brokerId || !argType)
        return { isValid: true, reason: "", isInformational: true };

      if (!isValidUUID(brokerId)) {
        // Consumable broker - assume valid (user responsibility)
        return {
          isValid: true,
          reason: "Consumable broker (type not validated)",
      ...
      if (
        compatibleTypes[broker.dataType]?.includes(argType) ||
        compatibleTypes[argType]?.includes(broker.dataType)
      ) {
        return {
          isValid: true,
          reason: "Compatible types (Python will handle conversion)",
          isInformational: true,
        };
      }
