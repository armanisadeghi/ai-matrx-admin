...
export const updateReducers = <TEntity extends EntityKeys>(
  ...
  const entityLogger = EntityLogger.createLoggerWithDefaults(
    "UPDATE REDUCERS",
  ...
  return {
    ...
    directUpdateRecordSuccess: (
      ...
      const recordKey = createRecordKey(
        state.entityMetadata.primaryKeyMetadata,
        action.payload,
      );
    ...
    updateRecord: (
      ...
      if (!unsavedData) {
        entityLogger.log(
          "error",
          "No unsaved changes found for record",
          matrxRecordId,
        );
        return;
      }
    ...
    optimisticUpdate: (
      ...
      const recordKey: MatrxRecordId = createRecordKey(
        state.entityMetadata.primaryKeyMetadata,
        record,
      );
    ...
    updateFieldSmart: (
      ...
      if (
        action.payload.keyOrId.includes(":") ||
        action.payload.keyOrId.includes("new-record-")
      ) {
        recordKey = action.payload.keyOrId;
      } else {
        recordKey = createRecordKey(state.entityMetadata.primaryKeyMetadata, {
          id: action.payload.keyOrId,
        });
      }
      ...
      if (!unsavedRecord) {
        ...
        if (existingRecord) {
          ...
          entityLogger.log(
            DEBUG,
            "Copied Existing Record to Unsaved",
            unsavedRecord,
          );
        } else {
          entityLogger.log(
            DEBUG,
            "Record Does Not Exist in Saved Or Unsaved Records. Exiting.",
            action.payload,
          );
    ...
    updateNestedFieldSmart: (
      ...
      entityLogger.log(DEBUG, "updateNestedFieldSmart", {
        keyOrId,
        field,
        nestedKey,
        value,
      ...
      entityLogger.log(
        DEBUG,
        `updateNestedFieldSmart ${isRecordKeyFormat ? "HAS" : "CREATED"} recordKey`,
        recordKey,
      );

      // Step 2: Ensure unsaved record exists using utility
      const { unsavedRecord, didExitEarly } = ensureUnsavedRecord(
        state,
        recordKey,
        action.payload,
        entityLogger,
      ...
      entityLogger.log(
        DEBUG,
        "updateNestedFieldSmart UNSAVED RECORD",
        unsavedRecord,
      );
      ...
      if (!unsavedRecord[field] || typeof unsavedRecord[field] !== "object") {
        ...
        entityLogger.log(
          DEBUG,
          `Initialized empty ${field} object for record`,
          { recordKey },
        );
      }
      ...
      entityLogger.log(
        DEBUG,
        "updateNestedFieldSmart CURRENT NESTED VALUE",
        currentNestedValue,
      );
      ...
      entityLogger.log(
        DEBUG,
        "Record State After Nested Update: ",
        state.unsavedRecords[recordKey],
      );
    },
    ...
    updateMultipleNestedFieldsSmart: (
      ...
      entityLogger.log(DEBUG, "updateMultipleNestedFieldsSmart", {
        keyOrId,
        updates,
      });
      ...
      if (hasChanges) {
        entityLogger.log(
          DEBUG,
          "Updating multiple nested fields in unsaved record",
        );
        state.unsavedRecords[recordKey] = {
          ...unsavedRecord,
          ...updatedFields,
