...
export function useOneRelationship(params: UseOneRelationshipParams) {
  ...
  const { selectors: parentSelectors, actions: parentActions } =
    useEntityTools(parentEntity);
  const { selectors: childSelectors, actions: childActions } =
    useEntityTools(childEntity);
  ...
  const activeParentRecordKey = useAppSelector(
    parentSelectors.selectActiveRecordId,
  );
  ...
  const matchingChildRecords = useAppSelector((state: RootState) =>
    childSelectors.selectRecordsByFieldValue(
      state,
      childReferenceField,
      activeParentId,
    ),
  ...
  const matchingChildRecordsCount = useAppSelector((state: RootState) =>
    childSelectors.selectRecordCountByFieldValue(
      state,
      childReferenceField,
      activeParentId,
    ),
    ...
    return new Promise<void>((resolve) => {
      ...
      const unsubscribe = setInterval(() => {
        ...
        if (iterationCount % 10 === 0) {
          console.warn(
            `waitForLoading still running after ${iterationCount} checks (${iterationCount * 100}ms)`,
          );
        }
  ...
  const setActiveParent = useCallback(
    async (recordKey: MatrxRecordId) => {
      ...
      if (isPermanentKey) {
        ...
        console.log(
          "⚠️ useOneRelationship: Setting active parent to a permanent record, but not fetching anything.",
          recordKey,
        );
        dispatch(parentActions.setActiveRecord(recordKey));
      } else if (isTempkey) {
        console.log(
          "⚠️ useOneRelationship: Setting active parent to a temporary record, but not fetching anything.",
          recordKey,
        );
        await waitForLoading();
        dispatch(parentActions.setActiveRecord(recordKey));
        ...
        dispatch(parentActions.setActiveRecord(recordKey));
        console.warn(
          "useOneRelationship: The record key used is not a valid key. Please check.",
          recordKey,
        );
      }
    },
