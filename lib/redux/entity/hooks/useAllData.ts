...
export const useEntityState = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey] as EntityState<TEntity>,
  );
};
...
export const useDataBrokerState = () => {
  return useAppSelector(
    (state: RootState) =>
      state.entities["dataBroker"] as EntityState<"dataBroker">,
  );
};

export const useEntityRecords = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey].records,
  );
};

export const useEntityUnsavedRecords = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey].unsavedRecords,
  );
};

export const useEntitySelectedRecordIds = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey].selection.selectedRecords,
  );
};

export const useEntityLoadingState = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  const loadingState = useAppSelector(
    (state: RootState) => state.entities[entityKey].loading,
  );
...
export const useEntityQuickRefRecords = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey].quickReference.records,
  );
};

export const useEntityMetadata = <TEntity extends EntityKeys>(
  entityKey: TEntity,
) => {
  return useAppSelector(
    (state: RootState) => state.entities[entityKey].entityMetadata,
  );
};
...
export const useEntitySelectors = <TEntity extends EntityKeys>(
  entityKey: TEntity,
): UseEntitySelectorsReturn<TEntity> => {
  ...
  const pkValueToMatrxId = useCallback(
    (pkValue: string) => {
      if (pkType === "composite") {
        console.error(
          "This Entity has a composite primary key. Use pkValuesToMatrxId instead.",
        );
      }
  ...
  const pkValuesToMatrxId = useCallback(
    (pkValues: Record<string, unknown>) => createRecordKey(pkMeta, pkValues),
    [pkMeta],
  );

  const matrxIdToPks = useCallback(
    (recordId: MatrxRecordId) =>
      parseRecordKey(recordId) as Record<AllEntityFieldKeys, unknown>,
    [],
  );
...
export const useEntityWithFetch = <TEntity extends EntityKeys>(
  entityName: TEntity,
) => {
...
export type UseEntityWithFetchReturn<TEntity extends EntityKeys> = ReturnType<
  typeof useEntityWithFetch<TEntity>
>;
...
export type UseDataInputComponentsWithFetchReturn = {
  ...
  dataInputComponentUnsavedRecords: Record<
    MatrxRecordId,
    Partial<DataInputComponentData>
  >;
...
export const useDataInputComponentsWithFetch =
  (): UseDataInputComponentsWithFetchReturn => {
    const {
...
type UseInputComponentsReturn = {
  ...
  inputComponentUnsavedRecords: Record<
    MatrxRecordId,
    Partial<DataInputComponentData>
  >;
...
type UseOutputComponentsReturn = {
  ...
  outputComponentUnsavedRecords: Record<
    MatrxRecordId,
    Partial<DataOutputComponentData>
  >;
...
type UseCompiledRecipesReturn = {
  ...
  compiledRecipeUnsavedRecords: Record<
    MatrxRecordId,
    Partial<CompiledRecipeData>
  >;
...
export const useCompiledRecipesWithFetch =
  (): UseCompiledRecipesWithFetchReturn => {
    const {
      selectors: compiledRecipeSelectors,
      actions: compiledRecipeActions,
      allRecords: compiledRecipeRecords,
...
type UseAiModelEndpointWithFetchReturn = {
  ...
  aiModelEndpointUnsavedRecords: Record<
    MatrxRecordId,
    Partial<AiModelEndpointData>
  >;
...
export const useAiModelEndpointWithFetch =
  (): UseAiModelEndpointWithFetchReturn => {
    const {
...
type UseRegisteredFunctionWithFetchReturn = {
  ...
  registeredFunctionUnsavedRecords: Record<
    MatrxRecordId,
    Partial<RegisteredFunctionData>
  >;
...
export const useRegisteredFunctionWithFetch =
  (): UseRegisteredFunctionWithFetchReturn => {
    const {
