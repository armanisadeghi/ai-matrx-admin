...
export const useEntityAnalyzer = (initialEntityKey?: EntityKeys | null) => {
  const [selectedEntityKey, setSelectedEntityKey] = useState<EntityKeys | null>(
    initialEntityKey ?? null,
  );
  const [isViewingEntity, setIsViewingEntity] =
    useState<boolean>(!!initialEntityKey);
  ...
  const getEntitySections = (): EntityAnalyzerState["sections"] => {
    ...
    const stateMap: Record<string, EntityStateSection> = {
      ...
      unsavedRecords: {
        ...
        data: entityState.unsavedRecords,
      },
      records: { title: "Records", data: entityState.records },
      parentInfo: {
        title: "Parent Info",
        data: {
          parentEntityField: entityState.parentEntityField,
          activeParentId: entityState.activeParentId,
        },
      },
      ...
      runtimeFilters: {
        ...
        data: entityState.runtimeFilters,
      },
      ...
      pendingOperations: {
        ...
        data: entityState.pendingOperations,
      },
      entityMetadata: {
