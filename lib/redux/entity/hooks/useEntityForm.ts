...
export function useEntityForm<TEntity extends EntityKeys>(
  ...
  const handleSave = React.useCallback(
    async (tempRecordId: MatrxRecordId) => {
      //TODO: This should already know what record we're working with. (I think it used to)
  ...
  // Utility functions
  const isFieldReadOnly = React.useCallback(
    (fieldName: string) => {
  ...
  const createRecord = React.useCallback(
    (tempRecordId: MatrxRecordId, callback?: Callback) => {
