...
const DEFAULT_ARG_RUNTIME_SORT: RuntimeSort = {
  field: "name",
  direction: "asc",
};
 ...
 */
function mergeArgsWithOverrides(
  functionArgs: any[],
  existingOverrides: ArgOverride[] = [],
): ArgOverride[] {
  ...
  return functionArgs.map((arg) => {
    ...
    return {
      ...
      value:
        ...
        convertPythonType(defaultValue, arg.dataType),
 ...
 */
function createStepFromFunctionData(
  ...
  return {
    ...
    override_data: {
      ...
      return_broker_override:
        existingStep.override_data?.return_broker_override,
...
export const getWorkflowActionsWithThunks = () => {
  ...
  const fetchBrokerIfNeeded =
    ...
    (dispatch: AppDispatch, getState: () => RootState) => {
      ...
      const currentBroker =
        getState().entities["dataBroker"].records[`id:${brokerId}`];
      if (!currentBroker) {
        // Real broker ID but not loaded, fetch it
        dispatch(
          dataBrokerActions.fetchOne({ matrxRecordId: `id:${brokerId}` }),
        );
      }
    };
  ...
  return {
    ...
    initialize: () => (dispatch: AppDispatch) => {
      ...
      console.log(
        "WORKFLOW ACTIONS: Dispatched initialization actions (brokers fetched on-demand).",
      );
    },
    ...
    createStepFromRegisteredFunction:
      (params: { functionId: string; stepName?: string }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const state = getState();

        // Get function data
        const registeredFunction =
          state.entities["registeredFunction"].records[
        ...
        const functionArgs = Object.values(
          state.entities["arg"].records,
        ).filter((arg: any) => arg.registeredFunction === params.functionId);

        // Create the step
        const newStep = createStepFromFunctionData(
          registeredFunction,
          functionArgs,
        );
    ...
    addStepFromRegisteredFunction:
      (params: { functionId: string; stepName?: string; stepIndex?: number }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const state = getState();
        ...
        // Add to workflow
        const currentWorkflow = state.entities["workflow"].records[
          keyOrId
        ] as any;
        if (!currentWorkflow) return;

        const currentBackendData = currentWorkflow.backendWorkflow || {
          steps: [],
        };
        const currentSteps = Array.isArray(currentBackendData.steps)
          ? [...currentBackendData.steps]
          : [];
        ...
        if (
          ...
          params.stepIndex >= 0 &&
          params.stepIndex <= currentSteps.length
        ) {
        ...
        dispatch(
          workflowActions.updateFieldSmart({
            keyOrId,
    ...
    updateStepArgOverride:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const currentWorkflow = getState().entities["workflow"].records[
          keyOrId
        ] as any;
        if (!currentWorkflow) return;
    ...
    refreshStepWithFunctionData:
      (params: { stepIndex: number }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        console.log(
          "🔍 refreshStepWithFunctionData called - this might be corrupting data!",
          params,
        );
        const state = getState();
        const keyOrId = state.entities["workflow"].selection.activeRecord;
        ...
        if (params.stepIndex >= 0 && params.stepIndex < currentSteps.length) {
          ...
          if (existingStep.function_type === "registered_function") {
            ...
            if (registeredFunction) {
              ...
              const updatedStep = updateStepWithFunctionData(
                existingStep,
                registeredFunction,
                functionArgs,
              );
    ...
    fetchArgsForRegisteredFunction:
      (params: { registeredFunctionId: string }) => (dispatch: AppDispatch) => {
        dispatch(
    ...
    setActiveRegisteredFunction:
      (registeredFunctionId: string) => (dispatch: AppDispatch) => {
    ...
    updateWorkflowEntityCustomData:
      (params: { customData: Record<string, unknown> }) =>
      (dispatch: AppDispatch) => {
        dispatch(
          workflowActions.updateCustomDataSmart({
    ...
    updateWorkflowFieldSmart:
      (params: { keyOrId?: string; field: string; value: any }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId =
          params.keyOrId ??
          getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
    ...
    updateWorkflowNestedFieldSmart:
      (params: {
        keyOrId?: string;
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!keyOrId) return;
        dispatch(
          workflowActions.updateNestedFieldSmart({ keyOrId, ...params }),
        );
      },

    // Update active workflow visual workflow data
    updateActiveWorkflowVisualWorkflow:
      (params: { visualWorkflowData: Record<string, unknown> }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId = getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
        dispatch(
    ...
    updateActiveWorkflowBackendWorkflow:
      (params: { backendWorkflowData: Record<string, unknown> }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId = getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
        dispatch(
    ...
    directUpdateWorkflow:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatch(
          workflowActions.directUpdateRecord({
            matrxRecordId,
            data: params.data,
    ...
    addWorkflowStep:
      (params: { step: any; stepIndex?: number }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId = getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
    ...
    updateWorkflowStep:
      (params: { stepIndex: number; step: any }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId = getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
    ...
    moveWorkflowStep:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (
          params.fromIndex >= 0 &&
          params.fromIndex < currentSteps.length &&
          params.toIndex >= 0 &&
          params.toIndex < currentSteps.length
        ) {
    ...
    updateWorkflowMetadata:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatch(
          workflowActions.updateNestedFieldSmart({
            keyOrId,
    ...
    updateWorkflowUserInputs:
      (params: { userInputs: any[] }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId = getState().entities["workflow"].selection.activeRecord;
        if (!keyOrId) return;
    ...
    getEnrichedStepData:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const state = getState();
        const workflow = state.entities["workflow"].records[
          state.entities["workflow"].selection.activeRecord
        ] as any;
        ...
        if (step.function_type === "registered_function") {
          ...
          if (!func) {
            dispatch(
              registeredFunctionActions.fetchOne({
                matrxRecordId: `id:${step.function_id}`,
              }),
            );
          }

          // Ensure we have the args data for this function
          const existingArgs = Object.values(
            state.entities["arg"].records,
          ).filter((arg: any) => arg.registeredFunction === step.function_id);
    ...
    fetchBrokersForStep:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (step.override_data?.return_broker_override) {
          dispatch(
            fetchBrokerIfNeeded(step.override_data.return_broker_override),
          );
        }
      },
