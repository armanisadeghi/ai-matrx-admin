...
export const getChatActionsWithThunks = () => {
  ...
  return {
    ...
    initialize: () => (dispatch: AppDispatch) => {
      ...
      dispatch(
        conversationActions.fetchRecords({
          page: 1,
      ...
      dispatch(
        messageActions.setRuntimeFilters(DEFAULT_MESSAGE_RUNTIME_FILTERS),
      );
    ...
    updateConversationCustomData:
      (params: { keyOrId: string; customData: Record<string, unknown> }) =>
      (dispatch: AppDispatch) => {
        dispatch(conversationActions.updateCustomDataSmart(params));
      },

    updateMessageCustomData:
      (params: { keyOrId: string; customData: Record<string, unknown> }) =>
      (dispatch: AppDispatch) => {
    ...
    updateNextOrderData:
      (params: {
        keyOrId?: string;
        nextDisplayOrderToUse?: number;
        nextSystemOrderToUse?: number;
        isNewChat?: boolean;
      }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const activeMessageRecordKey =
          getState().entities["message"].selection.activeRecord;
        const activeMessageRecord = getState().entities["message"].records[
          activeMessageRecordKey
        ] as Message;
        const nextDisplayOrder =
          params.nextDisplayOrderToUse ?? activeMessageRecord.displayOrder + 2;
        const nextSystemOrder =
          params.nextSystemOrderToUse ?? activeMessageRecord.systemOrder + 2;

        if (!convKeyOrId || !nextDisplayOrder || !nextSystemOrder) {
          console.warn(
            "UPDATE NEXT ORDER DATA: Missing required parameters. Received Params:",
            params,
          );
          console.log(
    ...
    fetchAdditionalConversations:
      (page: number, pageSize: number) => (dispatch: AppDispatch) => {
    ...
    fetchRelatedMessageForConversation:
      (params: { conversationId: string }) => (dispatch: AppDispatch) => {
        dispatch(
    ...
    fetchRelatedRecordsForActiveConversation:
      () => (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    saveActiveUnsavedMessage:
      (params: { messageTempId?: MatrxRecordId }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const activeMessageRecordKey =
          params.messageTempId ??
          getState().entities["message"].selection.activeRecord;
        if (!activeMessageRecordKey) return;
        dispatch(saveMessageThunk({ messageTempId: activeMessageRecordKey }));
      },

    setExternalConversationLoading:
      (loading: boolean) => (dispatch: AppDispatch) => {
        dispatch(conversationActions.setExternalLoading(loading));
      },

    setExternalMessageLoading:
      (loading: boolean) => (dispatch: AppDispatch) => {
    ...
    setActiveConversation:
      (conversationId: string) => (dispatch: AppDispatch) => {
        ...
        dispatch(
          messageActions.addRuntimeFilter({
            field: parentEntityField,
            operator: "eq",
            value: matrxRecordId,
          }),
        );
    ...
    coordinateActiveConversationAndMessageFetch:
      (conversationId: string) => (dispatch: AppDispatch) => {
        const matrxRecordId = `id:${conversationId}`;
    ...
    fetchMessagesForActiveConversation:
      () => (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!activeConversationKey) {
          console.warn(
            "FETCH MESSAGES FOR ACTIVE CONVERSATION: No active conversation found",
          );
          return;
        }
        const activeConversation = getState().entities["conversation"].records[
        ...
        dispatch(
          fetchRelatedMessagesThunk({ conversationId: activeConversationId }),
        );
      },
    ...
    createConversationAndMessage:
      (params: { messageContent?: string }) => (dispatch: AppDispatch) => {
    ...
    saveConversationAndMessage:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        return dispatch(
          saveConversationAndMessage({
            conversationTempId: convKeyOrId,
            messageTempId: msgKeyOrId,
    ...
    updateMessageContent:
      (params: { keyOrId?: string; value: string }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId =
          params.keyOrId ??
          getState().entities["message"].selection.activeRecord;
        if (!keyOrId) {
          console.warn(
            "CHAT ACTIONS THUNK - UPDATE MESSAGE CONTENT: Key or Id was not found",
          );
          return;
        }
        dispatch(
          messageActions.updateFieldSmart({
            keyOrId,
            field: "content",
            value: params.value,
          }),
    ...
    updateNestedConversationFieldSmart:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!keyOrId) return;
        dispatch(
          conversationActions.updateNestedFieldSmart({ keyOrId, ...params }),
        );
      },
    ...
    updateModel:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        const convKeyOrId =
          params.conversationkeyOrId ??
          getState().entities["conversation"].selection.activeRecord;
        const msgKeyOrId =
          params.messagekeyOrId ??
          getState().entities["message"].selection.activeRecord;
        if (!convKeyOrId || !msgKeyOrId) return;
        dispatch(
    ...
    updateMessageStatus:
      (params: { messagekeyOrId?: string; status: MessageStatusOptions }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const msgKeyOrId =
          params.messagekeyOrId ??
          getState().entities["message"].selection.activeRecord;
    ...
    updateModUserContext:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const msgKeyOrId =
          ...
          getState().entities["message"].selection.activeRecord;
        console.log(
          "[CHAT ACTIONS THUNK] updating ModUserContext",
          params.value,
        );
    ...
    updateMultipleNestedFields:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!convKeyOrId || !msgKeyOrId) return;
        dispatch(
          conversationActions.updateMultipleNestedFieldsSmart({
            keyOrId: convKeyOrId,
            updates: params.updates,
          }),
        );
        dispatch(
          messageActions.updateMultipleNestedFieldsSmart({
            keyOrId: msgKeyOrId,
            updates: params.updates,
          }),
        );
    ...
    updateFiles:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (params.value.length > 0) {
          ...
          ); // NEWLY ADDED! Might break things!
          console.log(
            "--> Warning! New feature. [CHAT ACTIONS THUNK] updated files is changing the message type to mixed",
          );
        }
      },

    setSocketEventName:
      (params: { eventName: string }) => (dispatch: AppDispatch) => {
        dispatch(
          conversationActions.setSocketEventName({
            socketEventName: params.eventName,
          }),
        );
        dispatch(
          messageActions.setSocketEventName({
            socketEventName: params.eventName,
          }),
        );
      },
    ...
    setIsStreaming: () => (dispatch: AppDispatch) => {
      ...
      dispatch(
        conversationActions.updateCustomDataSmart({
          customData: { isStreaming: true },
        }),
      );
      dispatch(
        messageActions.updateCustomDataSmart({
          customData: { isStreaming: true },
        }),
      );
    },
    ...
    setChatDebugMode:
      (params: { isDebugMode: boolean }) => (dispatch: AppDispatch) => {
        dispatch(
          conversationActions.updateCustomDataSmart({
            customData: { isDebugMode: params.isDebugMode },
          }),
        );
      },
    ...
    updateConversationMetadataFieldSmart:
      (params: { keyOrId?: string; nestedKey: string; value: any }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const keyOrId =
          params.keyOrId ??
          getState().entities["conversation"].selection.activeRecord;
    ...
    directUpdateConversation:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatch(
          conversationActions.directUpdateRecord({ matrxRecordId, ...params }),
        );
      },

    directUpdateConversationLabel:
      (params: { matrxRecordId?: MatrxRecordId; label: string }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const matrxRecordId =
          params.matrxRecordId ??
          getState().entities["conversation"].selection.activeRecord;
        if (!matrxRecordId) return;
        dispatch(
          conversationActions.directUpdateRecord({
            matrxRecordId,
    ...
    directUpdateConversationIsPublic:
      (params: { matrxRecordId?: MatrxRecordId; isPublic: boolean }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const matrxRecordId =
          params.matrxRecordId ??
          getState().entities["conversation"].selection.activeRecord;
    ...
    directUpdateConversationMetadata:
      (params: { matrxRecordId?: MatrxRecordId; metadata: any }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const matrxRecordId =
          params.matrxRecordId ??
          getState().entities["conversation"].selection.activeRecord;
    ...
    directUpdateMessage:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!matrxRecordId) return;
        dispatch(
          messageActions.directUpdateRecord({ matrxRecordId, ...params }),
        );
      },
    ...
    updateConversation:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!matrxRecordId) return;
        dispatch(
          conversationActions.updateRecord({ matrxRecordId, ...params }),
        );
      },
    ...
    updateUnsavedConversationField:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!recordId) return;
        dispatch(
          conversationActions.updateUnsavedField({ recordId, ...params }),
        );
      },
    ...
    updateUnsavedConversationFields:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatch(
          conversationActions.updateUnsavedFields({
            updates: updatesWithRecordId,
          }),
        );
      },
    ...
    updateUnsavedMessageFields:
      ...
      (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (!updatesWithRecordId.every((update) => update.recordId)) return;
        dispatch(
          messageActions.updateUnsavedFields({ updates: updatesWithRecordId }),
        );
      },
    ...
    startMessageUpdateById:
      (recordId?: MatrxRecordId) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const id =
          recordId ?? getState().entities["message"].selection.activeRecord;
        if (!id) return;
        dispatch(messageActions.startRecordUpdateById(id));
      },

    deleteConversation:
      (params: { matrxRecordId?: MatrxRecordId }) =>
      (dispatch: AppDispatch, getState: () => RootState) => {
        const matrxRecordId =
          params.matrxRecordId ??
          getState().entities["conversation"].selection.activeRecord;
...
export const getChatActions = (dispatch: AppDispatch) => {
  ...
  return {
    ...
    setActiveConversation: (conversationId: string) => {
      dispatch(
        conversationActions.fetchOneWithFkIfk({
          matrxRecordId: `id:${conversationId}`,
        }),
      );
    ...
    createConversationAndMessage: (params: { messageContent: string }) =>
      dispatch(
        createConversationAndMessage({ messageContent: params.messageContent }),
      ),

    saveConversationAndMessage: (params: {
      conversationTempId: MatrxRecordId;
      messageTempId: MatrxRecordId;
      taskId: string;
    }) =>
      dispatch(
        saveConversationAndMessage({
          conversationTempId: params.conversationTempId,
          messageTempId: params.messageTempId,
          taskId: params.taskId,
        }),
      ),
    ...
    updateConversationFieldSmart: (params: {
      keyOrId: string;
      field: string;
      value: any;
    }) => dispatch(conversationActions.updateFieldSmart(params)),

    updateMessageFieldSmart: (params: {
      keyOrId: string;
      field: string;
      value: any;
    ...
    updateNestedMessageFieldSmart: (params: {
      keyOrId: string;
      field: EntityFieldKeys<"message">;
      nestedKey: string;
      value: any;
    ...
    updateModAssistantContext: (params: {
      conversationkeyOrId: string;
      messagekeyOrId: string;
      value: string;
    }) => {
    ...
    updateConversationMetadataFieldSmart: (params: {
      keyOrId: string;
      nestedKey: string;
      value: any;
    }) =>
    ...
    updateMessageMetadataFieldSmart: (params: {
      keyOrId: string;
      nestedKey: string;
      value: any;
    }) =>
    ...
    directUpdateConversation: (params: {
      matrxRecordId: MatrxRecordId;
      data: Record<string, any>;
      callbackId?: string;
    }) => dispatch(conversationActions.directUpdateRecord(params)),

    directUpdateConversationLabel: (params: {
      matrxRecordId: MatrxRecordId;
      label: string;
    }) =>
      dispatch(
        conversationActions.directUpdateRecord({
          matrxRecordId: params.matrxRecordId,
          data: { label: params.label },
        }),
      ),

    directUpdateConversationIsPublic: (params: {
      matrxRecordId: MatrxRecordId;
      isPublic: boolean;
    }) =>
      dispatch(
    ...
    directUpdateConversationMetadata: (params: {
      matrxRecordId: MatrxRecordId;
      metadata: any;
    }) =>
      dispatch(
    ...
    updateUnsavedConversationField: (params: {
      recordId: MatrxRecordId;
      field: string;
      value: any;
    }) => dispatch(conversationActions.updateUnsavedField(params)),

    updateUnsavedMessageField: (params: {
      recordId: MatrxRecordId;
      field: string;
      value: any;
    }) => dispatch(messageActions.updateUnsavedField(params)),

    updateUnsavedConversationFields: (params: {
      updates: Array<{ recordId: MatrxRecordId; field: string; value: any }>;
    }) => dispatch(conversationActions.updateUnsavedFields(params)),

    updateUnsavedMessageFields: (params: {
      updates: Array<{ recordId: MatrxRecordId; field: string; value: any }>;
    }) => dispatch(messageActions.updateUnsavedFields(params)),

    startConversationUpdate: () =>
      dispatch(conversationActions.startRecordUpdate()),
    startMessageUpdate: () => dispatch(messageActions.startRecordUpdate()),

    startConversationUpdateById: (recordId: MatrxRecordId) =>
      dispatch(conversationActions.startRecordUpdateById(recordId)),
    startMessageUpdateById: (recordId: MatrxRecordId) =>
      dispatch(messageActions.startRecordUpdateById(recordId)),
