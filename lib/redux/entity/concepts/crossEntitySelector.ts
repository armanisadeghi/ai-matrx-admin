 ...
 */
export const createCrossEntitySelectorWithRelationships = (
  primaryEntityKey: EntityKeys,
) => {
  const { selectEntity: selectPrimaryEntity } =
    createEntitySelectors(primaryEntityKey);
  ...
  return createSelector(
    ...
    (
      primaryEntityState: EntityState<any>,
      globalSchema: AutomationEntities,
      entities,
    ) => {
      ...
      relationships.forEach((rel: Relationship) => {
        ...
        const { selectEntity: selectRelatedEntity } =
          createEntitySelectors(relatedEntityKey);

        const relatedEntityState: EntityState<any> = selectRelatedEntity({
          entities,
        } as RootState);
        ...
        if (rel.relationshipType === "foreignKey") {
          ...
          combinedData = combinedData.map((primaryItem) => ({
            ...primaryItem,
            [relatedEntityKey]:
              foreignKeyMap.get(primaryItem[rel.column]) || [],
          }));
