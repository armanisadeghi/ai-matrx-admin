...
function* handleGetOrFetchSelectedRecords<TEntity extends EntityKeys>(
    ...
    try {
        ...
        for (const recordId of existingRecords) {
            entityLogger.log('info', '-Existing records', existingRecords);
            entityLogger.log('info', 'Adding to selection', recordId);
        ...
        if (primaryKeysToFetch.length > 0) {
            ...
            entityLogger.log('info', 'Query options', queryOptions);
...
function* handleFetchQuickReference<TEntity extends EntityKeys>(
    ...
    try {
        ...
        entityLogger.log('info', 'Database response', {data, count});
...
function* handleBatchOperation<TEntity extends EntityKeys>(
    ...
    try {
        ...
        entityLogger.log('info', 'Starting', {operation, batchSize, records});
        ...
        entityLogger.log('info', 'Database response', {results});
...
function* handleQuickReferenceUpdate<TEntity extends EntityKeys>(
    ...
    entityLogger.log('info', 'Starting', record);
    ...
    entityLogger.log('info', 'Final result', quickReferenceRecord);
...
function* handleFetchMetrics<TEntity extends EntityKeys>(
    ...
    try {
        ...
        entityLogger.log('info', 'Final result', {operationCounts, performanceMetrics, cacheStats, errorRates});
...
function* handleCreateFromSocket<TEntity extends EntityKeys>(
    ...
    try {
        ...
        sagaLogger.log('info', `Handled create from socket for ${entityKey}`, data);
    } catch (error: any) {
...
function* handleUpdateFromSocket<TEntity extends EntityKeys>(
    ...
    try {
        ...
        sagaLogger.log('info', `Handled update from socket for ${entityKey}`, data);
    } catch (error: any) {
...
function* handleDeleteFromSocket<TEntity extends EntityKeys>(
    ...
    try {
        ...
        sagaLogger.log('info', `Handled delete from socket for ${entityKey}`, data);
    } catch (error: any) {
