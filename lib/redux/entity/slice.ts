...
export const createEntitySlice = <TEntity extends EntityKeys>(
  ...
  const slice = createSlice({
    ...
    extraReducers: (builder) => {
      const p = GET_OR_FETCH_SELECTED_RECORDS_THUNK;
      builder
        .addMatcher(
          (a): a is { type: string } => a.type === `${p}/pending`,
          (state) => {
        ...
        .addMatcher(
          (a): a is { type: string } => a.type === `${p}/fulfilled`,
          (state) => {
        ...
        .addMatcher(
          ...
          (state, action) => {
            entityLogger.log(
              "error",
              "getOrFetchSelectedRecordsThunk rejected",
              action.error,
            );
            setError(state, {
              payload: {
                message:
                  ...
                  "An error occurred during fetch by record IDs.",
...
export type EntitySlice<TEntity extends EntityKeys> = ReturnType<
  typeof createEntitySlice<TEntity>
>;
export type EntityActions<TEntity extends EntityKeys> =
  EntitySlice<TEntity>["actions"];
