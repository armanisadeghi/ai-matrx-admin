...
import { createEntitySlice } from "@/lib/redux/tables/entitySliceCreator";
import {
    ...
    return function* saga() {
        yield all([
            ...
            takeLatest(
                ...
                handleFetchByPrimaryKey.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_FETCH_BY_FIELD_REQUEST`,
                handleFetchByField.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_FETCH_SIMPLE_REQUEST`,
                handleFetchSimple.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_SUBSCRIBE`,
                handleSubscribeToChanges.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_UNSUBSCRIBE`,
                handleUnsubscribeFromChanges.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_FETCH_ALL_REQUEST`,
                handleFetchAll.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_FETCH_PK_AND_DISPLAY_FIELDS_REQUEST`,
                handleFetchPkAndDisplayFields.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_CREATE_BACKUP_REQUEST`,
                handleCreateBackup.bind(null, entityVariant, client, actions)
            ),
            takeLatest(
                `${baseType}_RESTORE_BACKUP_REQUEST`,
                handleRestoreBackup.bind(null, entityVariant, client, actions)
            ),
        ]);
    };
}
