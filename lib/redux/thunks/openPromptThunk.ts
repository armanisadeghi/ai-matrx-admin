...
>("promptRunner/openPrompt", async (config, { dispatch, getState }) => {
  try {
    ...
    if (config.promptId) {
      ...
      if (fetchStatus === "loading") {
        console.warn(
          `[openPrompt] Prompt ${config.promptId} is already being fetched`,
        );
        return;
      }
      ...
      // Fetch from database
      dispatch(
        setFetchStatus({ promptId: config.promptId, status: "loading" }),
      );
      ...
      if (error || !prompt) {
        dispatch(
          setFetchStatus({ promptId: config.promptId, status: "error" }),
        );
        console.error(
          `[openPrompt] Failed to fetch prompt ${config.promptId}:`,
          error,
        );
    ...
    throw new Error(
      "Either promptId or promptData must be provided to openPrompt",
    );
  } catch (error) {
...
>("promptCache/prefetchPrompt", async (promptId, { dispatch, getState }) => {
  try {
    const state = getState();
    ...
    if (error || !prompt) {
      ...
      console.error(
        `[prefetchPrompt] Failed to fetch prompt ${promptId}:`,
        error,
      );
      return;
    }
