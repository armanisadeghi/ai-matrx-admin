...
export const selectActiveWorkflow = createSelector(
  [selectWorkflowState],
  (workflowState) =>
    workflowState.activeId
      ? workflowState.entities[workflowState.activeId]
      : null,
);
...
export const selectSelectedWorkflows = createSelector(
  [selectWorkflowState],
  (workflowState) => {
    const selectedWorkflows = workflowState.selectedIds
      .map((id) => workflowState.entities[id])
      .filter(Boolean);
...
export const selectWorkflowsDataFreshness = createSelector(
  ...
  (workflowState) => ({
    ...
    isStale: workflowState.fetchTimestamp
      ? Date.now() - workflowState.fetchTimestamp > 5 * 60 * 1000
      : true, // 5 minutes
  }),
);
...
export const selectActiveWorkflows = createSelector(
  [selectAllWorkflows],
  (workflows) =>
    workflows.filter((workflow) => workflow.is_active && !workflow.is_deleted),
);
...
export const selectActiveWorkflowWithNodes = createSelector(
  ...
  (activeWorkflow, allNodes) => {
    ...
    const workflowNodes = allNodes.filter(
      (node) => node.workflow_id === activeWorkflow.id,
    );
    return {
      ...activeWorkflow,
      nodes: workflowNodes,
...
export const selectWorkflowUserDataSourceByBrokerId = createSelector(
  ...
  (userDataSources, brokerId) => {
    ...
    return userDataSources.find(
      (source) => source.brokerId === brokerId,
    ) as BrokerSourceConfig<"user_data"> | null;
  },
);
...
export const selectWorkflowUserInputSourceByBrokerId = createSelector(
  ...
  (userInputSources, brokerId) => {
    ...
    return userInputSources.find(
      (source) => source.brokerId === brokerId,
    ) as BrokerSourceConfig<"user_input"> | null;
  },
);
