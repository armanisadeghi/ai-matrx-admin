...
} from "@/features/prompt-actions/types/promptActionTypes";
...
>("promptExecution/startAction", async (payload, { dispatch, getState }) => {
  const {
    actionId,
    context,
    userProvidedVariables = {},
  ...
  try {
    ...
    Object.entries(action.broker_mappings).forEach(([varName, brokerId]) => {
      if (
        brokerValues[brokerId] !== undefined &&
        brokerValues[brokerId] !== null
      ) {
        ...
        console.log(
          `🔗 Mapped broker "${brokerId}" → variable "${varName}":`,
          resolvedVariables[varName],
        );
      }
    });
      ...
      startPromptInstance({
        ...
        executionConfig: {
          ...
          use_pre_execution_input:
            action.execution_config.use_pre_execution_input,
        },
        initialMessage,
        runId,
      }),
