...
export const createFieldFromBroker = async (
  brokerName: string,
  brokerId: string,
  fieldComponentId: string | undefined,
  ...
  if (fieldComponentId) {
    ...
    console.warn(
      `Field component with ID ${fieldComponentId} not found for broker ${brokerName}. Creating new field.`,
    );
  }

  // Create a new field component if no existing one found
  const field = normalizeFieldDefinitionWithUuid({
    component: "textarea",
    label: brokerName,
    description: "Auto-generated for " + brokerName,
  });
  const result = await dispatch(createFieldThunk(field)).unwrap();
  return result;
};

export const createContainerFromRecipe = async (
  recipeName: string,
  neededBrokers: NeededBroker[],
  dispatch: AppDispatch,
) => {
  const fields = await Promise.all(
    neededBrokers.map((broker) =>
      createFieldFromBroker(
        broker.name,
        broker.id,
        broker.fieldComponentId,
        dispatch,
      ),
    ),
  );
  ...
  const createdContainer = await dispatch(
    createContainerThunk(container),
  ).unwrap();
  ...
  const { getComponentGroupById } =
    await import("../service/fieldContainerService");
...
export const createAppletFromRecipe = async (
  recipeId: string,
  recipeName: string,
  dispatch: AppDispatch,
) => {
  const recipeConfig =
    await getCompiledRecipeByVersionWithNeededBrokers(recipeId);
  ...
  const { container, fieldToBrokerMappings } = await createContainerFromRecipe(
    recipeName,
    neededBrokers,
    dispatch,
  );
  ...
  const brokerMappings: BrokerMapping[] = fieldToBrokerMappings.map(
    (mapping) => ({
      appletId,
...
export const updateAppletFromRecipe = async (
  appletId: string,
  recipeId: string,
  dispatch: AppDispatch,
  getState: () => RootState,
