...
export const selectDbFunctionNodeState = (state: RootState) =>
  state.dbFunctionNode;
...
export const selectAllNodesArray = createSelector([selectAllNodes], (nodes) =>
  Object.values(nodes),
);

/** Get all node IDs */
export const selectAllNodeIds = createSelector([selectAllNodes], (nodes) =>
  Object.keys(nodes),
);
...
export const selectNodeAdditionalDependencies = createSelector(
  [selectNodeById],
  (node): WorkflowDependency[] =>
    asWorkflowDependencies(node?.additional_dependencies),
);

/** Get node argument mappings */
export const selectNodeArgMapping = createSelector(
  [selectNodeById],
  (node): ArgumentMapping[] => asArgumentMappings(node?.arg_mapping),
);

/** Get node argument overrides */
export const selectNodeArgOverrides = createSelector(
  [selectNodeById],
  (node): ArgumentOverride[] => asArgumentOverrides(node?.arg_overrides),
);

/** Get node return broker overrides */
export const selectNodeReturnBrokerOverrides = createSelector(
  [selectNodeById],
  (node): string[] => asStringArray(node?.return_broker_overrides),
);
...
export const selectNodeAdditionalDependencyBySourceId = createSelector(
  [
    selectNodeAdditionalDependencies,
    (_state: RootState, _nodeId: string, sourceBrokerId: string) =>
      sourceBrokerId,
  ],
...
export const selectNodeArgOverrideByName = createSelector(
  [
    selectNodeArgOverrides,
    (_state: RootState, _nodeId: string, name: string) => name,
  ],
  (overrides, name) =>
...
export const selectNodeArgMappingSourceBrokerIds = createSelector(
  [selectNodeArgMapping],
  (mappings) => [
    ...new Set(mappings.map((mapping) => mapping.source_broker_id)),
  ],
);
...
export const selectNodeArgMappingTargetArgNames = createSelector(
  [selectNodeArgMapping],
  (mappings) => [
    ...new Set(mappings.map((mapping) => mapping.target_arg_name)),
  ],
);
...
export const selectNodesByWorkflowId = createSelector(
  ...
  (nodes, workflowId) =>
    nodes.filter((node) => node.workflow_id === workflowId),
);
...
export const selectNodeIsConfigured = createSelector([selectNodeById], (node) =>
  Boolean(
    node?.function_id && node?.step_name && node.step_name !== "Unnamed Step",
  ),
);
...
export const createNodeSelector = <T>(
  selector: (node: DbFunctionNode | null) => T,
) => createSelector([selectNodeById], selector);

/** Batch selector to get multiple node properties at once */
export const selectNodeSummary = createSelector([selectNodeById], (node) =>
  node
    ? {
        id: node.id,
        ...
        hasDependencies:
          asWorkflowDependencies(node.additional_dependencies).length > 0,
        hasArgMappings: asArgumentMappings(node.arg_mapping).length > 0,
        hasArgOverrides: asArgumentOverrides(node.arg_overrides).length > 0,
      }
    : null,
);
