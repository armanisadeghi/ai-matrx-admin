...
export interface CallScope {
    ...
    /** TODO: Resolved from appContextSlice (slice not yet created) */
    organization_id?: string;

    /**
     * Workspace ID — nestable entity between org and project.
     * TODO: Resolved from appContextSlice (slice not yet created)
     */
    workspace_id?: string;
    ...
    /**
     * Conversation ID for continue-conversation calls.
     * Resolved from cx-conversation Redux state or passed as scopeOverride.
     */
...
async function waitForAuthReady(
    ...
    console.warn('[callApi] Auth not ready after timeout — proceeding without credentials.');
    return false;
}
 ...
 */
function resolveScope(state: RootState, overrides?: Partial<CallScope>): CallScope {
    // user_id: available from existing userSlice
    // TODO: replace with a proper selector once userSlice exposes profile.id directly
    const userId: string | undefined = (state as any)?.user?.profile?.id ?? undefined;
    ...
    const resolved: CallScope = {
        user_id: userId,
        organization_id: hasAppContext ? (selectOrganizationId(state) ?? undefined) : undefined,
        workspace_id: hasAppContext ? (selectWorkspaceId(state) ?? undefined) : undefined,
        project_id: hasAppContext ? (selectProjectId(state) ?? undefined) : undefined,
        task_id: hasAppContext ? (selectTaskId(state) ?? undefined) : undefined,
        conversation_id: hasAppContext ? (selectConversationId(state) ?? undefined) : undefined,
    };
...
/**
 * Assemble the final request body, injecting scope fields where the backend
 * schema expects them.
 *
 * Currently the FastAPI backend reads identity from the auth header rather than
 * the body. Scope fields (org, project, task) are reserved for future use.
 *
 * TODO: When the backend introduces scoped endpoints that accept these fields
 * in the body, inject them here rather than at each call site.
 */
function buildRequestBody(
    ...
    // Future: merge scope fields that the specific endpoint accepts.
    return body ?? {};
}
...
async function executeJsonRequest<T>(
    ...
    if (!response.ok) {
        ...
        return {
            ...
            conversationId,
            error: {
                type: response.status >= 400 && response.status < 500 ? 'validation_error' : 'http_error',
...
async function executeStreamingRequest(
    ...
    config: Pick<
        ApiCallConfig,
        'signal' | 'onStreamStart' | 'onStreamEvent' | 'onStreamComplete' | 'onStreamError'
    >
    ...
    const { events, requestId, conversationId } =
        parseNdjsonStream(response, config.signal ?? undefined);
...
export function callApi<
    P extends keyof paths,
    M extends HttpMethod = HttpMethod
>(config: ApiCallConfig<P, M>): ThunkAction<Promise<ApiCallResult>, RootState, unknown, Action> {
    return async (_dispatch, getState) => {
        ...
        try {
            if (config.stream) {
                return await executeStreamingRequest(url, config.method, headers, body, config);
            } else {
                return await executeJsonRequest(url, config.method, headers, body, config.signal);
            }
        } catch (err) {
...
export type AgentBlocksStartBody = components['schemas']['AgentBlocksStartRequest'];

/** Body type for POST /ai/conversations/{conversation_id} */
export type ConversationContinueBody = components['schemas']['ConversationContinueRequest'];
...
export function callConversationContinue(options: CallConversationContinueOptions) {
    return callApi({
