...
import {
  getChatActions,
  getChatActionsWithThunks,
} from "@/lib/redux/entity/custom-actions/chatActions";
import {
  createConversationSelectors,
  createMessageSelectors,
} from "@/lib/redux/entity/selectors";
...
import { entityUpdateActionsWithThunks } from "@/lib/redux/entity/slices/entityUpdateActionFactories";

export const useChatBasics = () => {
  const { aiModelRecords: models, fetchAll: fetchModels } =
    useAiModelWithFetch();
  ...
  const activeConversationRecord = useAppSelector(
    conversationSelectors.selectActiveRecord,
  ) as ConversationRecordWithKey;
  const conversationRecordKey = activeConversationRecord?.matrxRecordId;
  const conversationId = activeConversationRecord?.id;
  const activeMessageRecord = useAppSelector(
    messageSelectors.selectActiveRecord,
  ...
  const conversationMetadata =
    activeConversationRecord?.metadata as ConversationMetadata;

  const fileManager = useFileManagement({
    onFilesUpdate: (files) =>
      actions.updateFiles({
        keyOrId: messageRecordKey,
