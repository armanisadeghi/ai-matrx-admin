...
function CategoryInput({
    ...
    const [showSuggestions, setShowSuggestions] = useState(false);
    const filteredSuggestions = suggestions.filter(
        (s) => s.toLowerCase().includes(value.toLowerCase()) && s !== value && value.length > 0
    );
...
function MetadataForm({
    ...
    return (
        <div className="space-y-5 px-1">
            ...
            <div className="flex items-center justify-between py-1">
                <Label className="flex items-center gap-1.5 cursor-pointer">
                    <Archive className={cn("h-4 w-4", isArchived ? "text-muted-foreground" : "text-muted-foreground")} />
                    Archived
                </Label>
            ...
            <div className="flex gap-2 pt-2 pb-safe">
                <Button variant="outline" onClick={onClose} className="flex-1 h-10" disabled={isSaving}>
                    Cancel
                </Button>
                <Button onClick={handleSave} className="flex-1 h-10" disabled={isSaving}>
                    {isSaving ? (
                        <>
...
export function PromptMetadataModal({ isOpen, onClose, prompt }: PromptMetadataModalProps) {
