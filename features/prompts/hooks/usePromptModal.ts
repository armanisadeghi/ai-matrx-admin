 ...
 * @example
 * ```tsx
 * import { openPromptModal } from '@/features/prompts/hooks/usePromptModal';
 *
 * openPromptModal({
...
export function openPromptModal(
  options: Omit<UsePromptModalOptions, "onResult"> & {
    onResult?: (result: string) => void;
  },
) {
  ...
  console.warn(
    "openPromptModal not yet implemented. Use usePromptModal hook instead.",
  );
  return options;
}
