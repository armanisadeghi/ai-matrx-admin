...
export function useAgentConsumer(
  ...
  useEffect(() => {
    const handleVisibility = () => {
      ...
      // Tab became visible — check if we're stale
      // We use a store selector inside the handler to avoid stale closures
      // dispatch thunk that checks its own freshness internally
      dispatch((_, getState) => {
