...
export function usePromptExecutionAdapter({
  ...
  const messages = useAppSelector((state) =>
    enabled ? selectMessages(state, runId) : EMPTY_MESSAGES,
  );
  const isExecuting = useAppSelector((state) =>
    enabled ? selectIsExecuting(state, runId) : false,
  );
  const streamingText = useAppSelector((state) =>
    enabled ? selectStreamingTextForInstance(state, runId) : null,
  );
  ...
  useEffect(() => {
    if (!enabled || !runId || !streamingText || !streamingMessageIdRef.current)
      return;
