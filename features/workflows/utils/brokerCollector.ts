 ...
 * @returns Organized collection of broker information
 */
export function collectWorkflowBrokers(workflowData: CompleteWorkflowData): WorkflowBrokerCollection {
  if (!workflowData || !workflowData.userInputs || !workflowData.nodes || !workflowData.relays) {
    return createEmptyBrokerCollection();
  }
  ...
  // PRODUCERS: User inputs publish to their broker_id
  workflowData.userInputs.forEach(userInput => {
    addProducer(userInput.broker_id, {
      nodeId: userInput.id,
  ...
  // RELAYS: source_broker_id is consumed, target_broker_ids are produced
  workflowData.relays.forEach(relay => {
 ...
 */
export function getUniqueBrokerIds(workflowData: CompleteWorkflowData): string[] {
