...
import {
  recordFromJson,
  stringArrayFromJson,
} from "@/features/workflows/utils/jsonFromDb";
    ...
    async (id: string, type?: string) => {
      ...
      try {
        if (type === "registeredFunction" || type === "workflowNode") {
          ...
          const savedInput = await saveWorkflowUserInput(
            workflowId,
            userId,
            newNodeData,
            false,
          ...
          const newBrokerRelayData: BrokerRelayNodeData = {
            ...
            target_broker_ids: stringArrayFromJson(
              savedRelay.target_broker_ids,
            ),
            label: savedRelay.label || "Broker Relay",
            metadata: { ...recordFromJson(savedRelay.metadata) },
            workflow_id: workflowId,
          };
    ...
    async (source_broker_id: string, target_broker_ids?: string) => {
      ...
      try {
        ...
        const newBrokerRelayData: BrokerRelayNodeData = {
          ...
          type: "brokerRelay",
          source_broker_id: savedRelay.source_broker_id,
          target_broker_ids: stringArrayFromJson(savedRelay.target_broker_ids),
          label: savedRelay.label || "Broker Relay",
          metadata: { ...recordFromJson(savedRelay.metadata) },
          workflow_id: workflowId,
        };
        ...
        console.error(
          "Error adding direct relay node:",
          error,
          "inputs:",
          source_broker_id,
          target_broker_ids,
  ...
  const handlePrepareRecipeNode = useCallback(
    (
      ...
      try {
        ...
        const position = getCenterPosition();
        const newId = uuidv4();
        const { user_id: _userId, ...restBase } = baseNode;
        const nodeData: Omit<DbFunctionNode, "user_id"> = {
          ...restBase,
          id: newId,
          workflow_id: workflowId,
        };
  ...
  const handleConnection = useCallback(
    (matrxEdge: any) => {
      ...
      if (typeof window !== "undefined" && window.workflowSystemRef) {
        ...
        window.workflowSystemRef.onConnectionMade?.(
          sourceNode?.data,
          targetNode?.data,
          matrxEdge,
        );
