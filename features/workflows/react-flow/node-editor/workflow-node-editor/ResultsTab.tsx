...
const ResultsTab: React.FC<TabComponentProps> = ({
  nodeData,
  enrichedBrokers,
}) => {
  ...
  const firstReturnBroker = returnBrokerOverrides[0];

  const lastReturnBroker =
    returnBrokerOverrides[returnBrokerOverrides.length - 1];

  const returnBrokerData = enrichedBrokers?.find(
    (broker) => broker.id === firstReturnBroker,
  )?.knownBrokerData;

  const dataOutputComponentId = returnBrokerData?.outputComponent;
  const dataOutputComponentData =
    dataOutputComponentRecords[`id:${dataOutputComponentId}`];
  const importPath = dataOutputComponentData?.additionalParams
    ?.importPath as string;
  const rawData = useAppSelector((state) =>
    brokerSelectors.selectValue(state, firstReturnBroker),
  );

  let componentName = dataOutputComponentData?.uiComponent;
  console.log("[ResultsTab] componentName", componentName);

  const lastReturnBrokerData = useAppSelector((state) =>
    brokerSelectors.selectValue(state, lastReturnBroker),
  );

  const autoType = useMemo(
    () => detectResponseType(lastReturnBrokerData),
    [lastReturnBrokerData],
  );
                ...
                <div className="text-center space-y-4 max-w-md">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      This tab will display execution results for this node and
                      the entire workflow.
                    </p>
                  </div>
