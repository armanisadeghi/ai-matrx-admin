...
import {
  NodeDataMethods,
  ValidationMode,
} from "../custom-workflow-nodes/types";
import { validateNodeUpdate } from "@/features/workflows/utils/node-utils";
import {
  reactFlowToDatabase,
  databaseToReactFlow,
} from "@/features/workflows/service/functionNodeService";
...
export function createNodeDataMethods(
   ...
   */
  const toDbNode = (node: FunctionNode): WorkflowNodeInsert => {
   ...
   */
  const toReactFlowNode = (dbNode: WorkflowNodePersistShape): FunctionNode => {
   ...
   */
  const enhancedUpdateCallbackFromDb = (
    updatedDbNode: WorkflowNodePersistShape,
  ): void => {
  ...
  return {
    ...
    updateStepName: (stepName: string) => {
      enhancedUpdateCallback({
        ...currentNode,
        data: { ...currentNode.data, step_name: stepName },
      });
    },
    ...
    updateFunctionType: (functionType: string) => {
      enhancedUpdateCallback({
        ...currentNode,
        data: { ...currentNode.data, function_type: functionType },
      });
    },
    ...
    updateExecutionRequired: (required: boolean) => {
      enhancedUpdateCallback({
        ...currentNode,
        data: { ...currentNode.data, execution_required: required },
      });
    },
    ...
    updateFunctionId: (functionId: string) => {
      enhancedUpdateCallback({
        ...currentNode,
        data: { ...currentNode.data, function_id: functionId },
      });
    },
    ...
    updateWorkflowId: (workflowId: string) => {
      enhancedUpdateCallback({
        ...currentNode,
        data: { ...currentNode.data, workflow_id: workflowId },
      });
    },
    ...
    removeArgumentOverride: (argName: string) => {
      const updated = {
        ...
        data: {
          ...currentNode.data,
          arg_overrides: (currentNode.data.arg_overrides || []).filter(
            (o) => o.name !== argName,
          ),
        },
      };
    ...
    removeArgumentMappingsForArg: (argName: string) => {
      const updated = {
        ...
        data: {
          ...currentNode.data,
          arg_mapping: (currentNode.data.arg_mapping || []).filter(
            (m) => m.target_arg_name !== argName,
          ),
        },
      };
      enhancedUpdateCallback(updated);
    ...
    addDependency: (sourceBrokerId: string, targetBrokerId?: string) => {
      ...
      const updatedDbNode = addWorkflowDependency(
        dbNode,
        enhancedUpdateCallbackFromDb,
      );
      // Update the last added dependency with the actual values
      const dependencies = [
        ...asWorkflowDependencies(updatedDbNode.additional_dependencies),
      ];
    ...
    updateDependency: (
      index: number,
      field: "source_broker_id" | "target_broker_id",
      value: string,
    ) => {
      const dbNode = toDbNode(currentNode);
      updateWorkflowDependency(
      ...
      const updated = {
        ...
        data: {
          ...
          return_broker_overrides: [
            ...(currentNode.data.return_broker_overrides || []),
            brokerId,
          ],
        },
      };
    ...
    updateReturnBrokerOverride: (index: number, brokerId: string) => {
      ...
      if (overrides[index] !== undefined) {
        ...
        const updated = {
          ...currentNode,
          data: { ...currentNode.data, return_broker_overrides: overrides },
        };
        enhancedUpdateCallback(updated);
      }
    },
    ...
    getArgumentsWithMappings: () => {
      ...
      return argumentsWithData.map((arg) => ({
        ...
        override: currentNode.data.arg_overrides?.find(
          (o) => o.name === arg.name,
        ),
      }));
    },
