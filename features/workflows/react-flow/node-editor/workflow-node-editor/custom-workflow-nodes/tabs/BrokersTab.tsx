...
const BrokersTab: React.FC<BrokersTabProps> = ({
  nodeData,
  onNodeUpdate,
  nodeDefinition,
  enrichedBrokers,
  ...
  const getPredefinedBrokers = () => {
    ...
    const metaDefinition = asNodeDefinitionFromMetadata(nodeData.metadata);
    if (!metaDefinition?.predefined_brokers) return [];

    // Get the dynamic broker argument value (recipe_id)
    const dynamicBrokerArg = metaDefinition.dynamic_broker_arg;
    const recipeIdArg = asArgumentOverrides(nodeData.arg_overrides).find(
      (arg) => arg.name === dynamicBrokerArg,
    );
    ...
    return metaDefinition.predefined_brokers.map((broker: any) => ({
      id: broker.dynamic_id
        ? broker.id.replace(`{${dynamicBrokerArg}}`, recipeId)
        : broker.id,
      label: broker.label,
  ...
  const predefinedBrokerIds = new Set(
    runtimeBrokers.map((broker) => broker.id),
  );

  // Filter out predefined brokers from the editable list (they show in Auto-Generated section)
  const editableReturnBrokerOverrides = returnBrokerOverrides.filter(
    (brokerId) => !predefinedBrokerIds.has(brokerId),
  );
  ...
  React.useEffect(() => {
    if (runtimeBrokers.length > 0) {
      ...
      const currentOverrides = asStringArray(nodeData.return_broker_overrides);

      // Check if any predefined brokers are missing from overrides
      const missingBrokers = predefinedBrokerIds.filter(
        (id) => !currentOverrides.includes(id),
      );
      ...
      if (missingBrokers.length > 0) {
        ...
        const updated = {
          ...nodeData,
          return_broker_overrides: updatedOverrides,
        };
  ...
  const updateReturnBrokerOverride = (editableIndex: number, value: string) => {
    ...
    const editableBrokers = returnBrokerOverrides.filter(
      (brokerId) => !predefinedBrokerIds.has(brokerId),
    );
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Custom broker IDs to return from this node in addition to
                auto-generated brokers.
              </p>
            </div>
        ...
        <CardContent>
          {editableReturnBrokerOverrides.length > 0 ? (
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <Input
                          value={brokerId}
                          onChange={(e) =>
                            updateReturnBrokerOverride(index, e.target.value)
                          }
            ...
            <div className="text-center py-8 text-muted-foreground">
              No custom return broker overrides configured. Click "Add Override"
              to add custom brokers.
              {runtimeBrokers.length > 0 && (
                <div className="mt-2 text-sm">
                  Auto-generated brokers are shown in the section below.
                </div>
              )}
            </div>
          )}
      ...
      {runtimeBrokers.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              These brokers are automatically generated by this node when it
              executes.
            </p>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="text-left p-2 font-medium text-sm w-1/4">
                      Label
                    </th>
                    <th className="text-left p-2 font-medium text-sm w-1/2">
                      Broker ID
                    </th>
                    <th className="text-left p-2 font-medium text-sm w-1/8">
                      Type
                    </th>
                    <th className="text-left p-2 font-medium text-sm w-1/8">
                      Availability
                    ...
                    <React.Fragment key={broker.id}>
                      ...
                      >
                        ...
                        <td className="p-2 w-1/2">
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                navigator.clipboard.writeText(broker.id)
                              }
                        ...
                        <td className="p-2 w-1/8">
                          <Badge
                            variant={
                              broker.guaranteed ? "default" : "secondary"
                            }
                            className="text-xs"
                          >
