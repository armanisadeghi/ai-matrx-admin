...
import {
  ...
  NeededBroker,
  RecipeConfig,
} from "@/features/workflows/service/recipe-service";
...
const RecipeNodeEditor: React.FC<RecipeNodeEditorProps> = ({
  nodeData,
  onSave,
  onClose,
  open,
  ...
  const getRecipeParams = useCallback((currentNode: DbFunctionNode) => {
    const overrides = asArgumentOverrides(currentNode.arg_overrides);
    const recipeIdArg = overrides.find(
      (arg) => arg.name === nodeDefinition.dynamic_broker_arg,
    );
    const versionArg = overrides.find((arg) => arg.name === "version");
  ...
  const fetchRecipeDetails = useCallback(
    async (recipeId: string, version?: number) => {
      ...
      try {
        const details = await getCompiledRecipeByVersionWithNeededBrokers(
          recipeId,
          version || undefined,
        );
  ...
  const handleNodeUpdate = useCallback(
    (updatedNode: DbFunctionNode) => {
      ...
      const { recipeId: newRecipeId, version: newVersion } =
        getRecipeParams(updatedNode);
      ...
      nodeWithUpdates = {
        ...nodeWithUpdates,
        metadata: {
          ...asMetadataRecord(nodeWithUpdates.metadata),
  ...
  const RecipeOverviewTab = ({
    ...
    Object.entries(nodeDefinition.custom_sections).forEach(
      ([key, componentName]) => {
