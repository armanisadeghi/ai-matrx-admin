...
} from "@/features/workflows/react-flow/node-editor/workflow-node-editor/utils/dependency-utils";
...
const RecipeDetailsTab = ({
  ...
  const currentDependencies = asWorkflowDependencies(
    nodeData.additional_dependencies,
  );
  ...
  const addBrokerDependency = (brokerId: string) => {
    ...
    addWorkflowDependency(nodeData, (updatedNode) => {
      ...
      const depLen = asWorkflowDependencies(
        updatedNode.additional_dependencies,
      ).length;
      const lastIndex = Math.max(0, (depLen || 1) - 1);
      updateWorkflowDependency(
        updatedNode,
        onNodeUpdate,
        lastIndex,
  ...
  const removeBrokerDependency = (brokerId: string) => {
    const dependencyIndex = currentDependencies.findIndex(
      (dep) => dep.source_broker_id === brokerId,
    );
    ...
    <div className="space-y-4 p-4">
      ...
      {recipeDetails && (
        <div className="space-y-4">
          ...
          {recipeDetails.neededBrokers &&
            recipeDetails.neededBrokers.length > 0 && (
              <Card>
                          ...
                          <TableCell className="py-2">
                            {isBrokerInDependencies(broker.id) ? (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  removeBrokerDependency(broker.id)
                                }
          ...
          <div className="space-y-3">
            {recipeDetails.postResultOptions &&
              Object.keys(recipeDetails.postResultOptions).length > 0 && (
            ...
            {recipeDetails.settings &&
              Object.keys(recipeDetails.settings).length > 0 && (
                <details className="bg-gray-50 dark:bg-gray-800 rounded-md">
                  <summary className="cursor-pointer p-3 font-medium text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md">
                    Settings
                  </summary>
