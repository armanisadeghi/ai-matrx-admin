...
export const flashcardPersistenceService = {
     ...
     */
    async saveSet(input: FlashcardSetInsert): Promise<{ data: FlashcardSetRow | null; isExisting: boolean; error: any }> {
        try {
            ...
            if (input.message_id) {
                ...
                if (existing) {
                    return { data: existing as FlashcardSetRow, isExisting: true, error: null };
                }
            }
            ...
            return { data: data as FlashcardSetRow | null, isExisting: false, error };
        } catch (error) {
    ...
    async listSets(filters?: {
        ...
        is_archived?: boolean;
        search?: string;
    }): Promise<{ data: FlashcardSetRow[] | null; error: any }> {
        try {
            const userId = requireUserId();
            let query = supabase
                .from('user_flashcard_sets')
            ...
            return { data: data as FlashcardSetRow[] | null, error };
        } catch (error) {
     ...
     */
    async getSet(setId: string): Promise<{ data: FlashcardSetRow | null; error: any }> {
        try {
            const userId = requireUserId();
            const { data, error } = await supabase
                .from('user_flashcard_sets')
     ...
     */
    async getSetByMessageId(messageId: string): Promise<{ data: FlashcardSetRow | null; error: any }> {
        try {
            const userId = requireUserId();
            const { data, error } = await supabase
                .from('user_flashcard_sets')
     ...
     */
    async deleteSet(setId: string): Promise<{ error: any }> {
        try {
            const userId = requireUserId();
     ...
     */
    async submitReview(input: FlashcardReviewInsert): Promise<{ data: FlashcardReviewRow | null; error: any }> {
        try {
            const userId = requireUserId();
            ...
            return { data: data as FlashcardReviewRow | null, error };
        } catch (error) {
            return { data: null, error };
        }
    },
     ...
     */
    async getReviews(setId: string): Promise<{ data: FlashcardReviewRow[] | null; error: any }> {
        try {
            const userId = requireUserId();
            const { data, error } = await supabase
                .from('user_flashcard_reviews')
     ...
     */
    async getCardStats(setId: string, cardCount: number): Promise<{ stats: CardReviewStats[]; error: any }> {
        const { data: reviews, error } = await this.getReviews(setId);

        if (error || !reviews) {
            // Return empty stats for each card
            const stats: CardReviewStats[] = Array.from({ length: cardCount }, (_, i) => ({
                cardIndex: i,
                correct: 0,
                partial: 0,
                incorrect: 0,
     ...
     */
    async resetReviews(setId: string): Promise<{ error: any }> {
        try {
            const userId = requireUserId();
            const { error } = await supabase
                .from('user_flashcard_reviews')
