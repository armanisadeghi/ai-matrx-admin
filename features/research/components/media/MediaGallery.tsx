...
export default function MediaGallery() {
    ...
    const params = filter !== 'all' ? `media_type=${filter}` : undefined;
    const { data: media, refetch } = useResearchMedia(projectId, params);

    const mediaList = (media as ResearchMedia[]) ?? [];

    const handleToggleRelevance = useCallback(async (item: ResearchMedia) => {
        await api.updateMedia(projectId, item.id, { is_relevant: !item.is_relevant });
        refetch();
