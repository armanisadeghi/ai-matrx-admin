...
export function DataStoreBindPanel({
  ...
  const { memberOf, bind, unbind, loading: membershipLoading } =
    ...
    <div className="p-3 space-y-2.5">
      ...
      {loading && stores.length === 0 ? (
        ...
        <div className="px-3 py-3 border border-dashed border-border rounded-md bg-muted/20 text-[11px] text-muted-foreground leading-snug">
          No data stores yet. Create one to bind this document — agent
          retrieval (<code>rag_search</code>) requires an explicit store id.
        </div>
      ) : (
      ...
      <p className="text-[10px] text-muted-foreground/70 pt-1 leading-snug">
        ...
        <code className="mx-1">rag.search_data_store(store_id, query)</code>
        — no store id, no retrieval.
      </p>
    </div>
  );
}
