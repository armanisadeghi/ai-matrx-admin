...
} from "@/features/files/api/rag-search";
import { useDataStores } from "@/features/data-stores/hooks/useDataStores";
    ...
    <div className="flex h-[calc(100vh-3rem)] bg-background">
      ...
      <section className="flex-1 flex flex-col overflow-hidden">
        <header className="border-b">
          <form onSubmit={onSubmit} className="px-4 py-3 flex items-center gap-2">
          ...
          {storeId && (
            <div className="px-4 pb-2 -mt-1 text-xs text-muted-foreground">
              ...
              <strong className="text-foreground">
                {stores.stores.find((s) => s.id === storeId)?.name ?? "(unknown store)"}
              </strong>
              <button
                onClick={() => setStoreId(null)}
...
function EmptyState() {
  return (
    <div className="m-6 max-w-2xl rounded-md border bg-muted/20 p-6 text-sm text-muted-foreground">
      <p className="font-medium text-foreground mb-2">Search your indexed content</p>
      <p className="mb-2">
        Type a natural-language query above. Results are ranked by hybrid
        retrieval (vector + lexical + Cohere rerank). Pick a data store on
        the left to scope to one curated bucket; leave it on{" "}
        <em>All accessible content</em> to search everything you can see —
        your docs, your org's docs, and the global library (regulatory
        guidelines, etc.).
      </p>
      <p className="mb-2">
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5 text-xs text-muted-foreground">
        ...
        <span className="font-mono uppercase tracking-wide">{hit.source_kind}</span>
        {libraryShortCode && (
