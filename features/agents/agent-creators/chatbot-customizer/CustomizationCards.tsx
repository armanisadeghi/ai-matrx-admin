...
export const GenericOptionsCard: React.FC<
  CardComponentProps & {
    options: Array<
      | ToggleOption
      | SliderOption
...
export const PersonalityCard: React.FC<CardComponentProps> = (props) => {
  const personalityOptions: RadioGroupOption = {
    ...
    options: [
      ...
      },
      {
        id: "professional",
        label: "Professional",
        description: "Formal, precise, concise",
      },
      {
        id: "enthusiastic",
        label: "Enthusiastic",
        description: "Energetic, encouraging, positive",
      },
      {
        id: "balanced",
        label: "Balanced",
        description: "Neutral, adaptable, versatile",
      },
      {
        id: "academic",
        label: "Academic",
        description: "Detailed, thorough, analytical",
      },
    ],
  ...
  const currentValue =
    props.state["personality"] || (personalityOptions.defaultValue as string);
...
export const ToneCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["tone"] || (toneOptions.defaultValue as string);
...
export const VerbosityCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["verbosity"] || (verbosityOption.defaultValue as number);
...
export const FormalityCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["formality"] || (formalityOption.defaultValue as number);
...
export const StyleEnhancementsCard: React.FC<CardComponentProps> = (props) => {
  ...
  return <GenericOptionsCard {...props} options={options} />;
};

// Interactive Features Card
export const InteractiveFeaturesCard: React.FC<CardComponentProps> = (
  props,
) => {
  const options: ToggleOption[] = [
    {
...
export const ReasoningDepthCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["reasoning"] || (reasoningOption.defaultValue as number);
...
export const CreativityLevelCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["creativity"] || (creativityOption.defaultValue as number);
...
export const MemoryFeaturesCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["memory"] || (memoryOption.defaultValue as boolean);
...
export const ExpertiseAreasCard: React.FC<CardComponentProps> = (props) => {
  ...
  const currentValue =
    props.state["expertise"] || (expertiseOptions.defaultValue as string[]);
...
export const PersonalInfoCard: React.FC<CardComponentProps> = (props) => {
  const options = [
    ...
    },
    {
      ...
      placeholder:
        "Topics you're interested in (e.g., cooking, technology, travel)",
      defaultValue: "",
    },
  ];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {options.map((option) => {
          const currentValue =
            props.state[option.id] !== undefined
