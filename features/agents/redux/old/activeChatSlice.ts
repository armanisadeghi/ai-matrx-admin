 ...
 * This lets ConversationInput connect to a real Redux session on /a/ routes
 * without creating a conversation in the database. The session is bootstrapped
 * on mount and replaced when a real conversation is created.
 */
...
export const selectActiveChatSessionId = (
  s: StateWithActiveChat,
): string | null => s.activeChat.sessionId;

export const selectActiveChatAgent = (
  s: StateWithActiveChat,
): ActiveChatAgent => s.activeChat.selectedAgent;
...
export const selectFirstMessage = (
  s: StateWithActiveChat,
): FirstMessage | null => s.activeChat.firstMessage;
...
export const selectAgentDefaultSettings = (
  s: StateWithActiveChat,
): PromptSettings => s.activeChat.agentDefaultSettings;

export const selectMessageContext = (
  s: StateWithActiveChat,
): Record<string, unknown> => s.activeChat.messageContext;
