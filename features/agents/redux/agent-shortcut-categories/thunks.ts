...
export const fetchCategoriesForScope = createAsyncThunk<
  ...
  ThunkApi
>(
  "agentShortcutCategory/fetchForScope",
  async (scopeRef, { dispatch }) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/agent-shortcut-categories?${qs}`,
...
>(
  ...
  async (input, { dispatch, getState }) => {
    ...
    try {
      ...
      const result = await parseJsonOrThrow<{ data: CategoryApiRow }>(
        response,
      );
      const def = categoryRowToDef(result.data);
      dispatch(upsertCategory(def));
