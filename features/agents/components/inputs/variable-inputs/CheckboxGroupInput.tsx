    ...
    if (checked) {
      ...
      newSelected = [
        ...selectedItems.filter(
          (item) => item !== actualOption && !item.startsWith("Other: "),
        ),
        actualOption,
      ];
    ...
    <div className={compact ? "space-y-1" : "space-y-4"}>
      ...
      {value && hasNonOptionValues && (
        <div
          className={
            compact
              ? "p-1.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded"
              : "p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg"
          }
        >
          <p
            className={
              compact
                ? "text-[11px] font-medium text-amber-800 dark:text-amber-300 mb-0.5"
                : "text-xs font-medium text-amber-800 dark:text-amber-300 mb-1"
            }
          >
            Current values:
          </p>
          <div
            className={
              compact
                ? "text-xs text-amber-900 dark:text-amber-200 space-y-0.5"
                : "text-sm text-amber-900 dark:text-amber-200 space-y-0.5"
            }
          >
              ...
              <div key={idx} className="flex items-center gap-2">
                {!options.includes(item) && (
                  <span
                    className={
                      compact
                        ? "text-[11px] bg-amber-200 dark:bg-amber-800 px-1 rounded"
                        : "text-xs bg-amber-200 dark:bg-amber-800 px-1 rounded"
                    }
                  >
                    Custom
                  </span>
          ...
          <p
            className={
              compact
                ? "text-[11px] text-amber-700 dark:text-amber-400 mt-0.5"
      ...
      <div className={compact ? "space-y-1" : "space-y-2"}>
        {options.map((option, index) => {
          ...
          const itemKey = isEmptyOrWhitespace(option)
            ? `empty-${index}`
            : option;
            ...
            <div
              ...
              className={
                compact
                  ...
                  : "flex items-center space-x-3 p-3 bg-transparent rounded-lg border-border hover:border-gray-300 dark:hover:border-gray-600 transition-colors cursor-pointer"
              }
            ...
            >
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  handleToggle(displayOption, checked as boolean)
                }
                id={itemId}
              />
              <Label
