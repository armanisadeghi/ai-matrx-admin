...
import { Paperclip, Search, Lightbulb, ChevronDown, ArrowUp, Minimize2, Maximize2, X } from "lucide-react";
...
interface PromptInputProps {
    ...
    defaultModelKey?: MatrxRecordId;
}

const PromptInput: React.FC<PromptInputProps> = ({ onSendMessage, defaultModelKey }) => {
    ...
    const defaultModelName = models[defaultModelKey]?.commonName;
    ...
    useEffect(() => {
        if (textareaRef.current) {
            const minHeight = 110;
            const notificationHeight = uploadedFiles.length > 0 ? 24 : 0;
                            ...
                            <div className="absolute bottom-full right-0 mb-2 bg-white dark:bg-zinc-800 rounded-lg shadow-lg border border-zinc-200 dark:border-zinc-700 min-w-48 z-20">
                                ...
                                <div className="py-1">
                                    {Object.values(models).map((model) => (
                                        <button
                                            key={model.id}
                                            onClick={() => selectModel(model.commonName)}
                                            className={`block w-full text-left px-4 py-2 text-sm ${
                                                selectedModel === model.commonName
