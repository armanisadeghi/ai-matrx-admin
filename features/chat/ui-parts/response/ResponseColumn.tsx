...
const ResponseColumn: React.FC<ResponseColumnProps> = ({ chatHook }) => {
    const { currentMessages, chatSocket } = chatHook;
    ...
    const nextDisplayOrder = useMemo(() => Math.max(...baseMessages.map((m) => m.displayOrder)) + 1, [baseMessages]);

    const nextSystemOrder = useMemo(() => Math.max(...baseMessages.map((m) => m.systemOrder)) + 1, [baseMessages]);
    ...
    }, [isStreaming, streamingResponse, scrollToBottom]);
