...
export function StudioVariantTilePending({
    presetId,
}: {
    presetId: string;
}) {
    ...
    return (
        <div className="relative flex flex-col rounded-xl border border-dashed border-border bg-muted/20 overflow-hidden">
            <div
                ...
                style={{ aspectRatio: `${preset?.width ?? 1} / ${preset?.height ?? 1}` }}
            >
                <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
            </div>
            <div className="p-2.5 text-xs">
                <p className="font-medium text-sm truncate">{preset?.name ?? presetId}</p>
