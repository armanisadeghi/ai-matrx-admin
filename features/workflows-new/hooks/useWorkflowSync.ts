...
export const useWorkflowSync = (workflowId: string) => {
    ...
    const sourceNodes = (workflowSources || []).map((source, index) => {
      ...
      return {
        ...
        data: {
          ...
          displayMode: "compact", // Default display mode
        },
      ...
      if (node.inputs) {
        node.inputs.forEach((input, inputIndex) => {
          if (input.source_broker_id) {
            ...
            const sourceInputNode = workflowSources?.find(source => 
              source.brokerId === input.source_broker_id
            );
            
            if (sourceInputNode) {
              const sourceMapKey = `${node.id}-${sourceInputNode.sourceType}:${sourceInputNode.brokerId}`;
  ...
  const saveWorkflow = useCallback(async (reactFlowNodes: Node[], reactFlowEdges: Edge[], reactFlowViewport: any) => {
    try {
