...
import React, { useCallback, useMemo, useEffect, forwardRef, useImperativeHandle, useState } from "react";
import {
    ReactFlow,
    Background,
    Controls,
...
export const WorkflowCanvas = forwardRef<WorkflowCanvasRef, WorkflowCanvasProps>(
    (
        ...
        const handleUpdateReduxWithUiData = () => {
            // Placeholder for future Redux UI data synchronization
        };
        ...
        }, [initialNodes, initialEdges, isInitialized, setNodes]);
        ...
        const nodeTypes: NodeTypes = useMemo(
            () => ({
                ...
                functionNode: WorkflowNode,
                recipes: WorkflowNode,
                prompts: WorkflowNode,
                agents: WorkflowNode,
                processors: WorkflowNode,
                extractors: WorkflowNode,
                files: WorkflowNode,
                database: WorkflowNode,
                web: WorkflowNode,
                media: WorkflowNode,
                documents: WorkflowNode,
                integrations: WorkflowNode,
                commands: WorkflowNode,
                executors: WorkflowNode,
                api: WorkflowNode,
                other: WorkflowNode,
            ...
            (changes) => {
                ...
                if (hasSignificantChange) {
                    setTimeout(() => {
                        ...
                        setHistory((prev) => [...prev.slice(0, historyIndex + 1), { nodes: currentNodes, edges: currentEdges }]);
                        setHistoryIndex((prev) => prev + 1);
                    }, 0);
                }
            },
            [onNodesChange, historyIndex, reactFlowInstance]
        );
        ...
        // Save function - extract current React Flow state
        const handleSave = useCallback(async () => {
            if (onSave) {
                const currentNodes = reactFlowInstance.getNodes();
                const currentEdges = reactFlowInstance.getEdges();
                const currentViewport = reactFlowInstance.getViewport();
                onSave(currentNodes, currentEdges, currentViewport);
            }
        ...
        // Auto arrange function using improved layout
