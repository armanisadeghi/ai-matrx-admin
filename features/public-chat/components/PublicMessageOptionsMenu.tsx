...
const PublicMessageOptionsMenu: React.FC<PublicMessageOptionsMenuProps> = ({
    ...
    const requireAuth = (actionKey: string, featureName: string, description: string) => {
        if (!isAuthenticated) {
    ...
    const handleCopyGoogleDocs = async () => {
        await copyToClipboard(content, {
            ...
            onSuccess: () => {},
            onError: (error) => { throw new Error(getErrorMessage(error, "Failed to copy for Google Docs")); }
        });
    };
...
<body>
    ${filteredHtml}
                ...
                } catch { throw new Error("Failed to generate complete HTML"); }
            },
            onSuccess: () => {},
    ...
    const handleSendEmail = async (email: string) => {
        const response = await fetch('/api/public/email', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ to: email, subject: `AI Chat Response - ${new Date().toLocaleDateString()}`, content, isMarkdown: true }),
        });
        const data = await response.json();
        if (!data.success) throw new Error(data.msg || 'Failed to send email');
    };
    ...
    const handleSaveToScratch = async () => {
        if (isAuthenticated) {
            ...
            requireAuth('save-scratch', 'Save to Scratch', "Sign in to save this response to your Scratch notes. You'll be right back.");
            onClose();
        }
    };
    ...
    const handleSaveToNotes = () => {
        if (isAuthenticated) {
            ...
            requireAuth('save-notes', 'Save to Notes', "Sign in to save this response to your notes. You'll be right back.");
            onClose();
        }
    };
    ...
    const handleAddToTasks = () => {
        if (!requireAuth('add-to-tasks', 'Add to Tasks', "Sign in to add this as a task. You'll be right back.")) return;
    ...
    const handleConvertToBroker = () => {
        toast.info('Coming soon', { description: 'Convert to broker will be available shortly.' });
        onClose();
    };

    const handleAddToDocs = () => {
        toast.info('Coming soon', { description: 'Add to docs will be available shortly.' });
        onClose();
    };
    ...
    const menuItems: MenuItem[] = [
        ...
        },
        {
            ...
            category: "Actions",
            successMessage: isAuthenticated ? "Saved to Scratch!" : "Opening sign-in...",
        ...
        },
        {
            ...
            category: "Actions",
            successMessage: isAuthenticated ? "Opening save dialog..." : "Opening sign-in...",
