...
import {
    ArrowLeft, Plus, Pencil, Trash2, Link, Music, CheckCircle2, Circle,
...
function CopyLinkButton({ slug }: { slug: string }) {
    ...
    return (
        <button
            onClick={(e) => {
                e.stopPropagation();
                navigator.clipboard.writeText(`${window.location.origin}/p/podcast/${slug}`)
...
export function ShowDetailClient({ showId }: ShowDetailClientProps) {
    ...
    const [pendingDeleteEpId, setPendingDeleteEpId] = useState<string | null>(null);
    ...
    const setPanel = (value: string) => {
        startTransition(() => {
            ...
            router.replace(`/administration/podcasts/shows/${showId}?${params.toString()}`);
        });
    };
    ...
    const handleShowSaved = (saved: PcShow) => {
        ...
        if (isNew) {
            startTransition(() => router.replace(`/administration/podcasts/shows/${saved.id}?panel=show`));
        }
    };
    ...
    const back = () => startTransition(() => router.push('/administration/podcasts/shows'));
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b bg-background shrink-0">
                ...
                {show?.image_url && (
                    <img src={show.image_url} alt="" className="w-8 h-8 rounded object-cover shrink-0" />
                )}
                <div className="min-w-0 flex-1">
                    ...
                    {show?.slug && (
                        <p className="text-xs text-muted-foreground font-mono truncate">/p/podcast/{show.slug}</p>
                    )}
                </div>
                {show && (
            ...
            {!isNew && (
                <div className="flex items-center gap-1 px-4 pt-2 border-b bg-background shrink-0">
                    {(['show', 'episodes'] as const).map((tab) => (
                        ...
                        >
                            {tab === 'show' ? 'Show Settings' : `Episodes (${episodes.length})`}
                        </button>
                    ))}
                </div>
            )}
                    ...
                    <div className="flex flex-col h-full min-h-0">
                        <div className="flex items-center justify-between px-4 py-2 border-b bg-background shrink-0">
                            <p className="text-sm text-muted-foreground">
                                {episodes.length === 0 ? 'No episodes yet' : `${episodes.length} episode${episodes.length === 1 ? '' : 's'}`}
                            </p>
                            <Button
                                size="sm"
                                className="h-8 gap-1.5"
                                onClick={() => startTransition(() =>
                                    router.push(`/administration/podcasts/shows/${showId}/episodes/new`)
                                )}
                            >
                            ...
                            <div className="divide-y overflow-y-auto flex-1">
                                {episodes.map((ep) => (
                                    <div
                                        ...
                                        onClick={() => startTransition(() =>
                                            router.push(`/administration/podcasts/shows/${showId}/episodes/${ep.id}`)
                                        )}
                                    >
                                        {/* Thumbnail */}
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-1.5">
                                                {ep.episode_number != null && (
                                                    <span className="text-xs text-muted-foreground shrink-0">Ep {ep.episode_number}</span>
                                                )}
                                                ...
                                                {!ep.og_image_url && (
                                                    <AlertTriangle className="h-3.5 w-3.5 text-warning shrink-0" aria-label="No OG image — shares won't show a preview" />
                                                )}
                                            </div>
                                            <div className="flex items-center gap-3 mt-0.5">
                                                <span className="text-xs font-mono text-muted-foreground px-1 rounded bg-muted">{ep.display_mode}</span>
                                        ...
                                        <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); startTransition(() => router.push(`/administration/podcasts/shows/${showId}/episodes/${ep.id}`)); }}
                                                className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                                                title="Edit"
                                            >
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); setPendingDeleteEpId(ep.id); }}
            ...
            <AlertDialog open={!!pendingDeleteEpId} onOpenChange={(open) => !open && setPendingDeleteEpId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={isDeletingEp}>Cancel</AlertDialogCancel>
                        <AlertDialogAction
