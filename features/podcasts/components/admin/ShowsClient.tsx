...
import {
    Plus, RefreshCw, Search, Pencil, Trash2, Link, Mic, CheckCircle2, Circle, Loader2,
} from 'lucide-react';
        ...
        <>
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b bg-background shrink-0">
                <div className="relative flex-1 max-w-xs">
                    ...
                    />
                    {isPending && <Loader2 className="absolute right-2.5 top-1/2 -translate-y-1/2 h-3 w-3 animate-spin text-muted-foreground" />}
                </div>
                <Button variant="outline" size="sm" onClick={load} className="h-8 px-2" title="Refresh">
                    <RefreshCw className="h-3.5 w-3.5" />
                </Button>
                <Button
                    size="sm"
                    className="h-8 gap-1.5"
                    onClick={() => startTransition(() => router.push('/administration/podcasts/shows/new'))}
                >
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            filtered.map((show) => (
                                <tr
                                    key={show.id}
                                    onClick={() => startTransition(() => router.push(`/administration/podcasts/shows/${show.id}`))}
                                ...
                                >
                                    <td className="px-4 py-2.5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p className="font-medium truncate max-w-[200px]">{show.title}</p>
                                                <p className="text-xs text-muted-foreground">{show.is_published ? 'Published' : 'Draft'}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2.5 text-muted-foreground font-mono text-xs truncate max-w-[140px]">{show.slug}</td>
                                    <td className="px-4 py-2.5 text-muted-foreground text-sm truncate max-w-[140px]">{show.author ?? '—'}</td>
                                    <td className="px-4 py-2.5">
                                        {show.is_published
                                    ...
                                    <td className="px-4 py-2.5">
                                        <div className="flex items-center justify-end gap-0.5">
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); startTransition(() => router.push(`/administration/podcasts/shows/${show.id}`)); }}
                                                className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors opacity-0 group-hover:opacity-100"
                                                title="Edit show"
                                            >
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This cannot be undone. Episodes linked to this show will have their show reference removed.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
