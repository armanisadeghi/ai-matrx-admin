...
export function useAiRun(initialRunId?: string): UseAiRunReturn {
  ...
  const createRun = useCallback(
    async (input: CreateAiRunInput): Promise<AiRun> => {
      setIsSaving(true);
      setError(null);
  ...
  const updateRun = useCallback(
    async (input: UpdateAiRunInput): Promise<AiRun> => {
  ...
  const addMessage = useCallback(
    async (message: RunMessage, overrideRunId?: string): Promise<AiRun> => {
  ...
  const createTask = useCallback(
    async (input: CreateTaskInput, overrideRunId?: string): Promise<AiTask> => {
      const effectiveRunId = overrideRunId || runId;
      if (!effectiveRunId) throw new Error("No run ID set");
  ...
  const updateTask = useCallback(
    async (taskId: string, input: UpdateAiTaskInput): Promise<AiTask> => {
      try {
  ...
  const completeTask = useCallback(
    async (taskId: string, input: CompleteAiTaskInput): Promise<AiTask> => {
      try {
        const completed = await aiTasksService.complete(taskId, input);

        // Update in local tasks array
        setTasks((prev) =>
          prev.map((t) => (t.task_id === taskId ? completed : t)),
        );
