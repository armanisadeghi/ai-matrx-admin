...
export function TaskProvider({ children }: TaskProviderProps) {
  ...
  const [operatingProjectId, setOperatingProjectId] = useState<string | null>(
    null,
  );
  ...
  const projects: Project[] = dbProjectsWithTasks.map((dbProject) => {
    ...
    allTasks.forEach((dbTask) => {
      const task = {
        ...
        settings: (dbTask as any).settings || {},
      };
  ...
  const loadProjectsWithTasks = useCallback(
      ...
      prev.map((p) => {
        ...
        if (p.id === projectId) {
          return {
            ...
            tasks: [
              ...(p.tasks || []),
              { ...taskToMove, project_id: projectId },
            ],
          };
        }
        return p;
      ...
      prev.map((p) =>
        p.id === projectId
          ? {
              ...p,
              tasks:
                p.tasks?.map((t) => (t.id === taskId ? { ...t, title } : t)) ||
                [],
            }
          : p,
      ),
            ...
            p.id === projectId
              ? {
                  ...
                  tasks:
                    p.tasks?.map((t) =>
                      t.id === taskId
                        ? { ...t, description: previousDescription }
                        : t,
                    ) || [],
                }
              : p,
  ...
  const getAllTasks = (): TaskWithProject[] => {
