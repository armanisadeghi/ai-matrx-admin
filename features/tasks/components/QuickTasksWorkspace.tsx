...
const WorkspaceContext = React.createContext<QuickTasksWorkspaceContextType | null>(null);

export const useQuickTasksWorkspace = () => {
  const ctx = React.useContext(WorkspaceContext);
  if (!ctx) throw new Error("useQuickTasksWorkspace must be used within Provider");
  return ctx;
};
...
export function QuickTasksMain() {
  const { selectedTaskId, setSelectedTaskId, selectedProjectId } = useQuickTasksWorkspace();
  const { getFilteredTasks, newTaskTitle, setNewTaskTitle, isCreatingTask, addTask } = useTaskContext();
  ...
  if (!selectedTask) {
     return (
        <div className="flex-1 flex flex-col items-center justify-center h-full text-center p-6 bg-card/50">
           ...
           <p className="text-xs text-muted-foreground mb-4">Select a task from the sidebar, or create a new one.</p>
           ...
           <form onSubmit={handleAddTask} className="flex gap-2 w-full max-w-sm">
             ...
             />
             <Button type="submit" size="sm" className="h-8 text-xs font-semibold" disabled={isCreatingTask || !newTaskTitle.trim() || !selectedProjectId}>
                <Plus className="h-3.5 w-3.5 mr-1" /> Add Task
             </Button>
           </form>
           
           {!selectedProjectId && (
               <p className="text-[10px] text-destructive mt-2">Please select a project in the sidebar first.</p>
           )}
        </div>
     );
  }
  ...
  return (
      <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-background overflow-hidden relative">
          ...
          <div className="absolute top-2 right-2 z-10 opacity-0 hover:opacity-100 transition-opacity">
               <Button variant="outline" size="sm" className="h-7 text-xs bg-background/50 backdrop-blur" onClick={() => setSelectedTaskId(null)}>
                   Close Details
               </Button>
          </div>
