...
export const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({
      ...
      if (newNodes.length > 0 || removedNodeIds.length > 0) {
        ...
        setNodes((currentNodes) => {
          ...
          const filteredNodes = currentNodes
            ...
            .map((n) => ({
              ...n,
              selected: newNodes.length > 0 ? false : n.selected,
            }));

          // Ensure new nodes have higher z-index than existing ones
          const maxZIndex = Math.max(
            0,
            ...filteredNodes.map((n) => n.zIndex || 0),
          );
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      const newReduxEdges = initialEdges.filter(
        (e) => !currentEdgeIds.has(e.id),
      );

      // Only remove edges that were managed by Redux (not temporary React Flow edges)
      const removedReduxEdgeIds = edges
        .filter((e) => !reduxEdgeIds.has(e.id) && !e.data?.isTemporary)
        .map((e) => e.id);
      ...
      if (newReduxEdges.length > 0 || removedReduxEdgeIds.length > 0) {
        ...
        setEdges((currentEdges) => {
          const filteredEdges = currentEdges.filter(
            (e) => !removedReduxEdgeIds.includes(e.id),
          );
  ...
  const { handleProcessConnection } = useProcessConnection({
    workflowId,
    handleSave,
  });
      ...
      >
        {showGrid && (
          <Background
            ...
            style={{
              backgroundColor: currentTheme === "dark" ? "#1e293b" : "#f9fafb",
            }}
          />
        )}
