...
export const useWorkflowEdges = ({ workflowId }: UseWorkflowEdgesProps) => {
  ...
  const workflowSources = useAppSelector((state) =>
      ...
      if (node.inputs) {
        node.inputs.forEach((input, inputIndex) => {
          if (input.source_broker_id) {
            ...
            const sourceInputNode = workflowSources?.find(
              (source) => source.brokerId === input.source_broker_id,
            );
            ...
            if (sourceInputNode) {
              ...
              if (sourceNode) {
                ...
                edges.push({
                  ...
                  target: node.id,
                  sourceHandle:
                    sourceOutput?.broker_id || `output-${outputIndex}`,
                  targetHandle: targetHandle,
                  type: "smoothstep",
                  animated: false,
                  style: {
                    ...
                    targetNode.inputs.forEach((input, inputIndex) => {
                      if (input.source_broker_id === relay.id) {
                        ...
                        edges.push({
                          ...
                          sourceHandle:
                            output.broker_id || `output-${outputIndex}`,
                          targetHandle: targetHandle,
                          type: "smoothstep",
                          animated: false,
                          style: {
                            strokeWidth: 2,
                            stroke:
                              currentTheme === "dark" ? "#6b7280" : "#374151",
                          },
                          data: {
                    ...
                    if (input.source_broker_id === relay.id) {
                      ...
                      edges.push({
                        ...
                        style: {
                          strokeWidth: 2,
                          stroke:
                            currentTheme === "dark" ? "#3b82f6" : "#2563eb", // Blue color for source connections
                        },
                        data: {
