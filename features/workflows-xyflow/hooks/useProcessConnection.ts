...
export function useProcessConnection({
  workflowId,
  handleSave,
}: UseProcessConnectionProps) {
  const dispatch = useAppDispatch();
  const allWorkflowNodes = useAppSelector((state) =>
    workflowNodesSelectors.nodesByWorkflowId(state, workflowId),
  );
  const allWorkflowSources = useAppSelector((state) =>
    workflowsSelectors.workflowSources(state, workflowId),
  );
  ...
  const processConnectionToBroker = useCallback(
    (targetHandleId: string, sourceOutput: Output): Output => {
      const relay: Relay = {
        type: "broker",
        id: targetHandleId,
      };
      ...
      const relayIndex = existingRelays.findIndex(
        (r) => r.id === targetHandleId,
      );
      ...
      if (relayIndex !== -1) {
        ...
        updatedRelays = [
          ...existingRelays.slice(0, relayIndex),
          relay,
          ...existingRelays.slice(relayIndex + 1),
        ];
      } else {
        // Add the new relay
  ...
  const processSourceConnectionToBroker = useCallback(
    (
  ...
  const processConnectionToArgument = useCallback(
    (
  ...
  const handleProcessConnection = useCallback(
    (connection: Connection) => {
      ...
      const targetType =
        firstDashIndex > -1
          ? targetHandle.substring(0, firstDashIndex)
          : targetHandle;
      const targetHandleId =
        firstDashIndex > -1 ? targetHandle.substring(firstDashIndex + 1) : "";
      ...
      if (targetType === "argument") {
        ...
        const updatedInput = processConnectionToArgument(
          sourceHandleId,
          targetHandleId,
          workflowId,
        );
        dispatch(
          workflowNodesActions.addInput({
            id: targetNodeId,
            input: updatedInput,
          }),
        );
        ...
        const matchingSource = allWorkflowSources.find(
          (source) => source.brokerId === sourceHandleId,
        );
        if (matchingSource) {
          const updatedSource = processSourceConnectionToBroker(
            targetHandleId,
            matchingSource,
          );
          dispatch(
          ...
          console.error(
            `Source node ${sourceNodeId} not found in workflow sources`,
          );
        }
      } else if (targetType === "broker") {
        // Update the source node's output by adding a relay
        const updatedOutput = processConnectionToBroker(
          targetHandleId,
          sourceOutput,
        );
        dispatch(
          workflowNodesActions.addOrUpdateOutput({
            id: sourceNodeId,
      ...
      return {
        connectionType:
          targetType === "broker" ? "broker_relay" : "argument_mapping",
        sourceNode: {
          id: sourceNodeId,
        ...
        ...(targetType === "broker"
          ...
          : {
              targetInput: processConnectionToArgument(
                sourceHandleId,
                targetHandleId,
                workflowId,
              ),
    ...
    [
      nodeMap,
      outputMap,
      processConnectionToBroker,
      processConnectionToArgument,
      workflowId,
      dispatch,
      allWorkflowSources,
      handleSave,
