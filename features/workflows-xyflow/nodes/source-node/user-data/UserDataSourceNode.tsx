...
import type { BrokerSourceConfig } from "@/lib/redux/workflow/types";
import { useDataBrokerWithFetch } from "@/lib/redux/entity/hooks/entityUsedHooks";
import {
  BaseNode,
  NodeConfig,
  BaseNodeData,
} from "@/features/workflows-xyflow/nodes/base/BaseNode";
...
const UserDataCompactContent: React.FC = () => {
  ...
  const userDataSource = useAppSelector((state) =>
    data.brokerId
      ? workflowsSelectors.userDataSourceByBrokerId(
          state,
          data.workflowId || "",
          data.brokerId,
        )
...
const UserDataSourceNodeComponent: React.FC<UserDataSourceNodeProps> = ({
  data,
  ...nodeProps
}) => {
  ...
  ) as BrokerSourceConfig<"user_data">;
  ...
  const brokerDisplayName = brokerId
    ? dataBrokerRecordsById[brokerId]?.name || brokerId
    : "Unknown";
  const selectedTable = userDataSource?.sourceDetails;
  const sourceDataType = selectedTable?.type as
    | "table_row"
    | "table_column"
    | "table_cell"
    | "full_table";
