...
const UserDataSourceSettings: React.FC<UserDataSourceSettingsProps> = ({
  ...
  const existingSource = useAppSelector((state) =>
    brokerId
      ? workflowsSelectors.userDataSourceByBrokerId(state, workflowId, brokerId)
      : null,
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const mapEntry: BrokerMapEntry = {
        ...
        mappedItemId:
          formData.selectedReference.table_id ||
          formData.selectedReference.table_name,
        ...
        if (
          ...
          dispatch(
            brokerActions.removeRegisterEntry({
              source: existingSource.scope || "workflow",
              mappedItemId:
                (existingSource.sourceDetails as UserDataReference).table_id ||
                (existingSource.sourceDetails as UserDataReference).table_name,
            }),
          );
        }
      }
  ...
  const renderReferenceDetails = (reference: UserDataReference) => {
    ...
    if (reference.column_display_name) {
      details.push({
        ...
        value: reference.column_display_name,
      });
    }
    ...
    details.push({
      label: "Description",
      value: reference.description || "No description",
    });
  ...
  const hasChanges =
    ...
    !existingSource.sourceDetails ||
    formData.brokerId !== existingSource.brokerId ||
    JSON.stringify(formData.selectedReference) !==
      JSON.stringify(existingSource.sourceDetails) ||
    formData.source !== (existingSource.scope || "workflow") ||
    formData.sourceId !== workflowId;

  // Get broker display name
  const brokerDisplayName =
    (brokerId && dataBrokerRecordsById[brokerId]?.name) || brokerId;
      ...
      <DialogContent className="sm:max-w-5xl aria-describedby={undefined}">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <DialogDescription className="sr-only">
              Manage data sources for this workflow.
            </DialogDescription>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="text-sm text-muted-foreground">
            Configure the data table to use as a source for this workflow. The
            selected table's data will be available to workflow nodes.
          </div>
              ...
              <div className="flex flex-col items-center gap-3 p-6 bg-background border-2 border-dashed border-muted-foreground/30 rounded-2xl">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Choose table, row, column, or cell
                  </div>
                </div>
                <TableReferenceIcon
