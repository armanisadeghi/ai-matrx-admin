...
import {
  ...
  generateNodeInputs,
} from "@/features/workflows-xyflow/utils";
...
import { SectionTable } from "@/features/workflows-xyflow/common/SectionTable";
import { DefaultTabProps } from "./types";
import { toTitleCase } from "@/utils/dataUtils";

const InputsTab: React.FC<DefaultTabProps> = ({ nodeId }) => {
  const inputs = useAppSelector((state) =>
    workflowNodesSelectors.nodeInputs(state, nodeId),
  );
  const nodeData = useAppSelector((state) =>
    workflowNodesSelectors.nodeById(state, nodeId || ""),
  );
  ...
  const handleResetToDefaults = () => {
    ...
    const preservedInputs =
      inputs?.filter((input) => input.metadata?.allow_reset === false) || [];
  ...
  const renderValueInput = (input) => {
      ...
      return (
        <div className="space-y-1">
          <label className="text-xs text-muted-foreground">
            {isArgMapping ? "Mapping ID" : "Broker ID"}
          </label>
          <Input
            value={input.default_value || ""}
            onChange={(e) =>
              handleMappingValueChange(input.arg_name, e.target.value)
            }
    ...
    return (
      <DataTypeInput
        value={input.default_value}
        onChange={(newValue) =>
          handleDefaultValueChange(input.arg_name, newValue)
        }
  ...
  const visibleInputs = inputs.filter(
    (input) => !hiddenArgNames.includes(input.arg_name),
  );
              ...
              {visibleInputs.map((input, index) => {
                ...
                return (
                  <div
                    ...
                    className="grid grid-cols-5 gap-4 px-3 py-2 items-center hover:bg-muted/50 rounded border-b last:border-none min-h-[40px]"
                  >
                    {/* Input Name */}
                    ...
                    <div className="flex justify-center items-center">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          handleSystemDefaultToggle(input.arg_name, checked)
                        }
                        className="scale-75"
                      />
                    </div>
                    ...
                    <div className="flex justify-center items-center h-full">
                      <Select
                        value={input.type}
                        onValueChange={(value) =>
                          handleSourceChange(input.arg_name, value)
                        }
                        disabled={useSystemDefault}
                      >
                        <SelectTrigger
                          className={`h-7 w-full text-xs bg-transparent border border-border ${useSystemDefault ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                    ...
                    <div className="flex justify-center items-center">
                      <Switch
                        checked={setManually}
                        onCheckedChange={(checked) =>
                          handleSetManuallyToggle(input.arg_name, checked)
                        }
                        className="scale-75"
                      />
                    </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <Badge
                        variant={
                          input.metadata?.required ? "destructive" : "secondary"
                        }
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <Select
                        value={input.type}
                        onValueChange={(value) =>
                          handleTypeChange(input.arg_name, value)
                        }
                      >
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleReadyChange(input.arg_name, value)
                        }
                      >
