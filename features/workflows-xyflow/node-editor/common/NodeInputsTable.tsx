...
export const NodeInputsTable: React.FC<NodeInputsTableProps> = ({ nodeId }) => {
  ...
  const controlledNodeInputs = useAppSelector((state) =>
  ...
  const isInputControlled = useCallback(
    (inputName: string) => {
      return controlledNodeInputs.some(
        (controlledInput) => controlledInput.arg_name === inputName,
      );
    },
          ...
          <>
            ...
            <TableCell className="font-medium text-[10px] w-96 border-r border-border dark:border-border">
              <code className="px-2 py-1 rounded text-xs font-mono break-all">
                {input.arg_name || input.source_broker_id}
              </code>
            </TableCell>
