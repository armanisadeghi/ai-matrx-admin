...
const NodeSelectionOverlay: React.FC<NodeSelectionOverlayProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 border border-red-500"
    ...
    >
      <div className="bg-textured rounded-2xl shadow-2xl max-w-7xl w-full max-h-[80vh] overflow-hidden">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3 flex-1">
                      <div
                        className={`p-2 bg-${node.color}-50 dark:bg-${node.color}-900/30 rounded-lg border border-${node.color}-200 dark:border-${node.color}-700 group-hover:bg-${node.color}-100 dark:group-hover:bg-${node.color}-800/40 transition-all duration-200 flex-shrink-0`}
                      >
                        <DynamicIcon
                          name={node.icon}
                          color={node.color}
        ...
        {/* Footer */}
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-border">
