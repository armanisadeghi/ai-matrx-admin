...
export const DEFAULT_EXCLUDE_ARG_NAMES = [
  "recipe_brokers",
  "session_manager",
  "user_id",
  "stream_handler",
  "internal_config_object",
];
export const DEFAULT_HIDE_CONNECTIONS = ["recipe_id", "latest_version"];

export const ALL_HIDDEN_CONNECTIONS = [
  ...DEFAULT_HIDE_CONNECTIONS,
...
export function getRegisteredFunctions() {
  ...
  return cloneDeep(registeredFunctions).map((func) => ({
    ...
    args: func.args.filter(
      (arg) => !DEFAULT_EXCLUDE_ARG_NAMES.includes(arg.name),
    ),
  }));
}

// Helper function to get the effective value for an argument
export const getEffectiveArgValue = (
  arg: any,
...
export const updateArgOverride = (
  ...
  const existingIndex = updated.arg_overrides.findIndex(
    (override) => override.name === argName,
  );
  if (existingIndex >= 0) {
...
export const updateMultipleArgOverrides = (
  ...
  updates.forEach((update) => {
    const existingIndex = updated.arg_overrides.findIndex(
      (override) => override.name === update.argName,
    );
    ...
    if (existingIndex >= 0) {
      ...
      // Create new override
      const functionArg = functionData?.args.find(
        (arg) => arg.name === update.argName,
      );
      updated.arg_overrides.push({
        name: update.argName,
        default_value: update.value,
    ...
    if (inputValue && inputValue.trim() !== "") {
      try {
        ...
        if (parseResult.success) {
          ...
          console.warn(
            `Failed to parse JSON for ${arg.name}:`,
            parseResult.error,
          );
          value = inputValue;
        }
...
export const getBrokerMappingsForArg = (node: any, argName: string) => {
  return (
    node.arg_mapping?.filter((m: any) => m.target_arg_name === argName) || []
  );
};
