...
import {
  ...
  WorkflowNodeUiData,
  XyFlowNodeType,
} from "@/lib/redux/workflow-nodes/types";
...
function getRegisteredFunctions(): any[] {
  ...
  const brokers = Object.values(
    state.entities?.dataBroker?.records || {},
  ) as DataBrokerData[];

  return functions.map((func: any) => {
    const fullBroker =
      brokers.find((broker: any) => broker.id === func.returnBroker) ||
      func.return_broker;
    return {
      id: func.id,
      name: func.name,
...
export function generateNodeInputs(functionData: any): InputMapping[] {
  return functionData.args
    ...
    .map((arg) => ({
      type:
        arg.ready === false && arg.required === true
          ? "arg_mapping"
          : "arg_override",
      arg_name: arg.name,
...
export function resetNodePart(
  ...
  return {
    ...
    [part]:
      part === "inputs"
        ? generateNodeInputs(functionData)
        : generateNodeOutputs(functionData),
  };
}

export function generateInputsFromDefinition(
  nodeDefinition: any,
): InputMapping[] {
  return nodeDefinition.inputs
    ...
    .map((input: any) => {
      ...
      const matchingArg = nodeDefinition.arguments.find(
        (arg: any) => arg.name === input.name,
      );
      ...
      return {
        type:
          matchingArg.ready === false && matchingArg.required === true
            ? "arg_mapping"
            : "arg_override",
...
export function generateInputByName(
  nodeDefinition: any,
  argName: string,
): InputMapping {
  // Find the input with the matching name and input_type: "argument"
  const input = nodeDefinition.inputs.find(
    (input: any) => input.name === argName && input.input_type === "argument",
  );

  if (!input) {
    throw new Error(
      `No input found with name "${argName}" and input_type "argument"`,
    );
  }
...
export function generateInputsByNames(
  nodeDefinition: any,
  argNames: string[],
): InputMapping[] {
  ...
  return nodeDefinition.inputs
    .filter(
      (input: any) =>
        input.input_type === "argument" && argNames.includes(input.name),
    )
    .map((input: any) => {
      // Find the matching argument by name
      const matchingArg = nodeDefinition.arguments.find(
    ...
    .filter(
      (mapping: InputMapping | null): mapping is InputMapping =>
        mapping !== null,
    );
}
