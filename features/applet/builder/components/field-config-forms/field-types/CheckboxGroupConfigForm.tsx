...
import { CheckboxGroupFieldConfig, CheckboxOption } from '../../../../a-old-depricated-do-not-use/runner-depreciated-do-not-use/components/field-components/types';

interface CheckboxGroupConfigFormProps {
  config: Partial<CheckboxGroupFieldConfig>;
  onChange: (config: Partial<CheckboxGroupFieldConfig>) => void;
}

export const CheckboxGroupConfigForm: React.FC<CheckboxGroupConfigFormProps> = ({
  config,
  onChange
}) => {
  const [newOption, setNewOption] = useState<Partial<CheckboxOption>>({ id: '', value: '', label: '' });
  ...
  const handleAddOption = () => {
    if (newOption.value && newOption.label) {
      const newId = newOption.id || `option-${Date.now()}`;
      const option = { ...newOption, id: newId } as CheckboxOption;
  ...
  const handleSaveEdit = () => {
    if (editingIndex !== null && newOption.value && newOption.label) {
      ...
      const newId = newOption.id || `option-${Date.now()}`;
      options[editingIndex] = { ...newOption, id: newId } as CheckboxOption;
      onChange({ ...config, options });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            How checkbox options are arranged
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="minOptionWidth" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            Minimum Option Width (px)
          </Label>
          <Input
            id="minOptionWidth"
            name="minOptionWidth"
            type="number"
            placeholder="e.g. 180"
            value={config.minOptionWidth?.toString() || ''}
            onChange={handleChange}
            className="border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200"
          />
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Minimum width for each checkbox option
          </p>
        </div>
        
        <div className="space-y-2">
          <Label htmlFor="checkboxClassName" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            Checkbox Class Name
          </Label>
          <Input
            id="checkboxClassName"
            name="checkboxClassName"
            placeholder="e.g. rounded-md border-gray-300 dark:border-gray-600..."
            value={config.checkboxClassName || ''}
            onChange={handleChange}
            className="border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200"
          />
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Custom CSS classes for the checkboxes
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-zinc-200 dark:border-zinc-700 pt-6">
        <h3 className="text-lg font-medium text-zinc-800 dark:text-zinc-200 mb-4">Checkbox Options</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="space-y-2">
            ...
            <Input
              id="optionId"
              name="id"
              placeholder="e.g. option-1 (generated if empty)"
              value={newOption.id || ''}
