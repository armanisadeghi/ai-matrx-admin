...
import { 
  ...
  selectDoContainersMatch 
} from '@/lib/redux/app-builder/selectors/containerMatchSelectors';
...
export const ContainerComparisonModal: React.FC<ContainerComparisonModalProps> = ({
  ...
  const containersMatch = useSelector((state: RootState) =>
    selectDoContainersMatch(state, appletId, containerId)
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Detailed comparison between the applet container and the source container
          </DialogDescription>
        </DialogHeader>
        <ContainerComparisonDetails 
          appletId={appletId} 
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t dark:border-zinc-700">
          ...
          <Button
            variant="secondary"
            onClick={handleSetAsIdentical}
            disabled={!onSetAsIdentical}
          >
            Set as Identical
          </Button>
          <Button
...
const ContainerComparisonDetails: React.FC<ContainerComparisonModalProps> = ({
  appletId,
  containerId,
}) => {
  const comparisonResult = useSelector((state: RootState) =>
    selectContainerComparisonResult(state, appletId, containerId)
  );
  
  const applet = useSelector((state: RootState) =>
    selectAppletById(state, appletId)
  );
  
  const coreContainer = useSelector((state: RootState) =>
    selectContainerById(state, containerId)
  );
  ...
  const PropertyRow = ({ 
    ...
    const actualMatch = isMatch !== undefined 
      ? isMatch 
    ...
    return (
      <div className="grid grid-cols-3 gap-4 py-2 border-b dark:border-zinc-700 text-sm">
        <div>
          ...
          <div className={cn("mt-1", !actualMatch && "text-orange-600 dark:text-orange-400")}>
            {normalizedAppletValue}
          </div>
        </div>
        <div>
          <div className={cn("mt-1", !actualMatch && "text-blue-600 dark:text-blue-400")}>
            {normalizedCoreValue}
          </div>
        </div>
  ...
  const FieldDetails = ({ field, differences }: { 
    field: any; 
    differences: any;
  }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <StatusBadge 
          status={comparisonResult.coreContainerExists} 
          label="Core Container Exists" 
        />
        <StatusBadge 
          status={comparisonResult.appletExists} 
          label="Applet Exists" 
        />
        <StatusBadge 
          status={comparisonResult.containerExistsInApplet} 
          label="Container in Applet" 
        />
        <StatusBadge 
          status={comparisonResult.stringPropertiesMatch} 
          label="Properties Match" 
        />
        <StatusBadge 
          status={comparisonResult.fieldIdsMatch} 
          label="Field IDs Match" 
        />
        <StatusBadge 
          status={comparisonResult.fieldsAreSimilar} 
          label="Fields Similar" 
        />
      </div>
      ...
      <div className="space-y-1">
        <PropertyRow
          label="Label"
          appletValue={appletContainer?.label}
          coreValue={coreContainer?.label}
        />
        <PropertyRow
          label="Short Label"
          appletValue={appletContainer?.shortLabel}
          coreValue={coreContainer?.shortLabel}
        />
        <PropertyRow
          label="Description"
          appletValue={appletContainer?.description}
          coreValue={coreContainer?.description}
        />
        <PropertyRow
        ...
        <PropertyRow
          label="Help Text"
          appletValue={appletContainer?.helpText}
          coreValue={coreContainer?.helpText}
        />
      </div>
            ...
            <div>
              <div className="p-2 bg-white dark:bg-gray-700 rounded">
                ...
                <div className="text-2xl font-bold">
                  {(comparisonResult.details.similarityScore * 100).toFixed(0)}%
                </div>
              </div>
            </div>
                ...
                <div className="p-2 bg-red-50 dark:bg-red-950 dark:text-red-100 rounded">
                  ...
                  <div className="text-[10px] max-h-16 overflow-y-auto">
                    {comparisonResult.details.missingFieldIds.map((id) => (
                      <div key={id} className="ml-2 truncate" title={id}>• {id}</div>
                    ))}
                  </div>
                </div>
              )}
            ...
            <Tabs defaultValue={Object.keys(comparisonResult.details.fieldDifferences)[0]} className="w-full">
              <TabsList className="flex flex-wrap h-auto p-1">
                {Object.entries(comparisonResult.details.fieldDifferences).map(([fieldId, differences]) => {
                  const coreField = coreContainer?.fields?.find(f => f.id === fieldId);
                  const hasAnyDifference = !differences.component.match || 
