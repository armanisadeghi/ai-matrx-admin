...
import {
    selectIsBrokerMapped,
    selectBrokerMappedStatus,
    selectFieldLabelByBrokerId,
    selectFieldIdByBrokerId,
...
const BrokerCard = ({ broker, appletId, isSelected, onSelect }: BrokerCardProps) => {
    ...
    const fieldId = useAppSelector((state) => selectFieldIdByBrokerId(state, appletId, broker.id));
    const fieldLabel = useAppSelector((state) => selectFieldLabelByBrokerId(state, appletId, broker.id));
    ...
    return (
        ...
        >
            ...
            <div className="mt-1 text-xs">
                <div className="flex">
                    <span className="font-medium text-gray-700 dark:text-gray-300 w-28 min-w-28">Data Type:</span>
                    <span className="text-gray-900 dark:text-gray-100">{displayBrokerType(broker.dataType)}</span>
                </div>
                <div className="flex mt-1">
                    <span className="font-medium text-gray-700 dark:text-gray-300 w-28 min-w-28">Default Value:</span>
                    <span className="text-gray-900 dark:text-gray-100 truncate">
                        {displayValue(broker.defaultValue).substring(0, 40)}...
                    </span>
                </div>
                <div className="flex mt-1">
                    <span className="font-medium text-gray-700 dark:text-gray-300 w-28 min-w-28">Mapped Field:</span>
                    <span className="text-green-900 dark:text-green-100">{fieldLabel}</span>
                </div>
                <div className="flex justify-center mt-2">
                    <span className="text-xs text-amber-700 dark:text-amber-300">Hint: Set Default Components in the Cockpit</span>
                </div>
            </div>
        </div>
    );
