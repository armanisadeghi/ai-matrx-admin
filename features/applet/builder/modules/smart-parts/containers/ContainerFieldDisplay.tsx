...
const ContainerFieldDisplay: React.FC<ContainerFieldDisplayProps> = ({
    ...
    const handleMoveFieldUp = async (fieldId: string) => {
        ...
        // Log before state
        console.log("Before moveFieldUp:", {
            fieldId,
            containerFields: container?.fields.map(f => ({ id: f.id, label: f.label }))
        });
        
        try {
            await dispatch(moveFieldUpThunk({ containerId, fieldId })).unwrap();
            
            // Get updated container after the move
            const updatedContainer = selectContainerById(
                store.getState(),
                containerId
            );
    ...
    const handleMoveFieldDown = async (fieldId: string) => {
        ...
        // Log before state
        console.log("Before moveFieldDown:", {
            fieldId,
            containerFields: container?.fields.map(f => ({ id: f.id, label: f.label }))
        });
        ...
        try {
            ...
            // Log after state
            console.log("After moveFieldDown:", {
                fieldId,
                containerFields: updatedContainer?.fields.map(f => ({ id: f.id, label: f.label }))
            });
        } catch (error) {
