 ...
 */
const SmartContainerList = forwardRef<
  SmartContainerListRefType,
  {
  ...
  (
    ...
    const filteredGroups = React.useMemo(() => {
      ...
      switch (sortBy) {
        ...
        case "fields-asc":
          result.sort(
            (a, b) => (a.fields?.length || 0) - (b.fields?.length || 0),
          );
          break;
        case "fields-desc":
          result.sort(
    ...
    const refreshRef = useRef<
      (specificGroupIds?: string[]) => Promise<ComponentGroup[]>
    >(null!);
    ...
    const getContainerIcon = (groupId: string) => {
      ...
      const getColorClasses = (colorName: string) => {
        switch (colorName) {
          ...
          case "violet":
            return {
              bg: "bg-violet-100 dark:bg-violet-800/30",
          ...
          case "fuchsia":
            return {
              bg: "bg-fuchsia-100 dark:bg-fuchsia-800/30",
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  onClick={() => handleSortChange("label-desc")}
                >
                  Name (Z-A)
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => handleSortChange("fields-desc")}
                >
                  Most Fields
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => handleSortChange("fields-asc")}
                >
                  Least Fields
                </DropdownMenuItem>
              </DropdownMenuContent>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {viewMode === "grid" && (
                        <div
                          className={`h-24 w-full flex items-center justify-center ${iconData.colorClasses.bg}`}
                        >
                          <div
                            className={`h-12 w-12 ${iconData.colorClasses.text}`}
                          >
                      ...
                      <div className={viewMode === "list" ? "flex-1" : ""}>
                        <CardHeader className="pb-2">
                          <div className="flex items-center space-x-2">
                            {viewMode === "list" && (
                              <div
                                className={`p-1.5 rounded-md ${iconData.colorClasses.bg}`}
                              >
                                <IconComponent
                                  className={`h-5 w-5 ${iconData.colorClasses.text}`}
                                />
                              </div>
                            )}
                            <CardTitle
                              className={`text-lg font-medium truncate ${colorClasses.titleClass}`}
                            >
                        ...
                        <CardContent className="pt-0">
                          <div
                            className={`text-sm ${colorClasses.descriptionClass}`}
                          >
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-2">
                              {group.isPublic && (
                                <Badge
                                  variant="outline"
                                  className="text-xs px-1.5 py-0 h-5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800"
                                >
                                  Public
                                </Badge>
                              )}
