...
// import { tableReducers, tableSelectors } from "@/lib/redux/brokerSlice/types/tables";
// import { ensureValidWidthClass } from "@/features/applet/constants/field-constants";
// import { GripHorizontal, Plus, X } from "lucide-react";
// import { cn } from "@/lib/utils";
// import { FieldDefinition } from "@/types/customAppTypes";

// const { setBrokerTable, updateTableCell, addTableRow, deleteTableRow, addTableColumn, deleteTableColumn, renameTableColumn, reorderTableRows, reorderTableColumns } = tableReducers;
// const { selectBrokerTable, selectSortedTableRows, selectSortedTableColumns } = tableSelectors;
...
//     const idArgs = { source, sourceId: appletId, itemId: fieldId };

//     // Redux state
//     const stateValue = useAppSelector((state) => selectBrokerValue(state, source, fieldId));
//     const table = useAppSelector((state) => selectBrokerTable(state, idArgs));
//     const sortedRows = useAppSelector((state) => selectSortedTableRows(state, idArgs));
//     const sortedColumns = useAppSelector((state) => selectSortedTableColumns(state, idArgs));
...
//             initialTable = { columns: [], rows: [] };
//         }
//         dispatch(setBrokerTable({ idArgs, table: initialTable }));
...
//             dispatch(reorderTableRows({ idArgs, rowIds: newRows.map(row => row.id) }));
//         } else if (type === 'COLUMN') {
...
//             dispatch(reorderTableColumns({ idArgs, columnIds: newColumns.map(col => col.id) }));
//             requestAnimationFrame(measureColumnWidths);
//         }
//     };

//     const handleAddRow = () => dispatch(addTableRow({ idArgs }));
//     const handleDeleteRow = (rowId: string) => dispatch(deleteTableRow({ idArgs, rowId }));
//     const handleAddColumn = () => {
//         const timestamp = Date.now();
//         dispatch(addTableColumn({
//             idArgs,
...
//         if (!column?.isFixed) {
//             dispatch(deleteTableColumn({ idArgs, columnId }));
//             requestAnimationFrame(measureColumnWidths);
//         }
//     };
...
//         if (newName.trim()) {
//             dispatch(renameTableColumn({ idArgs, columnId, newName }));
//         }
...
//     const saveEdit = () => {
//         if (editingCell) {
//             dispatch(updateTableCell({ idArgs, ...editingCell, value: editValue }));
//             setEditingCell(null);
//             setEditValue("");
//         }
//     };
...
//         const totalWidth = columnWidths.length === (1 + sortedColumns.length + 1)
...
//                             <td key={col.id} style={{ width: columnWidths[index + 1] }} className="p-3 border-r border-gray-200 dark:border-gray-700">
//                                 <span className={cn(col.id === 'label' ? "font-medium text-gray-900 dark:text-gray-100" : "text-gray-700 dark:text-gray-300")}>
//                                     {row.cells[col.id] ?? '-'}
//                                 </span>
//                             </td>
//                         ))}
...
//                                                             style={{ ...providedCol.draggableProps.style, width: snapshotCol.isDragging ? columnWidths[index + 1] : undefined }}
//                                                             className={cn(
//                                                                 "text-left p-0 relative group", col.minWidthClass,
//                                                                 "border-r border-gray-200 dark:border-gray-700",
//                                                                 !col.isFixed && !disabled && !isEditing && "cursor-grab hover:bg-gray-100 dark:hover:bg-gray-700/50",
//                                                                 snapshotCol.isDragging && "bg-blue-100 dark:bg-blue-900 shadow-md opacity-95 z-20"
//                                                             )}
//                                                         >
...
//                                                                         onKeyDown={(e) => {
//                                                                             if (e.key === 'Enter') handleColumnRename(col.id, columnEditValue);
...
//                                                                     />
//                                                                 ) : (
//                                                                     <div className="flex items-center space-x-2 flex-grow min-w-0" onClick={() => handleStartRenameColumn(col.id)}>
...
//                                                 disabled={disabled}
//                                                 className={cn("h-full w-full flex items-center justify-center p-2", !disabled && "hover:bg-gray-100 dark:hover:bg-gray-700", disabled && "opacity-50 cursor-not-allowed")}
//                                             >
...
//                                 <tbody ref={providedRows.innerRef} {...providedRows.droppableProps} className={cn(snapshotRows.isDraggingOver && "bg-blue-50/50 dark:bg-blue-950/20")}>
//                                     {sortedRows.length === 0 && !disabled ? (
//                                         <tr><td colSpan={sortedColumns.length + 2} className="p-4 text-center text-sm text-gray-500 dark:text-gray-400">No rows defined. Click 'Add Row'.</td></tr>
//                                     ) : (
//                                         sortedRows.map((row, index) => (
//                                             <Draggable key={row.id} draggableId={`row-${fieldId}-${row.id}`} index={index} isDragDisabled={disabled}>
...
//                                                             <div {...providedRow.dragHandleProps} className={cn("h-full flex items-center justify-center p-2", !disabled && "cursor-grab hover:bg-gray-100 dark:hover:bg-gray-700")}>
//                                                                 <GripHorizontal className="h-4 w-4 text-gray-400 dark:text-gray-500" />
//                                                             </div>
//                                                         </td>
...
//                                                                         onBlur={saveEdit}
//                                                                         autoFocus
//                                                                         className={cn("w-full h-full px-3 py-2 text-sm border-0 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 bg-inherit", col.id === 'label' ? "font-medium text-gray-900 dark:text-gray-100" : "text-gray-700 dark:text-gray-300")}
//                                                                     />
//                                                                 ) : (
//                                                                     <span
//                                                                         onClick={() => handleCellClick(row.id, col.id, row.cells[col.id])}
//                                                                         className={cn("block w-full h-full px-3 py-2 text-sm cursor-pointer overflow-hidden text-ellipsis whitespace-nowrap", col.id === 'label' ? "font-medium text-gray-900 dark:text-gray-100" : "text-gray-700 dark:text-gray-300")}
//                                                                     >
//                                                                         {row.cells[col.id] ?? '-'}
//                                                                     </span>
//                                                                 )}
...
//                                                                 disabled={disabled}
//                                                                 className={cn("h-full w-full flex items-center justify-center p-2", !disabled && "hover:bg-red-100 dark:hover:bg-red-900/50", disabled && "opacity-50 cursor-not-allowed")}
//                                                             >
...
//                         disabled={disabled}
//                         className={cn("px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300", "hover:bg-gray-200 dark:hover:bg-gray-700", "border border-gray-300 dark:border-gray-600 rounded-md text-sm", disabled && "opacity-50 cursor-not-allowed", "flex items-center space-x-1.5")}
//                     >
