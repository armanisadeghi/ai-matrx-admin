...
interface ModelPricingEditorProps {
    ...
    onSave: (tiers: PricingTier[]) => Promise<void>;
}
...
export default function ModelPricingEditor({ tiers, onChange, onSave }: ModelPricingEditorProps) {
        ...
        <div className="space-y-4">
            ...
            {tiers.length === 0 && (
                <div className="rounded-md border border-dashed p-6 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">No pricing configured for this model.</p>
                    <p className="text-xs text-muted-foreground/70 mt-1">
                        Click &quot;Add Tier&quot; to define input/output token prices (per million tokens).
                    </p>
                </div>
            )}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                {i === tiers.length - 1 && hasMultipleTiers && (
                                    <Badge variant="outline" className="text-xs h-4 px-1">highest</Badge>
                                )}
                            </div>
                            <Button
                                variant="ghost"
            ...
            {hasMultipleTiers && (
                <div className="rounded-md bg-muted/50 border p-2.5 text-xs text-muted-foreground space-y-1">
                    ...
                    <p>
                        Tiers apply based on the prompt context length. The last tier (max_tokens = null) covers
                        all prompts above the previous threshold. Order tiers from smallest to largest.
                    </p>
                </div>
            )}
