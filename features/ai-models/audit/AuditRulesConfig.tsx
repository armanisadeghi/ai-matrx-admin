...
import { RotateCcw, Settings2, DollarSign, Cpu, Zap, FileText } from 'lucide-react';
import type { AuditRuleConfig, CapabilityKey } from './auditTypes';
import { DEFAULT_AUDIT_RULES, ALL_CAPABILITY_KEYS, CAPABILITY_LABELS, CAPABILITY_GROUPS } from './auditTypes';
...
export default function AuditRulesConfig({ rules, onChange }: AuditRulesConfigProps) {
    const set = <K extends keyof AuditRuleConfig>(key: K, value: AuditRuleConfig[K]) =>
        ...
        <div className="flex flex-col h-full min-h-0">
            <div className="flex items-center justify-between px-4 py-2.5 border-b shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold">Audit Rules Configuration</span>
                </div>
            ...
            <div className="flex-1 overflow-auto p-4 space-y-6">
                {/* Core Fields */}
                <section>
                    <SectionHeader icon={<FileText className="h-4 w-4" />} title="Core Fields" />
                ...
                {/* Capabilities */}
                <section>
                    <SectionHeader icon={<Zap className="h-4 w-4" />} title="Capabilities" />
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            <Label className="text-xs whitespace-nowrap">Minimum capabilities set to true</Label>
                            <Input
                                type="number"
                                min={0}
                                value={rules.capabilities_min_true}
                                onChange={(e) => set('capabilities_min_true', parseInt(e.target.value) || 0)}
                        ...
                        <div>
                            <Label className="text-xs block mb-2">Required capabilities (must be set to true)</Label>
                                        ...
                                        <div className="grid grid-cols-3 gap-1.5">
                                            {keys.map((k) => (
                                                <CapabilityCheckbox
                                                    ...
                                                    checked={rules.capabilities_required_keys.includes(k as CapabilityKey)}
                                                    onChange={(v) => toggleRequiredCap(k as CapabilityKey, v)}
                                                />
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {rules.capabilities_required_keys.length > 0 && (
                                <p className="text-[11px] text-muted-foreground mt-1.5">
                                    ...
                                    {rules.capabilities_required_keys.map((k) => CAPABILITY_LABELS[k]).join(', ')}
                                </p>
                            )}
                        </div>
                    </div>
