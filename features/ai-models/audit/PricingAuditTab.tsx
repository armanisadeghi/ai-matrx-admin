...
import { Plus, Trash2, Save, Loader2, CheckCircle2, DollarSign } from 'lucide-react';
import { aiModelService } from '../service';
import type { AiModelRow, PricingTier } from '../types';
...
function emptyTier(): PricingTier {
    return { max_tokens: null, input_price: 0, output_price: 0, cached_input_price: 0 };
}
...
function InlinePricingEditor({
    ...
    const [tiers, setTiers] = useState<PricingTier[]>(model.pricing ?? [emptyTier()]);
    ...
    const handleSave = async () => {
        ...
        setError(null);
        try {
            await aiModelService.patchField(model.id, 'pricing', tiers as unknown as AiModelRow['pricing']);
            onSaved(tiers);
        } catch (err) {
            ...
            {tiers.map((tier, i) => (
                <div key={i} className="flex items-end gap-2 flex-wrap">
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[10px] text-muted-foreground">Input $/1M</span>
                        <Input
                            type="number" step="0.0001" min="0"
                            value={tier.input_price}
                            onChange={(e) => updateTier(i, { input_price: parseFloat(e.target.value) || 0 })}
                            className="h-7 text-xs w-24 font-mono"
                        />
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[10px] text-muted-foreground">Output $/1M</span>
                        <Input
                            type="number" step="0.0001" min="0"
                            value={tier.output_price}
                            onChange={(e) => updateTier(i, { output_price: parseFloat(e.target.value) || 0 })}
                            className="h-7 text-xs w-24 font-mono"
                        />
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[10px] text-muted-foreground">Cached $/1M</span>
                        <Input
                            type="number" step="0.0001" min="0"
                            value={tier.cached_input_price}
                            onChange={(e) => updateTier(i, { cached_input_price: parseFloat(e.target.value) || 0 })}
                            className="h-7 text-xs w-24 font-mono"
                        />
                    </div>
                    <div className="flex flex-col gap-0.5">
                        ...
                        <Input
                            ...
                            onChange={(e) => {
                                ...
                                updateTier(i, { max_tokens: raw === '' ? null : parseInt(raw) });
                            }}
                            className="h-7 text-xs w-24 font-mono"
                        />
                    </div>
...
export default function PricingAuditTab({ results, allModels, onModelUpdated }: PricingAuditTabProps) {
                    ...
                    return (
                        <React.Fragment key={model.id}>
                            <tr className={`h-10 border-b border-border ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                                ...
                                <td className="px-3 py-1.5">
                                    {pricing.length === 0 ? (
                                        <span className="text-destructive/70 text-xs font-medium">Not set</span>
                                    ) : (
                                        <div className="flex flex-col gap-0.5">
                                            {pricing.map((t, i) => (
                                                <span key={i} className="text-[10px] font-mono text-muted-foreground whitespace-nowrap">
                                                    in {formatPrice(t.input_price)} · out {formatPrice(t.output_price)}
                                                    {t.max_tokens != null && ` · ≤${t.max_tokens.toLocaleString()}`}
                                                </span>
                                            ))}
                                        </div>
                                    )}
