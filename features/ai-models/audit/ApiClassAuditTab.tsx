...
export default function ApiClassAuditTab({ results, allModels, onModelUpdated }: ApiClassAuditTabProps) {
    ...
    const getEditValue = (model: AiModelRow) =>
        editValues[model.id] !== undefined ? editValues[model.id] : (model.api_class ?? '');
    ...
    const saveSingle = async (model: AiModelRow) => {
        ...
        try {
            ...
            setEditValues((prev) => { const next = { ...prev }; delete next[model.id]; return next; });
        } catch (err) {
            ...
            <div className="flex flex-col h-full min-h-0">
                ...
                >
                    {displayResults.map((r, idx) => {
                        ...
                        return (
                            <tr key={model.id} className={`h-10 border-b border-border ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        {existingClasses.length > 0 && (
                                            <Select
                                                ...
                                                onValueChange={(v) => {
                                                    if (v !== '__none__') {
                                                        setEditValues((prev) => ({ ...prev, [model.id]: v }));
                                                    }
                                                }}
                                            >
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="__none__">Pick existing…</SelectItem>
                                        ...
                                        <Input
                                            value={editVal}
                                            onChange={(e) =>
                                                setEditValues((prev) => ({ ...prev, [model.id]: e.target.value }))
                                            }
