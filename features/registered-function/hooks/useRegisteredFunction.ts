...
export const useRegisteredFunction = () => {
  ...
  const fetchOne = useCallback(
    (args: FetchOneThunkArgs) => dispatch(apiThunks.fetchOne(args)),
    [dispatch],
  );
  const fetchPaginated = useCallback(
    (args: FetchPaginatedThunkArgs) => dispatch(apiThunks.fetchPaginated(args)),
    [dispatch],
  );
  const deleteOne = useCallback(
    (args: DeleteOneThunkArgs) => dispatch(apiThunks.deleteOne(args)),
    [dispatch],
  );
  const deleteMany = useCallback(
    (args: DeleteManyThunkArgs) => dispatch(apiThunks.deleteMany(args)),
    [dispatch],
  );
  const update = useCallback(
    (args: UpdateThunkArgs) => dispatch(apiThunks.update(args)),
    [dispatch],
  );
  const create = useCallback(
    (args: CreateThunkArgs) => dispatch(apiThunks.create(args)),
    [dispatch],
  );
