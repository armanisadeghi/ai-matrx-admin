      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-2xl">
            ...
            Upload a document, bind it to a data store, and any agent
            scoped to that store can search it.
          </p>
        </header>
        ...
        <section className="border rounded-md bg-muted/20 p-4">
          ...
          <ol className="text-sm text-muted-foreground space-y-1.5 list-decimal pl-5">
            <li>
              <strong className="text-foreground">Add a document:</strong>{" "}
              go to{" "}
              <Link href="/rag/data-stores" className="underline">
                Data Stores
              </Link>{" "}
              → drag a PDF onto a store → it uploads, processes, chunks,
            ...
            <li>
              ...
              click any row in the library — pages, chunks, embeddings,
              and store-bindings are all there.
            </li>
            <li>
              <strong className="text-foreground">Test retrieval:</strong>{" "}
              use{" "}
              <Link href="/rag/search" className="underline">
                Search
              </Link>{" "}
              with a data-store filter to see exactly what an agent
              would retrieve.
            </li>
          </ol>
        </section>
  ...
  return (
    <div className={`rounded-md border p-3 flex flex-col gap-1 ${toneClass}`}>
      ...
      <span className="font-semibold text-xl tabular-nums">
        {loading ? <Skeleton className="h-6 w-12" /> : (value ?? 0).toLocaleString()}
      </span>
    </div>
  );
}
