...
| File | Role |
| --- | --- |
| `components/LibraryPage.tsx` | Top-level page; consumes everything below. |
| `components/ProcessingJobView.tsx` | Reusable rich live-job visualization (used by both surfaces). |
| `components/StageAnimations.tsx` | Per-stage animated heroes + stage metadata (`STAGE_META`). |
| `components/ProcessingProgressSheet.tsx` | Right-side multi-job sheet (900px wide). |
| `components/ActiveJobsStrip.tsx` | Inline strip of running jobs. |
| `components/AnimatedKpiCard.tsx` | Animated rollup tile (count-up + tone). |
| `components/StatusBadge.tsx` | Status pill (unchanged). |
| `components/StageStatusPills.tsx` | 6-stage pills used in `LibraryDocDetailSheet`. |
| `components/LibraryDocDetailSheet.tsx` | Per-doc drilldown sheet; renders `<ProcessingJobView/>` inline in its Stages tab when there's an active job for the open doc. |
| `components/QuickSearchDialog.tsx` | Search-inside-doc dialog. |
| `components/ProcessingProgressDialog.tsx` | **Legacy** full-screen dialog. Retained for `IngestProgressDialog` / Files surface. Do not extend. |
...
| `types.ts` | `LibraryDocSummary`, `LibraryDocDetail`, `LibrarySummary`, `DocStatus`. |
