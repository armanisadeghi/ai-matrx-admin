...
import { SeoMetaTagsInline, SeoMetaTagsOverlay } from "../renderers/seo-meta-tags";
...
import { WebResearchInline, WebResearchOverlay } from "../renderers/web-research";
import { CoreWebSearchInline, CoreWebSearchOverlay } from "../renderers/core-web-search";
import { DeepResearchInline, DeepResearchOverlay } from "../renderers/deep-research";
...
import { resultAsObject, resultAsString, getArg, collectMessages, filterStepEvents } from "../renderers/_shared";
import {
...
function seoMetaTagsHeaderExtras(entry: ToolLifecycleEntry): React.ReactNode {
    ...
    const batch = result.batch_analysis as Array<{ overall_ok: boolean }> | undefined;
...
function seoTitlesHeaderExtras(entry: ToolLifecycleEntry): React.ReactNode {
    const result = resultAsObject(entry);
    if (!result) return null;
    const analysis = result.title_analysis as Array<{ title_ok: boolean }> | undefined;
...
function seoDescriptionsHeaderExtras(entry: ToolLifecycleEntry): React.ReactNode {
    const result = resultAsObject(entry);
    if (!result) return null;
    const analysis = result.description_analysis as Array<{ description_ok: boolean }> | undefined;
    if (!analysis) return null;
    const total = (result.count as number | undefined) ?? analysis.length;
...
export const toolRendererRegistry: ToolRegistry = {
    web_search: {
        ...
        OverlayComponent: (props: ToolRendererProps) => {
            ...
            const adapted = { type: "brave_default_page" as const, content: first.metadata };
    ...
    news_get_headlines: {
        ...
        getHeaderExtras: (entry) => {
            const result = resultAsObject(entry);
            const totalResults = typeof result?.total_results === "number" ? (result.total_results as number) : undefined;
    ...
    web_search_v1: {
        ...
        getHeaderExtras: (entry, events) => {
            const queries = getArg<unknown[]>(entry, "queries");
            const query = getArg<string>(entry, "query");
            const queryCount = Array.isArray(queries) ? queries.length : typeof query === "string" ? 1 : 0;
            const messages = collectMessages(events);
            const browsingCount = messages.filter((m) => m.startsWith("Browsing ")).length;
...
export function getOverlayRenderer(toolName: string | null): React.ComponentType<ToolRendererProps> {
    ...
    if (toolRendererRegistry[toolName]) {
        ...
        return renderer.OverlayComponent ?? renderer.InlineComponent ?? GenericRenderer;
    }
...
export function getToolDisplayName(toolName: string | null): string {
    ...
    if (toolRendererRegistry[toolName]?.displayName) return toolRendererRegistry[toolName].displayName;
    const dynamic = getCachedRenderer(toolName);
