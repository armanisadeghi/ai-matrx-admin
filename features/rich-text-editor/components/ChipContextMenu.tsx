            ...
            <ContextMenuSub>
                ...
                <ContextMenuPortal>
                    <ContextMenuSubContent className='w-48'>
                        <ContextMenuItem onSelect={() => handleAction('color-blue')}>Blue</ContextMenuItem>
                        <ContextMenuItem onSelect={() => handleAction('color-green')}>Green</ContextMenuItem>
                        <ContextMenuItem onSelect={() => handleAction('color-red')}>Red</ContextMenuItem>
                    </ContextMenuSubContent>
                </ContextMenuPortal>
            </ContextMenuSub>
...
export const ChipMenuProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const showMenu = useCallback((editorId: string, chipId: string, x: number, y: number) => {
        setActiveChip({ chipId, editorId, position: { x, y } });
    }, []);
            ...
            {activeChip?.position && (
                <ContextMenu>
                    ...
                    <ContextMenuContent
                        ...
                        style={{
                            position: 'absolute',
                            left: `${activeChip.position.x}px`,
                            top: `${activeChip.position.y}px`,
                        }}
                    >
                        <MenuItems />
                    </ContextMenuContent>
