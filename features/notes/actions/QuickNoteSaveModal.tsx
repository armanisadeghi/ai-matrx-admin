 ...
 */
export function QuickNoteSaveModal({
  open,
  onOpenChange,
  initialContent,
  defaultFolder = "Scratch",
  onSaved,
}: QuickNoteSaveModalProps) {
  ...
  const { notes, isLoading: isLoadingNotes, refreshNotes } = useNotes();

  // Get all unique folders from user's notes; always include defaultFolder so the Select value is valid
  const allFolders = useMemo(() => {
    ...
    const d = defaultFolder?.trim();
    if (d) folderSet.add(d);
    return Array.from(folderSet).sort();
  }, [notes, defaultFolder]);

  // Materialize new folders (schema: folder exists iff ≥1 note) so pickers stay in sync
  useEffect(() => {
    if (!open) return;
    const name = defaultFolder?.trim();
    if (!name) return;
    ...
    (async () => {
      try {
        ...
        if (!cancelled) await refreshNotes();
      } catch (e) {
        console.error("QuickNoteSaveModal: ensureFolderMaterialized", e);
        toast.error("Could not prepare folder. You can still try saving.");
      }
    })();
  ...
  }, [open, defaultFolder, refreshNotes, toast]);
  ...
  const executeSave = async () => {
    ...
    try {
      if (saveMode === "create") {
        ...
        const finalContent =
          updateMethod === "append" && selectedNote
        ...
        const methodText =
          updateMethod === "append" ? "appended to" : "overwrote";
        toast.success(
          `Content ${methodText} ${selectedNote?.label || "note"}!`,
        );
      }
  ...
  const isSaveDisabled =
    isSaving || !content.trim() || (saveMode === "update" && !selectedNoteId);
                ...
                {selectedNoteId && (
                  <div className="grid gap-2">
                    ...
                    <RadioGroup
                      value={updateMethod}
                      onValueChange={(value) =>
                        setUpdateMethod(value as UpdateMethod)
                      }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to completely replace the content of{" "}
