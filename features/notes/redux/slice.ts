  ...
  reducers: {
    ...
    registerInstance(state, action: PayloadAction<string>) {
      ...
      if (!state.instances[id]) {
        state.instances[id] = { id, openTabs: [], activeTabId: null, findReplace: null, splitNoteId: null };
      }
    },
    ...
    createAutogeneratedNote(
      ...
      // Also add to instance tabs and set active
      const inst = state.instances[instanceId];
      if (inst) {
        if (!inst.openTabs.includes(id)) inst.openTabs.push(id);
        inst.activeTabId = id;
      }
    },
