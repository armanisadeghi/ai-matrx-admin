...
export const autoSaveMiddleware: Middleware = (storeApi) => (next) => (action) => {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      if (currentRecord._isAutogenerated) {
        // ── First save: INSERT (materialize) ─────────────────────
        const userId = currentRecord.user_id || (currentState as any).user?.id;
        if (!userId) {
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            folder_name: currentRecord.folder_name,
            tags: currentRecord.tags,
            metadata: currentRecord.metadata,
            position: currentRecord.position ?? 0,
          })
          .select("updated_at")
          .single();
        ...
        console.log("[AutoSave] Materialized note", noteId);
      } else {
        // ── Subsequent saves: UPDATE ──────────────────────────────
        // Only send dirty fields
        const updates: Record<string, unknown> = {};
        for (const field of currentRecord._dirtyFields) {
          updates[field] = currentRecord[field];
        }
      ...
      if (currentRecord._dirtyFields.has("label")) {
        window.dispatchEvent(
          new CustomEvent("notes:labelChange", {
            detail: { noteId, label: currentRecord.label },
          }),
        );
      }
    } catch (err) {
