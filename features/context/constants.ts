...
import type { ContextItemStatus, ContextFetchHint, ContextSensitivity, ContextValueType } from './types';

// Status lifecycle phases
export const STATUS_PHASES = [
  { key: 'discovery', label: 'Discovery', statuses: ['idea', 'stub'] as ContextItemStatus[] },
  { key: 'collection', label: 'Collection', statuses: ['gathering', 'partial', 'needs_review'] as ContextItemStatus[] },
  { key: 'refinement', label: 'Refinement', statuses: ['ai_enriched', 'in_revision', 'pending_approval'] as ContextItemStatus[] },
  { key: 'live', label: 'Live', statuses: ['active', 'provisional'] as ContextItemStatus[] },
  { key: 'maintenance', label: 'Maintenance', statuses: ['stale', 'needs_update', 'superseded', 'archived', 'deprecated'] as ContextItemStatus[] },
] as const;
...
export const STATUS_TRANSITIONS: Record<ContextItemStatus, ContextItemStatus[]> = {
  idea: ['stub'],
  stub: ['gathering'],
...
export const ATTENTION_STATUSES: ContextItemStatus[] = [
  'needs_review', 'stale', 'needs_update', 'partial', 'ai_enriched',
];

// Fetch hint display config
export const FETCH_HINT_CONFIG: Record<ContextFetchHint, { label: string; description: string; iconName: string }> = {
  always: { label: 'Always Pre-load', description: 'Included in every agent context automatically', iconName: 'Zap' },
  on_demand: { label: 'On Demand', description: 'Agent fetches when it decides this item is relevant', iconName: 'MousePointerClick' },
  batch_related: { label: 'Batch with Related', description: 'Fetched together with related items as a group', iconName: 'Layers' },
  lazy: { label: 'Lazy', description: 'Only fetched if explicitly referenced or needed', iconName: 'Coffee' },
  never: { label: 'Never', description: 'Not available for agent access — internal reference only', iconName: 'EyeOff' },
};

// Sensitivity display config
export const SENSITIVITY_CONFIG: Record<ContextSensitivity, { label: string; description: string; iconName: string }> = {
  public: { label: 'Public', description: 'Visible to all agents and users', iconName: 'Globe' },
  internal: { label: 'Internal', description: 'Visible within the organization only', iconName: 'Building2' },
  restricted: { label: 'Restricted', description: 'Limited to specific roles or teams', iconName: 'Lock' },
  privileged: { label: 'Privileged', description: 'Highest sensitivity — owner and admins only', iconName: 'ShieldCheck' },
};

// Value type display config
export const VALUE_TYPE_CONFIG: Record<ContextValueType, { label: string; iconName: string }> = {
...
export const INDUSTRY_CATEGORIES = [
  ...
  { key: 'web_development_agency', label: 'Web Development Agency', iconName: 'Code2' },
