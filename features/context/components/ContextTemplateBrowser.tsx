...
import { Code2, Search, Users, Scale, Shield, Heart, ShoppingBag, Store, Brain, Globe, ChevronRight, Bot, Building2, FolderKanban, ListTodo, User } from 'lucide-react';
    ...
    return (
      <div className="space-y-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {item.example_value && (
                    <details className="mt-1">
                      <summary className="text-[10px] text-primary cursor-pointer">See example</summary>
                      <pre className="text-[9px] font-mono bg-muted/50 rounded p-1 mt-1 overflow-x-auto">
                        {typeof item.example_value === 'string' ? item.example_value : JSON.stringify(item.example_value, null, 2)}
                      </pre>
                    </details>
                  )}
                </div>
        ...
        <Button
          size="sm"
          className="text-xs"
          onClick={() => setApplyDialog({ industry: selectedIndustry, items: detailItems })}
        >
...
function ApplyTemplateDialog({
  items, scope, onApply, onClose, isPending,
}: {
  items: ContextTemplate[];
  scope: ScopeState;
  onApply: (selectedItems: ContextTemplate[]) => void;
  onClose: () => void;
  isPending: boolean;
}) {
  const [step, setStep] = useState<ApplyStep>('scope');
  const [selectedScope, setSelectedScope] = useState(scope);
  const [checkedIds, setCheckedIds] = useState<Set<string>>(() => new Set(items.map(i => i.id)));
  ...
  // Fetch existing keys when moving to confirm step
  useEffect(() => {
    if (step === 'confirm') {
      setLoadingKeys(true);
      contextService.fetchExistingKeys(selectedScope.scopeType, selectedScope.scopeId)
        .then(keys => setExistingKeys(keys))
        .finally(() => setLoadingKeys(false));
    }
  }, [step, selectedScope]);

  // Pre-select recommended scope from template items
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <div className="flex gap-1 mt-2">
            {(['scope', 'items', 'confirm'] as ApplyStep[]).map((s, i) => (
              <div key={s} className={`flex-1 h-1 rounded-full ${step === s ? 'bg-primary' : i < (['scope', 'items', 'confirm'] as ApplyStep[]).indexOf(step) ? 'bg-primary/40' : 'bg-muted'}`} />
            ))}
          </div>
        </DialogHeader>
              ...
              {(['user', 'organization', 'workspace', 'project', 'task'] as ContextScopeLevel[]).map(s => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setSelectedScope({ ...selectedScope, scopeType: s })}
                  >
                    <Icon className="h-4 w-4 text-muted-foreground shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium capitalize">{s}</p>
                      {s === scope.scopeType && <p className="text-[10px] text-muted-foreground">{scope.scopeName}</p>}
                    </div>
                    {isRecommended && <Badge variant="secondary" className="h-4 text-[9px] px-1 shrink-0">Recommended</Badge>}
                  </button>
                );
              })}
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Pre-populate values using available context</p>
                </div>
                {/* TODO: Wire agent — pre-populates value fields for selected template items using org/project context */}
              </button>
                ...
                <div>
                  ...
                  {required.map(item => (
                    <div key={item.id} className="flex items-center gap-2 py-1">
                      ...
                      <Badge variant="outline" className="h-4 text-[9px] ml-auto font-mono">{item.item_key}</Badge>
                    </div>
                  ))}
                </div>
              )}
          ...
          {step === 'confirm' && (
            <div className="space-y-3">
              {loadingKeys ? (
                ...
                <>
                  <p className="text-xs">
                    Creating <span className="font-semibold">{toCreate.length}</span> context items in <span className="font-semibold capitalize">{selectedScope.scopeType}: {selectedScope.scopeName}</span>
                  </p>
                  {toCreate.length > 0 && (
                    <div className="space-y-0.5">
                      {toCreate.map(item => (
                        <div key={item.id} className="flex items-center gap-2 py-0.5">
                          ...
                          <span className="text-xs">{item.item_display_name}</span>
                          <code className="text-[9px] font-mono text-muted-foreground ml-auto">{item.item_key}</code>
                        </div>
                      ))}
                    </div>
                  )}
                  {toSkip.length > 0 && (
                    <div>
                      <p className="text-[10px] text-muted-foreground mb-1">Already exists — will skip:</p>
                      <div className="space-y-0.5 opacity-50">
                        {toSkip.map(item => (
                          <div key={item.id} className="flex items-center gap-2 py-0.5">
                            <span className="h-1.5 w-1.5 rounded-full bg-gray-400" />
                            <span className="text-xs line-through">{item.item_display_name}</span>
                            <code className="text-[9px] font-mono text-muted-foreground ml-auto">{item.item_key}</code>
                          </div>
                        ))}
                      </div>
        ...
        <DialogFooter>
          ...
          {step === 'confirm' ? (
            <Button size="sm" className="text-xs" onClick={() => onApply(selectedItems)} disabled={isPending || loadingKeys}>
