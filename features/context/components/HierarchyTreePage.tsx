...
export function HierarchyTreePage() {
  ...
  const orgScopeTypes = useAppSelector((state) => {
    ...
    for (const orgId of orgIds) {
      result[orgId] = selectScopeTypesByOrg(state, orgId);
    }
    return result;
  });
  ...
  const orgScopes = useAppSelector((state) => {
    ...
    for (const orgId of orgIds) {
      result[orgId] = selectScopesByOrg(state, orgId);
    }
    return result;
  });
            ...
            {filteredTree.map((node) => (
              <TreeRow
                ...
                onDelete={(type, id, name) =>
                  setDeleteConfirm({ type: type as HierarchyNodeType, id, name })
                }
              />
            ))}
...
function TreeRow({
  ...
  const accent = iconColor ? "" : (ACCENT_TEXT[node.type] ?? "text-muted-foreground");
