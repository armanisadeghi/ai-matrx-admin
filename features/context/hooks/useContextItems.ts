...
const KEYS = {
  manifest: (scope: string, id: string) => ['context-manifest', scope, id] as const,
  ...
  attention: (scope: string, id: string) => ['context-attention', scope, id] as const,
  accessLog: (scope: string, id: string) => ['context-access-log', scope, id] as const,
  ...
  accessVolume: (scope: string, id: string) => ['context-access-volume', scope, id] as const,
  usageRankings: (scope: string, id: string) => ['context-usage-rankings', scope, id] as const,
};

// ─── Manifest (item list) ──────────────────────────────────────────

export function useContextManifest(scopeType: ContextScopeLevel, scopeId: string) {
  return useQuery({
...
export function useContextDashboardStats(scopeType: ContextScopeLevel, scopeId: string) {
  return useQuery({
...
export function useContextCategoryHealth(scopeType: ContextScopeLevel, scopeId: string) {
  return useQuery({
...
export function useContextAttentionQueue(scopeType: ContextScopeLevel, scopeId: string) {
  return useQuery({
...
export function useContextAccessVolume(scopeType: ContextScopeLevel, scopeId: string, days = 30) {
  return useQuery({
...
export function useContextUsageRankings(scopeType: ContextScopeLevel, scopeId: string) {
  return useQuery({
...
export function useCreateContextItem(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (formData: ContextItemFormData) =>
      contextService.createItem(scopeType, scopeId, formData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: KEYS.manifest(scopeType, scopeId) });
      queryClient.invalidateQueries({ queryKey: KEYS.stats(scopeType, scopeId) });
...
export function useUpdateContextItem(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ itemId, updates }: { itemId: string; updates: Partial<ContextItemFormData> }) =>
...
export function useUpdateContextStatus(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ itemId, status, statusNote }: { itemId: string; status: ContextItemStatus; statusNote?: string }) =>
      contextService.updateStatus(itemId, status, statusNote),
    onMutate: async ({ itemId, status }) => {
      await queryClient.cancelQueries({ queryKey: KEYS.manifest(scopeType, scopeId) });
      const previous = queryClient.getQueryData<ContextItemManifest[]>(KEYS.manifest(scopeType, scopeId));
      if (previous) {
    ...
    onError: (err: Error, _vars, context) => {
      if (context?.previous) {
        queryClient.setQueryData(KEYS.manifest(scopeType, scopeId), context.previous);
      }
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: KEYS.item(data.id) });
      queryClient.invalidateQueries({ queryKey: KEYS.stats(scopeType, scopeId) });
      queryClient.invalidateQueries({ queryKey: KEYS.attention(scopeType, scopeId) });
      toast.success(`Status updated to ${data.status}`);
    },
  });
}

export function useCreateContextValue(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ itemId, valueData, sourceType }: { itemId: string; valueData: ContextValueFormData; sourceType?: string }) =>
...
export function useArchiveContextItem(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
...
export function useDuplicateContextItem(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
...
export function useApplyTemplate(scopeType: ContextScopeLevel, scopeId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (templateItems: ContextTemplate[]) => {
      const existingKeys = await contextService.fetchExistingKeys(scopeType, scopeId);
      return contextService.applyTemplate(scopeType, scopeId, templateItems, existingKeys);
    },
