  ...
  reducers: {
    ...
    startSession: (state, action: PayloadAction<StartSessionPayload>) => {
      ...
      const session: ConversationSession = {
        ...
        messages: [],
        toolCallsById: {},
        protocolDbMessages: null,
        protocolDbToolCalls: null,
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
    ...
    setConversationId: (
      state,
      action: PayloadAction<SetConversationIdPayload>,
    ) => {
      const session = state.sessions[action.payload.sessionId];
      if (!session) return;
    ...
    appendStreamChunk: (
      state,
      action: PayloadAction<AppendStreamChunkPayload>,
    ) => {
    ...
    clearMessages: (state, action: PayloadAction<string>) => {
      ...
      session.protocolDbMessages = null;
      session.protocolDbToolCalls = null;
      session.updatedAt = Date.now();
    },

    /** Clear raw DB snapshot used for chat-protocol (`buildCanonicalMessages`) only. */
    clearProtocolDbSnapshot: (state, action: PayloadAction<string>) => {
      const session = state.sessions[action.payload];
      if (!session) return;
      session.protocolDbMessages = null;
      session.protocolDbToolCalls = null;
    ...
    updateVariable: (state, action: PayloadAction<UpdateVariablePayload>) => {
      ...
      const varDef = session.variableDefaults.find(
    ...
    setExpandedVariable: (
      state,
      action: PayloadAction<SetExpandedVariablePayload>,
    ) => {
    ...
    setModelOverride: (
      state,
      action: PayloadAction<{ sessionId: string; model: string | null }>,
    ) => {
    ...
    applyMessageHistory: (
      state,
      action: PayloadAction<ApplyMessageHistoryPayload>,
    ) => {
      const { sessionId, messageId, updatedRawContent, updatedContentHistory } =
        action.payload;
      const session = state.sessions[sessionId];
      if (!session) return;
