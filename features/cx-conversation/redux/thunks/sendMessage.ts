...
function buildChatBody(
  ...
  if (config.internalWebSearch != null)
    body.internal_web_search = config.internalWebSearch;
  if (config.internalUrlContext != null)
    body.internal_url_context = config.internalUrlContext;
  if (config.reasoningEffort != null)
    body.reasoning_effort = config.reasoningEffort;
  if (config.reasoningSummary != null)
    body.reasoning_summary = config.reasoningSummary;
  ...
  if (config.includeThoughts != null)
    body.include_thoughts = config.includeThoughts;
...
>(
  ...
  async (
    { sessionId, content, resources = [], variables = {}, signal },
    { dispatch, getState },
  ) => {
    ...
    const blockMode = isAdmin && uiState.isBlockMode;
    ...
    const messageContext: Record<string, unknown> =
      state.activeChat?.messageContext ?? {};
    ...
    dispatch(
      chatConversationsActions.addMessage({
        sessionId,
        message: {
          id: userMessageId,
          role: "user",
    ...
    dispatch(
      chatConversationsActions.setSessionStatus({
        sessionId,
        status: "executing",
      }),
    );
    ...
    const onStreamStart = (
      ...
      if (conversationId) {
        dispatch(
          chatConversationsActions.setConversationId({
            sessionId,
            conversationId,
          }),
        );
      ...
      switch (event.event) {
        ...
        case "chunk": {
          ...
          if (blockMode) {
            ...
            dispatch(
              chatConversationsActions.appendStreamChunk({
                sessionId,
                messageId: assistantMessageId,
                chunk: text,
              }),
        ...
        case "error": {
          ...
          const errorMessage =
            errData.user_message || errData.message || "Stream error";
          dispatch(
            chatConversationsActions.updateMessage({
              sessionId,
              messageId: assistantMessageId,
    ...
    const onStreamComplete = (
      ...
      const resolvedConversationId =
        _conversationId ?? selectConversationId(getState(), sessionId);
          ...
          if (hasArtifacts(accumulatedContent)) {
            ...
            if (realMessage?.id) {
              persistArtifactsFromContent(
                ...
                console.error(
                  "[sendMessage] artifact persistence failed:",
                  err,
                ),
              );
            }
    ...
    const onStreamError = (error: ApiCallError) => {
      const message =
        error.type === "abort_error" ? "Request cancelled" : error.message;
      dispatch(
        chatConversationsActions.updateMessage({
          sessionId,
          messageId: assistantMessageId,
    ...
    try {
      ...
      if (apiMode === "chat") {
        ...
        if (!existingConversationId)
          throw new Error(
            "Conversation mode requires a conversationId on the session",
          );
        result = await dispatch(
          callApi({
