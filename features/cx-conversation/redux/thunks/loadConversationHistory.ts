  ...
  async ({ sessionId, conversationId, agentId }, { dispatch }) => {
    ...
    try {
      ...
      const messages: ConversationMessage[] = processedMessages.map((msg) => ({
        ...
        agentId: msg.agentId,
        source: msg.source,
        isVisibleToModel: msg.isVisibleToModel,
        isVisibleToUser: msg.isVisibleToUser,
        userContent: msg.userContent,
        dbMetadata: msg.dbMetadata,
        contentHistory: (Array.isArray(msg.contentHistory)
          ? msg.contentHistory
          : null) as CxContentHistoryEntry[] | null,
        createdAt: msg.createdAt,
        deletedAt: msg.deletedAt,
        // Derived
        toolUpdates: msg.toolUpdates?.length > 0 ? msg.toolUpdates : undefined,
        rawToolCalls:
          msg.rawToolCalls?.length > 0 ? msg.rawToolCalls : undefined,
        isCondensed: msg.isCondensed || undefined,
      }));

      dispatch(
        chatConversationsActions.loadConversation({
          sessionId,
          conversationId,
          agentId,
          messages,
