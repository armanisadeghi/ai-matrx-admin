...
import { getMessageActions, resumePendingAuthAction } from '@/features/cx-conversation/actions/messageActionRegistry';
...
const MessageOptionsMenu: React.FC<MessageOptionsMenuProps> = ({
    ...
    const instance = useAppSelector((state) => selectMessageActionInstance(state, instanceId));
    ...
    const hasUnsavedChanges = useAppSelector((state) =>
        instance?.sessionId && instance?.messageId
            ? selectMessageHasUnsavedChanges(state, instance.sessionId, instance.messageId)
            : false,
    );
    ...
    useEffect(() => {
        if (instance) {
            resumePendingAuthAction(isAuthenticated, instance.content, instanceId, dispatch);
        }
