 ...
 *
 * Returns MenuItem[] based on a context object. All sub-modal actions dispatch to
 * messageActionsSlice (rendered by MessageActionsController at the app root).
...
function requireAuth(
    ...
    if (!ctx.isAuthenticated) {
        ...
        ctx.dispatch(
            messageActionsActions.openOverlay({
...
export function getMessageActions(ctx: MessageActionContext): MenuItem[] {
    const {
        instanceId, content, isAuthenticated, sessionId, messageId,
        rawContent, metadata, hasUnsavedChanges, hasHistory,
        dispatch, onClose, showFullPrint, onFullPrint, isCapturing, ttsState,
    } = ctx;
    ...
    const items: MenuItem[] = [
        ...
        },
        {
            ...
            action: async () => {
                if (!sessionId || !messageId) return;
                const contentBlocks = buildContentBlocksForSave(content, rawContent ?? undefined);
        ...
        // ── Actions (auth-gated) ────────────────────────────────────
        {
            ...
            label: 'Add to Tasks',
            action: () => {
                if (!requireAuth(ctx, 'add-to-tasks', 'Add to Tasks', 'Sign in to create and track tasks from your AI responses.')) return;
        ...
        // ── Audio ────────────────────────────────────────────────────
        {
            ...
            disabled: ttsState.isTtsGenerating || ttsState.isTtsPlaying || ttsState.isBrowserTtsPlaying,
        },
        ...
        // ── Export ───────────────────────────────────────────────────
        {
            ...
            label: 'HTML preview',
            action: () => {
                dispatch(messageActionsActions.openOverlay({ instanceId, overlay: 'htmlPreview' }));
                onClose();
            },
            category: 'Export',
            showToast: false,
        ...
        },
        {
            ...
            action: async () => {
                if (!isAuthenticated) {
                    dispatch(messageActionsActions.openOverlay({ instanceId, overlay: 'emailDialog' }));
                    return;
                }
        ...
        },
        {
            ...
            action: () => { printMarkdownContent(content, 'AI Response'); onClose(); },
            category: 'Export',
            showToast: false,
        },
        ...(showFullPrint && onFullPrint ? [{
            ...
            action: () => { if (!isCapturing) { onFullPrint(); onClose(); } },
        ...
        // ── Save / Actions ───────────────────────────────────────────
        {
            ...
            action: async () => {
                if (!requireAuth(ctx, 'save-scratch', 'Save to Scratch', 'Sign in to save notes to your Scratch folder.')) return;
        ...
        },
        {
            ...
            label: 'Save to Notes',
            action: () => {
                if (!requireAuth(ctx, 'save-notes', 'Save to Notes', 'Sign in to save notes and organize your AI responses.')) return;
                dispatch(messageActionsActions.openOverlay({ instanceId, overlay: 'saveToNotes' }));
            },
            category: 'Actions',
            showToast: false,
        },
...
export function resumePendingAuthAction(
    ...
    try {
        ...
        if (action === 'save-scratch') {
            ...
            dispatch(openOverlay({
                overlayId: 'quickTasks',
                data: { content: savedContent, prePopulate: { title: 'New Task from AI Response', description: savedContent, metadataInfo: '' } },
            }));
        }
