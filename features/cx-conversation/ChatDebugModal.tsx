...
export function ChatDebugModal({ sessionId, isOpen, onClose }: ChatDebugModalProps) {
    ...
    const showSystemMessages = useAppSelector((s) => selectShowSystemMessages(s, sessionId));
            ...
            <DialogContent className="sm:max-w-[480px] max-h-[85vh] overflow-y-auto">
                ...
                <div className="grid gap-4 py-2">
                            ...
                            <div className="flex items-center justify-between space-x-2 bg-red-50 dark:bg-red-950/20 p-3 rounded-lg border border-red-200 dark:border-red-800">
                                <Label htmlFor="global-debug" className="flex flex-col space-y-1">
                                    ...
                                    <span className="font-normal text-xs text-red-600 dark:text-red-400">
                                        Enable system-wide debug features and indicators across all sessions
                                    </span>
                                </Label>
                                <Switch
                                    id="global-debug"
                    ...
                    {isAdmin && (
                        <>
                            <div className="flex items-center justify-between space-x-2">
                                <Label htmlFor="use-localhost" className="flex flex-col space-y-1">
                            ...
                            <div className="space-y-2">
                                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Session State</p>
                                <div className="rounded-md bg-muted/60 border border-border p-3 text-[10px] font-mono space-y-1 text-muted-foreground">
                                    <div><span className="text-foreground">sessionId:</span> {sessionId}</div>
                                    <div><span className="text-foreground">conversationId:</span> {session?.conversationId ?? "—"}</div>
                                    <div><span className="text-foreground">agentId:</span> {session?.agentId ?? "—"}</div>
                                    <div><span className="text-foreground">apiMode:</span> {session?.apiMode ?? "—"}</div>
                                    <div><span className="text-foreground">status:</span> {session?.status ?? "—"}</div>
                                    <div><span className="text-foreground">messages:</span> {messages.length}</div>
                                    <div><span className="text-foreground">resources:</span> {resources.length}</div>
                                    <div><span className="text-foreground">modelOverride:</span> {uiState.modelOverride ?? "—"}</div>
                                    <div><span className="text-foreground">useLocalhost:</span> {String(uiState.useLocalhost)}</div>
                                    <div><span className="text-foreground">useBlockMode:</span> {String(uiState.useBlockMode)}</div>
                                </div>
                            </div>

                            {/* Model settings (only if any are set) */}
                            {uiState.modelSettings && Object.keys(uiState.modelSettings).length > 0 && (
                                <div className="space-y-2">
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Model Settings Override</p>
