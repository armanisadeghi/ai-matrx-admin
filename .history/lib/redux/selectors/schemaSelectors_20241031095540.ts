...
export const selectGlobalCache = (state: RootState): GlobalCacheState =>
  state.globalCache;
...
export const selectConversionMaps = createSelector(
    [selectGlobalCache],
    (cache) => ({
...
interface QueryConversionPayload<T extends EntityKeys> {
    entityName: T;
      ...
      return {
        ...
        filters: options.filters
          ? Object.entries(options.filters).reduce((acc, [key, value]) => {
              const dbField = fieldMap[key as string] || key;
              acc[dbField] = value;
              return acc;
            }, {} as Record<string, unknown>)
          : undefined,
        sorts: options.sorts?.map(sort => ({
          column: fieldMap[sort.column as string] || sort.column,
          ascending: sort.ascending
        })),
        columns: options.columns?.map(
          field => fieldMap[field as string] || field
        ),
        limit: options.limit,
...
export const makeSelectEntityFormat = () =>
  createSelector(
    [
      ...
      (_: RootState, params: { entityName: EntityKeys; format: string }) =>
        params,
    ],
...
export const makeSelectFieldFormat = () =>
  createSelector(
    [
      ...
      (
        _: RootState,
        params: { entityName: EntityKeys; fieldName: string; format: string }
      ) => params,
    ],
...
export const makeSelectFieldToDatabase = () =>
  createSelector(
    ...
    (maps, { entityName, fieldName }) =>
      maps.fieldToDatabase[entityName]?.[fieldName]
  );
...
export const makeSelectFieldToBackend = () =>
  createSelector(
    ...
    (maps, { entityName, fieldName }) =>
      maps.fieldToBackend[entityName]?.[fieldName]
  );
...
export const makeSelectFieldToFrontend = () =>
  createSelector(
    ...
    (maps, { entityName, fieldName }) =>
      maps.fieldToCanonical[entityName]?.[fieldName]
  );
...
export const useFieldToDatabase = (
  entityName: EntityKeys,
  fieldName: string
) => {
...
export const useFieldToBackend = (
  entityName: EntityKeys,
  fieldName: string
) => {
...
export const useFieldToFrontend = (
  entityName: EntityKeys,
  fieldName: string
) => {
...
export const useFieldFormat = (
  ...
  return useSelector((state) =>
    select(state, { entityName, fieldName, format })
  );
};
