...
import {createFieldId, EntityNameOfficial} from "@/types/schema";
...
// Basic state selectors with memoization
...
export const selectEntityNames = (state: RootState) =>
    state.globalCache.entityNames;
...
export const makeSelectEntityFields = () =>
    createSelector(
        [
            (state: RootState) => state.globalCache.fields,
            (state: RootState, entityName: EntityNameOfficial) =>
                state.globalCache.fieldsByEntity[entityName] || []
        ],
        (fields, fieldIds) => fieldIds.map(id => fields[id])
    );
