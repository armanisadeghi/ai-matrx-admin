...
function* withConversion<TEntity extends EntityKeys>(
    ...
    try {
      ...
      // Extract options from action.payload, defaulting to empty object if undefined
      const options = action.payload.options || {};

      // Convert frontend options to database query options
      const dbQueryOptions: QueryOptions<TEntity> = yield select(
        selectDatabaseConversion,
        entityKey,
        options
      );
