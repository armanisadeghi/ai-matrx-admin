...
export function createEntitySlice<TEntity extends EntityKeys>(
    ...
    const initialState: EntitySliceState<TEntity> = {
        ...
        initialized: false,
        loading: false,
        error: null,        import { createSlice, PayloadAction } from '@reduxjs/toolkit';
        import { Draft } from 'immer';
        import { AutomationEntity, EntityData, EntityKeys } from "@/types/entityTypes";
        ...
        export function createEntitySlice<TEntity extends EntityKeys>(
            ...
            const slice = createSlice({
                name: entityKey.toUpperCase(), // Use entity name directly
                initialState,
                reducers: {
                    fetchRequest: (state) => {
                        state.loading = true;
    ...
    const slice = createSlice({
        ...
        reducers: {
            ...
            setBackup: (
                ...
                state.backups[action.payload.key] = action.payload.data as Draft<Array<EntityData<TEntity>>>;
            },
            setAllPkAndDisplayFields: (
                state: Draft<EntitySliceState<TEntity>>,
                action: PayloadAction<Array<{ pk: string; display?: string }>>
            ) => {
                state.allPkAndDisplayFields = action.payload as Draft<Array<{ pk: string; display?: string }>>;
            },
            setStale: (
                state: Draft<EntitySliceState<TEntity>>,
                action: PayloadAction<boolean>
