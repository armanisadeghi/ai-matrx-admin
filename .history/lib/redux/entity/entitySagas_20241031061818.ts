...
function* handleFetchPaginated<TEntity extends EntityKeys>(
  ...
  try {
    ...
    const to = from + action.payload.pageSize - 1;

    // Build query with any additional options from the payload
    let query = databaseApi
      .select('*', { count: 'exact' })
      .range(from, to);
    ...
    if (action.payload.options) {
      ...
      if (action.payload.options.filters) {
        // Apply filters
        query = applyFilters(query, action.payload.options.filters);
      }
      if (action.payload.options.sort) {
        // Apply sorting
        query = applySorting(query, action.payload.options.sort);
      }
    }

    const { data, error, count } = yield call([query, query.then]);

    if (error) {
      throw error;
    }

    const frontendData = yield select(selectFrontendConversion, data);

    yield put(actions.fetchPaginatedSuccess({
      data: frontendData,
