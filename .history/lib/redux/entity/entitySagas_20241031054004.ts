...
function* handleFetchOne<TEntity extends EntityKeys>(
    ...
    try {
      console.log('handleFetchOne starting with:', entityKey, action.payload);
      ...
      yield put(actions.fetchOneFailure(error));
    }
  }
...
function* handleCreate<TEntity extends EntityKeys>(
  ...
  try {
    ...
    if (data) {
      const currentData = yield select((state) => state[entityKey].data);
