...
const entityReducers = Object.keys(automationEntities).reduce((acc, entityName) => {
    const entitySchema = automationEntities[entityName as keyof typeof automationEntities];
    const entitySlice = createEntitySlice(entityName as EntityName, entitySchema);
    acc[entityName] = entitySlice.reducer;
    return acc;
}, {} as Record<string, any>);
