...
const FlashcardComponent: React.FC = () => {
    const [cards, setCards] = useState<Flashcard[]>(
        InitialFalshcardsWithExample.map(card => ({
            ...card,
            reviewCount: 0,
            correctCount: 0,
            incorrectCount: 0
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-background text-foreground p-4">
            ...
            <div className="w-full max-w-4xl flex justify-between items-start mb-4">
                ...
                <div className="w-3/4 pl-4">
                    ...
                    <div className="w-full h-80 [perspective:1000px] mt-4">
                        <div
                        ...
                        >
                            <div className="absolute inset-0 w-full h-full [backface-visibility:hidden]">
                                <Card className="w-full h-full flex flex-col bg-card">
                                    <CardContent className="flex-grow flex items-center justify-center p-6 overflow-auto">
                                        <p className="text-center" style={{ fontSize: `${fontSize}px` }}>
                                            {cards[currentIndex].front}
                                        </p>
                                    </CardContent>
                                    <CardFooter className="flex justify-between p-2">
                                        <Button onClick={(e) => { e.stopPropagation(); handleAnswer(false); }} variant="destructive">
                                            <XCircle className="mr-2 h-4 w-4" /> Incorrect
                                        </Button>
                                        <Button onClick={(e) => { e.stopPropagation(); handleAnswer(true); }} variant="default" className="bg-green-500 hover:bg-green-600">
                            ...
                            <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)]">
                                <Card className="w-full h-full flex flex-col bg-card">
                                    <CardContent className="flex-grow flex flex-col items-start justify-start p-6 overflow-auto">
                                        <p className="text-left mb-4" style={{ fontSize: `${fontSize}px` }}>
                                            {cards[currentIndex].back}
                                        </p>
                                        <div className="w-full border-t border-primary my-2"></div>
                                        <p className="text-left text-blue-500" style={{ fontSize: `${fontSize}px` }}>
