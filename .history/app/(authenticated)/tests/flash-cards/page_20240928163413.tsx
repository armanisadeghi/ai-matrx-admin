        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Card className="w-full h-full relative transform-style-3d">
              <CardContent className={`absolute inset-0 flex items-center justify-center p-6 backface-hidden ${isFlipped ? 'rotate-y-180' : ''}`}>
                <p className="text-xl text-center">{cards[currentIndex].front}</p>
              </CardContent>
              <CardContent className={`absolute inset-0 flex items-center justify-center p-6 backface-hidden rotate-y-180 ${isFlipped ? '' : 'invisible'}`}>
